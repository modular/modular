...
> Note: Mojo has `@register_passable("trivial")` as well, which isn't really
...
fn example2():
  ...
  use_rp_type_owned(a_rp^) # pass by owned
...
```mlir
  ...
  kgen.generator @example2() {
    // lit.var.decl turns int oa stack allocation with explicit lifetime
    // markers.
    %0 = pop.stack_allocation 1 x struct<()> marked
    %1 = kgen.call @MyRPType::__init__()
    pop.stack_alloc.lifetime.start(%0)
    pop.store %1, %0
    
    %2 = kgen.call @use_rp(%0)
    %3 = kgen.call @use_rp_type_mut(%0)
    %4 = kgen.call @use_rp_type_owned(%0)
    pop.stack_alloc.lifetime.end(%0)
...
Of course, the BODY of these functions can be doing anything pointer - it need
not be simple loads and stores. For example, the owned version might pass the
owned pointer as a mutable argument to another function:

```mojo
@export
fn other_owned(owned a: MyRPType):
  use(a)
...
Remember that this function had a call to the destructor… and if you scroll
above you'll notice that it is passing the address down to the destructor,
because the destructor function also uses the value owned and before this pass,
owned non-trivial types are passed by memory.
...
around and looking at the IR to see that `owned` and `mut` ”just work” as well
...
```mlir
  kgen.func @"take_generic_type,T=Int"(%arg0: !kgen.pointer<index> read_mem){
  }
  kgen.func @"take_generic_type,T=String"(%arg0: !kgen.pointer<struct<(struct<(pointer<none>, index, index) memoryOnly>) memoryOnly>> read_mem) -> !kgen.none {
  }
  kgen.func @"take_generic_type,T=MyRPType"(%arg0: !kgen.pointer<struct<()>> read_mem) -> !kgen.none {
  }
```
...
```mlir
    kgen.call @"take_generic_type, T="(%0) : (!kgen.pointer<struct<(struct<(pointer<none>, index, index) memoryOnly>) memoryOnly>> read_mem) -> !kgen.none
```
...
```mlir
  kgen.func @"take_generic_type,T=Int"(%arg0: index) {
    %0 = pop.stack_allocation 1 x index
    pop.store %arg0, %0 : !kgen.pointer<index>
  }
  kgen.func @"take_generic_type,T=String"(%arg0: !kgen.pointer<struct<(struct<(pointer<none>, index, index) memoryOnly>) memoryOnly>> read_mem) {
  }
  kgen.func @"take_generic_type,T=MyRPType"(%arg0: struct<()>) {
    %0 = pop.stack_allocation 1 x struct<()>
    pop.store %arg0, %0 : !kgen.pointer<struct<()>>
  }
```
...
<500 lines of C++ code) use information that is only available after
...
The compiler interprets this variadic pack syntax as saying that example4 takes
a `VariadicPack` instance as an argument directly. It is as if you write the
callee like this:
...
all argument conventions, including `owned` and `mut` and `ref` , this is just
...
it is a non-trivial `@register_passable` type (because the `owned` form needs a
