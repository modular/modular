...
//
// A conversion thunk (generated by generateConversionThunk in
// ExprConversions.cpp) adapts a user kernel's signature for
// enqueue_function_experimental. The original user kernel (e.g.
// @add_one_kernel) is encoded as the last compile-time parameter of the
// thunk's SymbolConstantAttr. The setter/getter pair below maintains this
// convention.
//===----------------------------------------------------------------------===//
...
/// Encode the user kernel as the last compile-time parameter of a conversion
/// thunk (see generateConversionThunk in ExprConversions.cpp). Call this after
/// appending all other index bindings for the thunk. The convention established
/// here is what getThunkCallee() relies on to recover the user kernel.
void appendThunkCallee(ParameterEvaluator &evaluator, TypedAttr calleeParam);

/// Recover the user kernel GeneratorOp encoded as the last compile-time
/// parameter of a conversion thunk's SymbolConstantAttr (see
/// appendThunkCallee / generateConversionThunk in ExprConversions.cpp).
/// Returns null if the symbol is not a thunk or the callee cannot be found.
