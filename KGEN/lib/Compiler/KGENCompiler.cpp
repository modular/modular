...
/// Metadata key used to identify compiled GPU kernels.
static constexpr llvm::StringLiteral kKernelIdMetadataKey =
    "kgen.offload.kernelid";

/// Append a kernel ID to a metadata array.
static void appendKernelId(SmallVector<Attribute> &metadata, MLIRContext *ctx,
                           uint64_t kernelId) {
  metadata.push_back(StringAttr::get(ctx, kKernelIdMetadataKey));
  metadata.push_back(IntegerAttr::get(IndexType::get(ctx), kernelId));
}

/// Extract LLVM metadata from the first callee function that has metadata,
/// filtering out any kgen.offload.kernelid that may have been set on the
/// callee. This handles the case where a kernel with metadata decorators is
/// passed through a constrained function type parameter, creating a wrapper.
/// The metadata from the original kernel needs to be propagated to the wrapper
/// (which becomes the GPU entry point).
///
/// Note: This only checks immediate callees. If the wrapper calls another
/// wrapper that in turn calls the kernel with metadata, the metadata will NOT
/// be propagated. This is intentional - we only support single-level wrapping
/// which is the pattern generated by constrained function type parameters.
///
/// Returns a pair of (metadataArray, argMetadataArray), both null if no
/// callee with metadata is found.
static std::pair<ArrayAttr, ArrayAttr>
extractCalleeMetadata(GeneratorOp func, SymbolTable &symtab) {
  ArrayAttr calleeMetadataArray;
  ArrayAttr calleeArgMetadataArray;
  func.getBodyRegion().walk([&](CallOp call) {
    // Stop if we've already extracted metadata from a previous callee.
    if (calleeMetadataArray)
      return WalkResult::interrupt();

    SymbolConstantAttr calleeSymbol = call.getCallee();
    StringAttr calleeName = calleeSymbol.getSymbol().getRootReference();
    if (auto *calleeOp = symtab.lookup(calleeName)) {
      if (auto calleeGen = dyn_cast<GeneratorOp>(calleeOp)) {
        // Filter out kgen.offload.kernelid from callee's metadata since each
        // kernel gets its own unique kernelId assigned later. The callee may
        // have already been processed and have its own kernelId set.
        ArrayAttr rawMetadata = calleeGen.getLLVMMetadataArrayAttr();
        if (rawMetadata && !rawMetadata.empty()) {
          SmallVector<Attribute> filtered;
          ArrayRef<Attribute> attrs = rawMetadata.getValue();
          for (size_t i = 0; i + 1 < attrs.size(); i += 2) {
            if (auto nameAttr = dyn_cast<StringAttr>(attrs[i])) {
              if (nameAttr.getValue() == kKernelIdMetadataKey)
                continue; // Skip kernelId - it will be set separately
            }
            filtered.push_back(attrs[i]);
            filtered.push_back(attrs[i + 1]);
          }
          if (!filtered.empty())
            calleeMetadataArray =
                ArrayAttr::get(calleeGen.getContext(), filtered);
        }
        calleeArgMetadataArray = calleeGen.getLLVMArgMetadataArrayAttr();
      }
    }
    return WalkResult::advance();
  });
  return {calleeMetadataArray, calleeArgMetadataArray};
}

/// Apply LLVM metadata to a target generator, propagating metadata from a
/// callee if available, otherwise falling back to the source generator's
/// metadata. This handles the constrained function type case where a wrapper
/// needs to inherit metadata from the kernel it calls.
///
/// The `target` is the generator to set metadata on (e.g., wrapper or sliced).
/// The `source` is the generator to walk for callees and fall back to if no
/// callee has metadata. The `symtab` is used for looking up callees. If
/// `kernelId` is provided, it is appended to the metadata.
static void applyPropagatedMetadata(GeneratorOp target, GeneratorOp source,
                                    SymbolTable &symtab,
                                    std::optional<uint64_t> kernelId) {
  // Extract callee metadata if source is a wrapper calling another function.
  auto [calleeMetadataArray, calleeArgMetadataArray] =
      extractCalleeMetadata(source, symtab);

  // Use callee metadata if available, otherwise use source's own metadata.
  SmallVector<Attribute> metadataArray;
  if (calleeMetadataArray && !calleeMetadataArray.empty())
    metadataArray = llvm::to_vector(calleeMetadataArray.getValue());
  else if (ArrayAttr sourceMetadata = source.getLLVMMetadataArrayAttr())
    metadataArray = llvm::to_vector(sourceMetadata.getValue());

  // Add kernel ID if provided.
  if (kernelId)
    appendKernelId(metadataArray, target->getContext(), *kernelId);
  target.setLLVMMetadataArrayAttr(
      ArrayAttr::get(target.getContext(), metadataArray));

  // Apply arg metadata: use callee's if available, otherwise source's.
  if (calleeArgMetadataArray && !calleeArgMetadataArray.empty())
    target.setLLVMArgMetadataArrayAttr(calleeArgMetadataArray);
  else
    target.setLLVMArgMetadataArrayAttr(source.getLLVMArgMetadataArrayAttr());
}
...
generateInstantiateStub(GeneratorOp func, SymbolConstantAttr symbol,
  ...
  // Apply metadata to the wrapper, propagating from callee if available.
  if (symtab) {
    applyPropagatedMetadata(wrapper, sliced, *symtab, kernelId);
  } else {
    // No symbol table - just copy metadata from sliced (no callee lookup).
    SmallVector<Attribute> metadataArray;
    if (ArrayAttr slicedMetadata = sliced.getLLVMMetadataArrayAttr())
      metadataArray = llvm::to_vector(slicedMetadata.getValue());
    if (kernelId)
      appendKernelId(metadataArray, sliced->getContext(), *kernelId);
      ...
      for (auto [op, symbolInfo] : offloadInfo.symbols) {
        ...
        for (auto [symbol, kernelInfo] : symbolInfo) {
          if (!symbol.getParamValues().empty()) {
            ...
            // Apply metadata to sliced, propagating from callee if available.
            GeneratorOp sliced = cast<GeneratorOp>(mapping.lookup(func));
            applyPropagatedMetadata(sliced, sliced, slicedSymtab,
                                    kernelInfo.kernelId);
          }
        }
        if (newName) {
