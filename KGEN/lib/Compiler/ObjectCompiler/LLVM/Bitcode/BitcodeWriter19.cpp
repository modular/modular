...
// Get this file from llvm-project/llvm/lib/Bitcode/Writer/BitcodeWriter.cpp
// with tag llvmorg-19.1.7

#include "BitcodeWriter19.h"
...
public:
  ...
  ModuleBitcodeWriterBase(const Module &M, StringTableBuilder &StrtabBuilder,
    ...
    // (which would have been assigned an ID by the ValueEnumerator19).
    // The starting ValueId is just after the number of values in the
    // ValueEnumerator19, so that they can be emitted in the VST.
...
void ModuleBitcodeWriter::writeAttributeGroupTable() {
  const std::vector<M::KGEN::ValueEnumerator19::IndexAndAttrSet> &AttrGrps =
  ...
  for (M::KGEN::ValueEnumerator19::IndexAndAttrSet Pair : AttrGrps) {
...
void ModuleBitcodeWriter::writeTypeTable() {
  const M::KGEN::ValueEnumerator19::TypeList &TypeList = VE.getTypes();
...
void ModuleBitcodeWriter::writeConstants(unsigned FirstVal, unsigned LastVal,
  ...
  const M::KGEN::ValueEnumerator19::ValueList &Vals = VE.getValues();
  Type *LastTy = nullptr;
...
void IndexBitcodeWriter::writeCombinedGlobalValueSummary() {
  ...
  for (auto &T : ReferencedTypeIds) {
    ...
    for (auto It = TidIter.first; It != TidIter.second; ++It) {
      writeTypeIdSummaryRecord(NameVals, StrtabBuilder, It->second.first.str(),
                               It->second.second);
...
BitcodeWriter19::BitcodeWriter19(SmallVectorImpl<char> &Buffer)
    : Stream(new BitstreamWriter(Buffer)) {
  writeBitcodeHeader(*Stream);
}

BitcodeWriter19::BitcodeWriter19(raw_ostream &FS)
    : Stream(new BitstreamWriter(FS, FlushThreshold)) {
  writeBitcodeHeader(*Stream);
}

BitcodeWriter19::~BitcodeWriter19() { assert(WroteStrtab); }

void BitcodeWriter19::writeBlob(unsigned Block, unsigned Record,
                                StringRef Blob) {
...
void BitcodeWriter19::writeSymtab() {
...
void BitcodeWriter19::writeStrtab() {
  assert(!WroteStrtab);
...
void BitcodeWriter19::copyStrtab(StringRef Strtab) {
  writeBlob(bitc::STRTAB_BLOCK_ID, bitc::STRTAB_BLOB, Strtab);
  WroteStrtab = true;
}

void BitcodeWriter19::writeModule(const Module &M,
                                  bool ShouldPreserveUseListOrder,
                                  const ModuleSummaryIndex *Index,
...
void BitcodeWriter19::writeIndex(
    const ModuleSummaryIndex *Index,
    const std::map<std::string, GVSummaryMapTy> *ModuleToSummariesForIndex,
...
static void WriteBitcodeToFile(const Module &M, raw_ostream &Out,
                               ...
                               const ModuleSummaryIndex *Index,
                               bool GenerateHash, ModuleHash *ModHash) {
  auto Write = [&](BitcodeWriter19 &Writer) {
  ...
  if (TT.isOSDarwin() || TT.isOSBinFormatMachO()) {
    ...
    BitcodeWriter19 Writer(Buffer);
    Write(Writer);
    emitDarwinBCHeaderAndTrailer(Buffer, TT);
    Out.write(Buffer.data(), Buffer.size());
  } else {
    BitcodeWriter19 Writer(Out);
    Write(Writer);
  }
}
...
void BitcodeWriter19::writeThinLinkBitcode(const Module &M,
                                           const ModuleSummaryIndex &Index,
                                           const ModuleHash &ModHash) {
  assert(!WroteStrtab);
...
void writeThinLinkBitcodeToFile19(const Module &M, raw_ostream &Out,
                                  const ModuleSummaryIndex &Index,
                                  const ModuleHash &ModHash) {
  SmallVector<char, 0> Buffer;
  Buffer.reserve(256 * 1024);
...
void embedBitcodeInModule19(llvm::Module &M, llvm::MemoryBufferRef Buf,
  ...
  if (EmbedBitcode) {
    if (Buf.getBufferSize() == 0 ||
      ...
      WriteBitcode19ToFile(M, OS, /* ShouldPreserveUseListOrder */ true,
