...
//===- Bitcode/Writer/ValueEnumerator19.h - Number values ---------*- C++
//-*-===//
//
...
public:
  ValueEnumerator19(const Module &M, bool ShouldPreserveUseListOrder);
  ValueEnumerator19(const ValueEnumerator19 &) = delete;
  ValueEnumerator19 &operator=(const ValueEnumerator19 &) = delete;
  ...
  unsigned getTypeID(Type *T) const {
    ...
    assert(I != TypeMap.end() && "Type not in ValueEnumerator19!");
    return I->second - 1;
  }
  ...
  unsigned getAttributeListID(AttributeList PAL) const {
    ...
    assert(I != AttributeListMap.end() &&
           "Attribute not in ValueEnumerator19!");
    return I->second;
  }
  ...
  /// use these two methods to get its data into the ValueEnumerator19!
