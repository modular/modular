  ...
  for (auto [i, smcInfos] : llvm::enumerate(symbolAndMCInfos)) {
    ...
    for (auto [j, mcInfo] : llvm::enumerate(smcInfos->mcInfos)) {
      ...
      for (auto &global : module->globals()) {
        // If the global was generated by LLVM's codegen pipeline with a private
        // linkage (e.g.,`PreISelIntrinsicLoweringPass` would insert private
        // constants @.memset_pattern), linker will rename the global if there's
        // a conflict. We rename and record the occurrence here so that the use
        // of it in MachineFunction will be updated accordingly later.
        if (!symbolLinkageTypes.contains(global.getName().str()) &&
          ...
          mcInfo->renamedPrivateSymbol.insert({originalName, uniqueName});
