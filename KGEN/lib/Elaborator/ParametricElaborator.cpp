...
evaluateParamForStub(PImplNode *parent, mlir::Location loc, TypedAttr iterator,
                     SymbolConstantAttr symbol, GeneratorOp gen, FuncOp func,
                     FuncType funcType) {
  TypedAttr input = paramForIteratorFuncInput(funcType, iterator);
  parent->getEvaluator().setErrorLoc(loc);
  if (gen) {
    ParametricIREvaluator nestedEvaluator(parent->getEvaluator());
    SmallVector<TypedAttr> paramValues;
    for (auto [decl, val] :
         llvm::zip(gen.getInputParams(), symbol.getParamValues())) {
      paramValues.push_back(parent->getEvaluator().getReboundAttribute(val));
      nestedEvaluator.overwriteDeclBinding(decl, paramValues.back());
    }
    nestedEvaluator.pushParamValues(paramValues, true);
    return nestedEvaluator.evaluateGenerator(gen, input);
  }
  return parent->getEvaluator().evaluateFunction(func, input);
}
...
ElaborationState ParametricElaborator::processParamForOp(PImplNode *parent,
  ...
  StringAttr getLowerBoundName =
      cast<FlatSymbolRefAttr>(getLowerBoundSymbol.getSymbol()).getAttr();
  auto getLowerBoundGen = oldSymTab.lookup<GeneratorOp>(getLowerBoundName);

  FuncOp getLowerBoundFunc;
  ...
  if (!getLowerBoundGen) {
    ...
    if (!*funcOr)
      return ElaborationState::skipNode();
    getLowerBoundFunc = *funcOr;
    getLowerBoundType = getLowerBoundFunc.getFuncTypeGenerator().getBody();
  } else {
    getLowerBoundType = getLowerBoundGen.getFuncTypeGenerator().getBody();
  }
  ...
  ErrorTreeOr<TypedAttr> lowerBoundResult = evaluateParamForStub(
      parent, op.getLoc(), iterator, getLowerBoundSymbol, getLowerBoundGen,
      getLowerBoundFunc, getLowerBoundType);
