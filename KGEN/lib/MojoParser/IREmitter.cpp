...
// target.
LIT::LoopOp IREmitter::emitForStmt(
    SMLoc forLoc, ExprNode *targetExpr, ExprNode *seqExpr,
    std::function<LogicalResult(LIT::LoopOp, VarDeclOp)> bodyFn,
    std::function<void()> errorFn) {
  if (!builder) {
    emitErrorForDynamicValueInParameter(forLoc, " for loop at compile time");
    return {};
  }
  ...
  VarDeclOp indvarDeclOp =
      emitVarDecl(target, UnresolvedType::get(getContext()), forLocation,
  ...
  ASTExprAnd<AnyValue> loadedSeq = {emitExpr(seqExpr, EC_ForIterator), seqExpr};
  if (!loadedSeq.ir)
    return {};

  // Get an iterator into the iterable by emitting a call to `__iter__`.
  VarDeclOp rangeRef = emitVarDecl("$RANGE", UnresolvedType::get(getContext()),
                                   forLocation, VarDeclKind::Synthesized);
  ValueDest rangeDest(rangeRef, EC_ForIterator);
  if (!emitNamedMethodCall("__iter__", {loadedSeq}, rangeDest,
  ...
  CValue hasNextBool = emitNamedMethodCall(
      "__has_next__", CallOperands({{MLValue(rangeRef), seqExpr}}), lengthDest,
      CallSyntax::kMethodCall, seqExpr);
  if (!hasNextBool)
    return {};
  CValue hasNext = emitI1({hasNextBool, seqExpr}, EC_ForIterator);
  if (!hasNext)
    return {};
  SRValue shouldContinue = emitSRValue({hasNext, seqExpr}, EC_ForIterator);
  if (!shouldContinue)
    return {};
  ...
  if (!emitNamedMethodCall("__next__",
                           CallOperands({{MLValue(rangeRef), seqExpr}}),
                           indvarDest, CallSyntax::kMethodCall, seqExpr))
    return {};

  avoidDroppingDeclOnFail.release();

  // Parse the body of the for loop, using the callback.
  if (failed(bodyFn(loopOp, indvarDeclOp)))
    return {};
