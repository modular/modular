...
kgen.generator.interface @erf_scalar<type: dtype>(%in: !pop.simd<1, type>) -> !pop.simd<1, type>

lit.func @erf_scalar_taylor<type: dtype>(%x: !pop.simd<1, type>) -> !pop.simd<1, type>
  ...
  %c0 = pop.constant(1.1283791670955125739) : !pop.simd<1, type>
  %c1 = pop.constant(-0.37612638903183752463) : !pop.simd<1, type>
  %x2 = pop.mul %x, %x : !pop.simd<1, type>
  %t0 = pop.fma %x2, %c1, %c0 : !pop.simd<1, type>
  %t1 = pop.mul %t0, %x : !pop.simd<1, type>
  kgen.return %t1 : !pop.simd<1, type>
}

// CHECK-LABEL: kgen.func @"erf_scalar_taylor,type=f32"(%{{.*}}: !pop.simd<1, f32>) -> !pop.simd<1, f32> {
// CHECK-LABEL: kgen.func @"erf_scalar_taylor,type=f64"(%{{.*}}: !pop.simd<1, f64>) -> !pop.simd<1, f64> {
...
// CHECK: kgen.call @"erf_scalar_taylor,type=f32"(%[[V0]]) : (!pop.simd<1, f32>) -> !pop.simd<1, f32>
kgen.generator @erf_scalar_f32(%arg0: f32) -> f32 {
  %0 = pop.cast_from_builtin %arg0 : f32 to !pop.simd<1, f32>
  %1 = kgen.call @erf_scalar<type: dtype = f32>(%0) : (!pop.simd<1, f32>) -> !pop.simd<1, f32>
...
// CHECK-SAME: %[[ARG0:.*]]: f64
// CHECK: %[[V0:.*]] = pop.cast_from_builtin %[[ARG0]]
// CHECK: kgen.call @"erf_scalar_taylor,type=f64"(%[[V0]]) : (!pop.simd<1, f64>) -> !pop.simd<1, f64>
kgen.generator @erf_scalar_f64(%arg0: f64) -> f64 {
  %0 = pop.cast_from_builtin %arg0 : f64 to !pop.simd<1, f64>
  %1 = kgen.call @erf_scalar<type: dtype = f64>(%0) : (!pop.simd<1, f64>) -> !pop.simd<1, f64>
...
lit.func @erf_impl1<type: dtype>(%in: !zap.buffer<?, type>, %out : !zap.buffer<?, type>)
  ...
  scf.for %i = %zero to %size step %one {
      %src  = zap.buffer.load %in[%i] : !zap.buffer<?, type>, !pop.simd<1, type>
      %res  = kgen.call @erf_scalar<type: dtype = type>(%src) : (!pop.simd<1, type>) -> !pop.simd<1, type>
