...
kgen.func public @while_accum_loop() -> f32 {
  ...
  %accum_init = pop.constant(0.0 :f32) : !meta.scalar<f32>
  // while (iter+8 < size) {
  //     int ii = iter;
  //     while (ii < iter + 8) {
  //         accum += ii;
  ...
  //     iter += 8;
  // }
  %new_index, %accum0 = scf.while (%iter = %iter_init, %accum = %accum_init) : (index, !meta.scalar<f32>) -> (index, !meta.scalar<f32>) {
    %cond = index.cmp sle(%iter, %size_minus_8)
    scf.condition(%cond) %iter, %accum : index, !meta.scalar<f32>
  } do {
  ^bb0(%iter1: index, %accum1: !meta.scalar<f32>):
    %ii_last = index.add %iter1, %eight
    //     while (ii < iter + 8) {
    //         accum += ii;
    //         ii++;
    //     }
    %new_index, %accum2 = scf.while (%ii = %iter1, %accum = %accum1) : (index, !meta.scalar<f32>) -> (index, !meta.scalar<f32>) {
      %cond = index.cmp slt(%ii, %ii_last)
      scf.condition(%cond) %ii, %accum : index, !meta.scalar<f32>
    } do {
    ...
    scf.yield %ii_last, %accum2 : index, !meta.scalar<f32>
  }
  // while (iter < size) {
  //     accum += iter;
  //     iter++;
  // }
  %iter1, %accum = scf.while (%iter = %new_index, %accum = %accum0) : (index, !meta.scalar<f32>) -> (index, !meta.scalar<f32>) {
