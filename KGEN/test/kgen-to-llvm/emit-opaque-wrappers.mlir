// RUN: kgen-opt %s -split-input-file -lower-kgen-to-llvm='c-call=kernel emit-opaque-wrappers' | FileCheck %s

// CHECK-LABEL: llvm.func @kernel(
// CHECK-SAME: attributes {opaque_wrapper = @kernel_opaque_wrapper}
kgen.func public @kernel(%a: !pop.struct<index, pointer<scalar<invalid>>, !kgen.dtype>, %i: i32) -> !pop.struct<index, pointer<scalar<invalid>>, !kgen.dtype> {
