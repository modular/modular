...
kgen.func @heap() -> !kgen.pointer<i16> {
  ...
  %0 = kgen.param.materialize: !kgen.pointer<i16> = <#M.memref<[(mem_heap, heap, [])], 0, 0>>
...
kgen.func @stack() {
  ...
  %0 = kgen.param.materialize: !kgen.pointer<i16> = <#M.memref<[(mem_stack, stack, [])], 0, 0>>
  kgen.return
}
...
kgen.func @global() -> !kgen.pointer<i8> {
  ...
  // CHECK: %[[RESULT_TYPED:.*]] = llvm.bitcast %[[RESULT]]
  %0 = kgen.param.materialize: !kgen.pointer<i8> = <#M.memref<[(mem_global, const_global, [])], 0, 2>>
...
kgen.func @pointer_to_pointer() {
  ...
  %0 = kgen.param.materialize: !kgen.pointer<pointer<i16>> = <#M.memref<[
    (foo, stack, [(0, 1, 0)]),
    (bar, heap, [])
  ], 0, 0>>
  kgen.return
...
kgen.func @string() {
  ...
  %0 = kgen.param.materialize: !kgen.pointer<i8> = <#M.memref<[(mem_string, const_global, [])], 0, 0>>
  // COM: Ensure `const_global` handle gets deduplicated.
  // CHECK: llvm.mlir.addressof @mem_string :
  %1 = kgen.param.materialize: !kgen.pointer<i8> = <#M.memref<[(mem_string, const_global, []), (mem_stack, stack, [])], 0, 0>>
  kgen.return
}
