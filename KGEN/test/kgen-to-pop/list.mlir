...
// CHECK-SAME: %arg0: !pop.struct<i32, struct<index, index>, i32>
// CHECK-SAME: -> !pop.struct<index, index>
kgen.func @list_in_struct(%s: !pop.struct<i32, !kgen.list<index[2]>, i32>) -> !kgen.list<index[2]> {
  ...
  // CHECK-NEXT: replace %{{.*}}, %{{.*}}[0] : !pop.struct<i32, struct<index, index>, i32>
  %4 = pop.struct.replace %1, %s[0] : !pop.struct<i32, !kgen.list<index[2]>, i32>
  // CHECK-NEXT: replace %{{.*}}, %{{.*}}[1] : !pop.struct<i32, struct<index, index>, i32>
  // CHECK-NEXT: replace %{{.*}}, %{{.*}}[2] : !pop.struct<i32, struct<index, index>, i32>
  ...
  // CHECK-NEXT: "hold"(%{{.*}}) : (!pop.struct<i32, struct<index, index>, i32>)
  "hold"(%6) : (!pop.struct<i32, !kgen.list<index[2]>, i32>) -> ()

  // CHECK-NEXT: return %1 : !pop.struct<index, index>
  kgen.return %2 : !kgen.list<index[2]>
}

// CHECK-LABEL: @empty_list
// CHECK-SAME: %arg0: !pop.struct<struct<>, i32>
// CHECK-NOT: %arg1
kgen.func @empty_list(%s: !pop.struct<!kgen.list<index[0]>, i32>) -> !kgen.list<index[0]> {
  // CHECK-NEXT: pop.struct.extract %{{.*}}[0] : !pop.struct<struct<>, i32>
  %1 = pop.struct.extract %s[0] : !pop.struct<!kgen.list<index[0]>, i32>
  // CHECK-NEXT: pop.struct.extract %{{.*}}[1] : !pop.struct<struct<>, i32>
  %2 = pop.struct.extract %s[1] : !pop.struct<!kgen.list<index[0]>, i32>
  // CHECK-NEXT: "hold"
  "hold"(%2) : (i32) -> ()
  // CHECK: return %0 : !pop.struct<>
  kgen.return %1 : !kgen.list<index[0]>
}

// CHECK-LABEL: @nested_list_in_struct
// CHECK-SAME: %arg0: !pop.struct<struct<struct<index, index>, struct<index, index>>>
// CHECK-SAME: %arg1: !pop.struct<struct<index, index>, struct<index, index>>
// CHECK-SAME: -> !pop.struct<struct<index, index>, struct<index, index>>
kgen.func @nested_list_in_struct(%s: !pop.struct<!kgen.list<!kgen.list<index[2]>[2]>>, %list: !kgen.list<!kgen.list<index[2]>[2]>) -> !kgen.list<!kgen.list<index[2]>[2]> {
  ...
  // CHECK-NEXT: %{{.*}} = "foo"(%arg1) : (!pop.struct<struct<index, index>, struct<index, index>>) -> !pop.struct<struct<index, index>, struct<index, index>>
  %1 = "foo"(%list) : (!kgen.list<!kgen.list<index[2]>[2]>) -> !kgen.list<!kgen.list<index[2]>[2]>

  // CHECK-NEXT: pop.struct.extract %arg0[0] : !pop.struct<struct<struct<index, index>, struct<index, index>>>
...
// CHECK-SAME: %arg0: !pop.struct<struct<pointer<struct<index, index>>, pointer<struct<index, index>>>>
...
// CHECK-SAME: %arg0: !pop.pointer<struct<struct<index, index>>>
...
// CHECK-SAME: %arg0: !pop.pointer<struct<struct<>>>
...
// CHECK-SAME: %arg0: !pop.struct<struct<index>>
// CHECK-SAME: -> (!pop.struct<struct<index>>, i1)
...
// CHECK-SAME: %arg0: !pop.struct<struct<index>>
// CHECK-SAME: -> !pop.struct<struct<index>>
kgen.func @hlcf_scf_loops(%list: !kgen.list<!kgen.list<index[1]>[1]>, %cond: i1, %ub: index) -> !kgen.list<!kgen.list<index[1]>[1]> {
  ...
  // CHECK: %{{.*}} = hlcf.loop (%{{.*}} = %arg0 : !pop.struct<struct<index>>) -> !pop.struct<struct<index>>
  %2 = hlcf.loop (%a = %list : !kgen.list<!kgen.list<index[1]>[1]>) -> !kgen.list<!kgen.list<index[1]>[1]> {
    ...
    hlcf.if %4 {
      // CHECK: hlcf.break %{{.*}} : !pop.struct<struct<index>>
      hlcf.break %3 : !kgen.list<!kgen.list<index[1]>[1]>
    } else {
      hlcf.yield
    }
    // CHECK: kgen.return %{{.*}} : !pop.struct<struct<index>>
...
kgen.func @list_create_op(%arg0: index, %arg1: index) -> !kgen.list<index[2]> {
  ...
  // CHECK: return %0 : !pop.struct<index, index>
  kgen.return %list : !kgen.list<index[2]>
}

// CHECK-LABEL: @list_in_coroutine
// CHECK-SAME: !pop.coroutine<() -> !pop.struct<>>
...
kgen.func @list_attr_in_attr() {
  // CHECK-NEXT: struct<struct<index, index>, struct<i32, i32, i32>> = <{ { 1, 2 }, { 5, 6, 7 } }>
  %0 = kgen.param.constant: struct<!kgen.list<index[2]>, !kgen.list<i32[3]>>
    = <{ [1, 2], [5, 6, 7] }>
  // CHECK-NEXT: struct<struct<struct<index, index>, struct<index, index>>> = <{ { { 1, 2 }, { 3, 4 } } }>
  %1 = kgen.param.constant: struct<!kgen.list<!kgen.list<index[2]>[2]>> = <{ [[1, 2], [3, 4]] }>
  // CHECK-NEXT: variant<struct<>, array<2, struct<i32, i32>>> = <#pop.variant<:struct<> {  }>>
...
kgen.func @list_in_fn_type() -> (() -> !kgen.list<i1[0]>) {
  // CHECK-NEXT: %0 = kgen.addressof @return_none : () -> !pop.struct<>
  %0 = kgen.addressof @return_none : () -> !kgen.list<i1[0]>
  // CHECK-NEXT: return %0 : () -> !pop.struct<>
