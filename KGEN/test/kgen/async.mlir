...
kgen.func @slow_function(%arg0: index) -> !pop.coroutine<() -> index> {
  ...
  %ctx = pop.pointer.bitcast %ctxPtr : !pop.pointer<struct<index>> to !pop.pointer<struct<pointer<scalar<invalid>>, pointer<scalar<invalid>>>>
  %ctxOpaque = pop.pointer.bitcast %ctx : !pop.pointer<struct<pointer<scalar<invalid>>, pointer<scalar<invalid>>>> to !pop.pointer<i8>
...
kgen.func @async_coroutine(%arg0: index) -> !pop.coroutine<() -> index> {
  ...
  %calleeCtx = pop.pointer.bitcast %calleeCtxPtr : !pop.pointer<struct<index>> to !pop.pointer<struct<pointer<scalar<invalid>>, pointer<scalar<invalid>>>>
  %calleeOpaqueCtx = pop.pointer.bitcast %calleeCtxPtr : !pop.pointer<struct<index>> to !pop.pointer<i8>

  %runtimePtr = pop.struct.gep %ctx[1] : <struct<pointer<scalar<invalid>>, pointer<scalar<invalid>>>>
  %calleeRuntimePtr = pop.struct.gep %calleeCtx[1] : <struct<pointer<scalar<invalid>>, pointer<scalar<invalid>>>>
  %runtime = pop.load %runtimePtr : !pop.pointer<pointer<scalar<invalid>>>
  pop.store %runtime, %calleeRuntimePtr : !pop.pointer<pointer<scalar<invalid>>>
  pop.external_call @KGEN_CompilerRT_LLCL_InitializeContext(%calleeOpaqueCtx) : (!pop.pointer<i8>) -> ()
...
kgen.func @call_it() -> index {
  ...
  %runtime = pop.external_call @KGEN_CompilerRT_LLCL_CreateRuntime(%nThreads) : (i8) -> !pop.pointer<scalar<invalid>>
  ...
  %ctxPtr = pop.pointer.bitcast %ctxPtrRaw : !pop.pointer<struct<index>> to !pop.pointer<struct<pointer<scalar<invalid>>, !pop.pointer<scalar<invalid>>>>
  %runtimePtr = pop.struct.gep %ctxPtr[1] : <struct<pointer<scalar<invalid>>, pointer<scalar<invalid>>>>
  pop.store %runtime, %runtimePtr : !pop.pointer<pointer<scalar<invalid>>>

  %ctxPtrVoid = pop.pointer.bitcast %ctxPtr : !pop.pointer<struct<pointer<scalar<invalid>>, pointer<scalar<invalid>>>> to !pop.pointer<i8>
  ...
  pop.external_call @KGEN_CompilerRT_LLCL_ExecuteAndWait(%coroResume, %hdl, %ctxPtr)
    : ((!llvm.ptr<i8>) -> (), !pop.coroutine<() -> index>, !pop.pointer<struct<pointer<scalar<invalid>>, pointer<scalar<invalid>>>>) -> ()
  ...
  pop.external_call @KGEN_CompilerRT_LLCL_DestroyRuntime(%runtime) : (!pop.pointer<scalar<invalid>>) -> ()
