...
kgen.generator export @top() {
  // expected-error @below {{custom op 'kgen.param.constant' the immediate emission kind must be either '=llvm', '=asm', '=llvm-opt', '=sharedobj', or '=elabmlir'}}
...
kgen.generator export @top() {
  // expected-error @below {{custom op 'kgen.param.constant' 'compile_assembly' second operand should evaluate to either 'asm', 'llvm', 'llvm-opt', 'sharedobj', or 'elabmlir'}}
  kgen.param.constant: string = <compile_assembly(current_target(), 5, "", 0, :() -> () @kernel)>
  kgen.return
}
