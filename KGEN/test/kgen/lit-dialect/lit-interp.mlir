...
lit.file_module @module {
  lit.fn @store_load(%a: index) -> !kgen.struct<(index, index)> {
...
lit.fn @ger_load(%a: !lit.struct<@Int>) -> index {
...
// CHECK-LABEL: lit.fn @interpret_ger
lit.fn @interpret_ger() {
  // CHECK-NEXT: constant = <42>
  kgen.param.constant = <apply(:!lit.generator<("a": !lit.struct<@Int>) -> index> @ger_load, { 42 })>
  kgen.return
}

lit.fn @load_undef_var() -> index {
  %x = lit.var.decl "x" var : !lit.ref<index, mut lt>
  %0 = lit.ref.load %x : <index, mut lt>
  kgen.return %0 : index
}

// CHECK-LABEL: lit.fn @interpret_undef_var
lit.fn @interpret_undef_var() {
...
// CHECK-LABEL: lit.fn @interpret_undef_ger_load
lit.fn @interpret_undef_ger_load() {
  // CHECK-NEXT: constant = <#interp.uninitmem>
  kgen.param.constant = <apply(:!lit.generator<() -> index> @load_undef_ger)>
  kgen.return
}

lit.fn @double_ger_store_load(%a: index) -> !lit.struct<@Int> {
...
lit.fn @initialize_pair(%a: index, %b: index) -> !lit.struct<@Pair> {
  %x = lit.var.decl "x" var : !lit.ref<@Pair, mut lt>
  %0 = lit.ref.struct.ger %x[first] : <@Pair, mut lt> -> @Int
  %1 = lit.ref.struct.ger %0[value] : <@Int, mut lt->first> -> index
  lit.ref.store %a, %1 : <index, mut lt->first->value>
...
// CHECK-LABEL: lit.fn @interpret_ger_store
lit.fn @interpret_ger_store() {
...
lit.fn @partial_ger_store(%i: index) -> !lit.struct<@Pair> {
...
// CHECK-LABEL: lit.fn @interpret_partial_ger_store
lit.fn @interpret_partial_ger_store() {
