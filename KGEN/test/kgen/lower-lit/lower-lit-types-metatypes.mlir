...
kgen.generator @func<T: trait<@Trait>>(%arg0: !lit.declref<@Container<:trait<@Trait> T>>) {
  kgen.return
}
...
kgen.generator @top(%arg0: !lit.declref<@Container<:trait<@Trait> [@Element, {"f": () -> () = @f}]>>) {
  // CHECK-NEXT: call @func<:type [struct<() memoryOnly>, {{{.*}}}]>(%arg0) : (!kgen.struct<(struct<() memoryOnly>) memoryOnly>) -> ()
  kgen.call @func<:trait<@Trait> [@Element, {"f": () -> () = @f}]>(%arg0) : (!lit.declref<@Container<:trait<@Trait> [@Element, {"f": () -> () = @f}]>>) -> ()
  kgen.return
}
