...
// CHECK-LABEL: @floor
kgen.func @floor() -> (!pop.simd<2, f32>, !pop.simd<2, si8>,
  ...
  %4 = pop.floor %0 : !pop.simd<2, f32>
  %5 = pop.floor %1 : !pop.simd<2, si8>
  %6 = pop.floor %2 : !pop.scalar<index>
  %7 = pop.floor %3 : !pop.scalar<uindex>
  kgen.return %4, %5, %6, %7 : !pop.simd<2, f32>, !pop.simd<2, si8>,
                               !pop.scalar<index>, !pop.scalar<uindex>
}

// CHECK-LABEL: @ceil
kgen.func @ceil() -> (!pop.simd<2, f32>, !pop.simd<2, si8>,
                       !pop.scalar<index>, !pop.scalar<uindex>) {
  // CHECK-DAG: <"2", "-1">
  // CHECK-DAG: <3, -4>
  // CHECK-DAG: <7>
  ...
  %2 = kgen.param.constant: scalar<index> = <7>
  %3 = kgen.param.constant: scalar<uindex> = <8>
  %4 = pop.ceil %0 : !pop.simd<2, f32>
  %5 = pop.ceil %1 : !pop.simd<2, si8>
  %6 = pop.ceil %2 : !pop.scalar<index>
  %7 = pop.ceil %3 : !pop.scalar<uindex>
  kgen.return %4, %5, %6, %7 : !pop.simd<2, f32>, !pop.simd<2, si8>,
                               !pop.scalar<index>, !pop.scalar<uindex>
}
...
kgen.func @cast_bool() -> (!pop.scalar<f16>, !pop.scalar<ui8>, !pop.scalar<index>) {
  %c = kgen.param.constant: scalar<bool> = <true>
  // CHECK-DAG: %[[C0:.*]] = kgen{{.*}}<"1">
  %0 = pop.cast %c : !pop.scalar<bool> to !pop.scalar<f16>
  // CHECK-DAG: %[[C1:.*]] = kgen{{.*}}ui8{{.*}}<1>
  %1 = pop.cast %c : !pop.scalar<bool> to !pop.scalar<ui8>
  // CHECK-DAG: %[[C2:.*]] = kgen{{.*}}index{{.*}}<1>
  %2 = pop.cast %c : !pop.scalar<bool> to !pop.scalar<index>
  // CHECK-NEXT: return %[[C0]], %[[C1]], %[[C2]]
  kgen.return %0, %1, %2 : !pop.scalar<f16>, !pop.scalar<ui8>, !pop.scalar<index>
}

// CHECK-LABEL: @cast_bf16
kgen.func @cast_bf16() -> (!pop.scalar<f16>, !pop.scalar<ui8>, !pop.scalar<bool>, !pop.scalar<index>) {
  %c = kgen.param.constant: scalar<bf16> = <"10.125">
  // CHECK-DAG: %[[C0:.*]] = kgen{{.*}}<"10.125">
  %0 = pop.cast %c : !pop.scalar<bf16> to !pop.scalar<f16>
  // CHECK-DAG: %[[C1:.*]] = kgen{{.*}}ui8{{.*}}<10>
  %1 = pop.cast %c : !pop.scalar<bf16> to !pop.scalar<ui8>
  // CHECK-DAG: %[[C2:.*]] = kgen{{.*}}<true>
  %2 = pop.cast %c : !pop.scalar<bf16> to !pop.scalar<bool>
  // CHECK-DAG: %[[C3:.*]] = kgen{{.*}}index{{.*}}<10>
  %3 = pop.cast %c : !pop.scalar<bf16> to !pop.scalar<index>
...
kgen.func @cast_si128() -> (!pop.scalar<f16>, !pop.scalar<ui8>, !pop.scalar<bool>, !pop.scalar<index>) {
  %c = kgen.param.constant: scalar<si128> = <18446744073709551615>
  // CHECK-DAG: %[[C0:.*]] = kgen{{.*}}<"+Inf">
  %0 = pop.cast %c : !pop.scalar<si128> to !pop.scalar<f16>
  // CHECK-DAG: %[[C1:.*]] = kgen{{.*}}ui8{{.*}}<255>
  %1 = pop.cast %c : !pop.scalar<si128> to !pop.scalar<ui8>
  // CHECK-DAG: %[[C2:.*]] = kgen{{.*}}<true>
  %2 = pop.cast %c : !pop.scalar<si128> to !pop.scalar<bool>
  // CHECK-DAG: %[[C3:.*]] = kgen{{.*}}index{{.*}}<-1>
  %3 = pop.cast %c : !pop.scalar<si128> to !pop.scalar<index>
  // CHECK-NEXT: return %[[C0]], %[[C1]], %[[C2]], %[[C3]]
  kgen.return %0, %1, %2, %3 : !pop.scalar<f16>, !pop.scalar<ui8>, !pop.scalar<bool>, !pop.scalar<index>
}
...
module attributes {M.target_info = #M.target<triple="", arch="", features="", data_layout="e-p:32:32">} {
  ...
  kgen.func @shl_index() -> (!pop.scalar<index>, !pop.scalar<index>) {
  ...
  kgen.func @uindex_div() -> !pop.scalar<uindex> {
