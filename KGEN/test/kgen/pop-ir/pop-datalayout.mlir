...
kgen.generator @pop_sizeof_alignof<N, T:type, DT:dtype>() {
  ...
  // CHECK-NEXT: <0>
  kgen.param.constant: index = <get_sizeof(!kgen.pack<[]>, #target)>
  // CHECK-NEXT: <24>
  kgen.param.constant: index = <get_sizeof(!kgen.pack<[i8, i32, i64, i32]>, #target)>
  // CHECK-NEXT: <1>
  kgen.param.constant: index = <get_alignof(!kgen.pack<[]>, #target)>
  // CHECK-NEXT: <4>
  kgen.param.constant: index = <get_alignof(!kgen.pack<[i8, i32, i16]>, #target)>
  // CHECK-NEXT: <16>
  ...
  // CHECK-NEXT: <16>
  kgen.param.constant: index = <get_sizeof(variadic<i32>, #target)>
  // CHECK-NEXT: <8>
  kgen.param.constant: index = <get_alignof(variadic<i32>, #target)>
