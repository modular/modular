// RUN: kgen-opt -automatic-inline="update-debug-info=immediate optimization-level=0" -mlir-print-debuginfo -split-input-file %s | FileCheck %s
...
kgen.func @callee_inner() -> index always_inline {
  %idx0 = index.constant 0 loc(#locCalleeInner)
  kgen.return %idx0 : index loc(#locCalleeInner)
} loc(#locCalleeInner)

kgen.func @callee_nodebug() -> index always_inline_no_debug {
  %idx0 = index.constant 1
  kgen.call @callee_inner() : () -> index
  kgen.return %idx0 : index
}

kgen.func @callee_debug() -> index always_inline {
