...
kgen.func export @known_true_or_false(%arg0: i32, %arg1: f32, %arg2: i8) -> i1 {
  %0 = pop.variant.create %arg0, 1 : <i8, i32>
  // CHECK: %[[TRUE:.*]] = kgen.param.constant: i1 = <1>
  %1 = pop.variant.is %0, 1 : <i8, i32>
  // CHECK: hlcf.if %[[TRUE]]
  %2 = hlcf.if %1 -> !pop.variant<f32, i8> {
    %3 = pop.variant.create %arg2, 1 : <f32, i8>
    hlcf.yield %3 : !pop.variant<f32, i8>
  } else {
    %3 = pop.variant.create %arg1, 0 : <f32, i8>
    hlcf.yield %3 : !pop.variant<f32, i8>
  }
  // CHECK: %[[FALSE:.*]] = kgen.param.constant: i1 = <0>
  %5 = pop.variant.is %2, 0 : <f32, i8>
  // CHECK: return %[[FALSE]]
  kgen.return %5 : i1
}
...
kgen.func export @known_false(%arg0: i32, %arg1: i8, %arg2: i1) -> i1 {
  %0 = hlcf.if %arg2 -> !pop.variant<i8, i32, f32> {
    %1 = pop.variant.create %arg0, 1 : <i8, i32, f32>
    hlcf.yield %1 : !pop.variant<i8, i32, f32>
  } else {
    %1 = pop.variant.create %arg1, 0 : <i8, i32, f32>
    hlcf.yield %1 : !pop.variant<i8, i32, f32>
  }
  // CHECK: %[[FALSE:.*]] = kgen.param.constant: i1 = <0>
  %2 = pop.variant.is %0, 2 : <i8, i32, f32>
  // CHECK: return %[[FALSE]]
  kgen.return %2 : i1
}
...
kgen.func @always_index() -> !pop.variant<i8, index> {
  %0 = index.constant 5
  %1 = pop.variant.create %0, 1 : <i8, index>
  // CHECK: return %{{.*}} : index
...
kgen.func @entry() -> !pop.variant<i8, i32> {
  ...
  %1 = pop.variant.is %0, 0 : <i8, index>
  // CHECK: %[[IF_RESULT:.*]] = hlcf.if
  %2 = hlcf.if %1 -> !pop.variant<i8, i32> {
    %3 = pop.variant.get %0, 0 : <i8, index>
    %4 = pop.variant.create %3, 0 : <i8, i32>
    hlcf.yield %4 : !pop.variant<i8, i32>
  } else {
    %3 = kgen.param.constant: i32 = <0>
    %5 = pop.variant.create %3, 1 : <i8, i32>
    hlcf.yield %5 : !pop.variant<i8, i32>
  }
  // CHECK: %[[RETURN:.*]] = pop.variant.get %[[IF_RESULT]], 1 : <i8, i32>
  // CHECK: return %[[RETURN]]
...
kgen.func @do_not_rewrite() -> !pop.variant<index> {
  %0 = index.constant 0
  %1 = pop.variant.create %0, 0 : <index>
...
kgen.func @multiple_returns(%a: i32, %b: i64) -> !pop.variant<i32, i64> {
  %0 = pop.variant.create %a, 0 : <i32, i64>
  %1 = hlcf.loop (%arg0 = %0 : !pop.variant<i32, i64>) -> !pop.variant<i32, i64> {
    %2 = pop.variant.is %arg0, 0 : <i32, i64>
    // CHECK: hlcf.if
    hlcf.if %2 {
      // CHECK-NEXT: %[[RES:.*]] = pop.variant.get %arg2, 0 : <i32, i64>
    ...
    // CHECK: %[[I64:.*]] = pop.variant.create %arg1, 1 : <i32, i64>
    // CHECK-NEXT: %[[I32:.*]] = pop.variant.get %[[I64]], 0 : <i32, i64>
    // CHECK-NEXT: kgen.return %[[I32]]
    %3 = pop.variant.create %b, 1 : <i32, i64>
    kgen.return %3 : !pop.variant<i32, i64>
  }
  // CHECK: %[[RES:.*]] = pop.variant.get %1, 0 : <i32, i64>
  // CHECK-NEXT: kgen.return %[[RES]]
