...
kgen.generator @no_constrains_deduplication() {
  ...
  kgen.param.if <to_builtin(:scalar<bool> eq(cond, 1))> {
    ...
    kgen.param.assert <to_builtin(:scalar<bool> eq(2, 3))>, B0
    kgen.return
  } else {
    kgen.param.declare B1 : !kgen.string = <"bar">
    // CHECK: kgen.param.assert <0>, "bar"
    kgen.param.assert <to_builtin(:scalar<bool> eq(2, 3))>, B1
    kgen.param.yield
  }
  kgen.param.declare B2 : !kgen.string = <"baz">
  // CHECK: kgen.param.assert <0>, "baz"
  kgen.param.assert <to_builtin(:scalar<bool> eq(2, 3))>, B2
  kgen.return
}
