...
> : !debuginfo.subroutine<(!pop.pointer<@HasMemFields>) -> (!lit.none): DW_CC_normal>
...
lit.struct.decl @HasMemFields attributes {destructor = #kgen.symbol.constant<@HasMemFields::@__del__> : !kgen.signature<(!pop.pointer<@HasMemFields> owned_in_mem) -> !lit.none>} {
  ...
  lit.func @__del__(%self: !pop.pointer<@HasMemFields> loc(#loc) owned_in_mem) -> !lit.none {
    // CHECK-DAG: %[[VAR0:.*]] = lit.struct.gep %self[a] : <@S> from <@HasMemFields> loc(#[[LOC]])
    // CHECK-DAG: %[[VAR1:.*]] = kgen.call @S::@__del__(%[[VAR0]]) : (!pop.pointer<@S> owned_in_mem) -> !lit.none loc(#[[LOC]])
    lit.ownership.mark.destroyed %self : !pop.pointer<@HasMemFields> loc(#loc1)
