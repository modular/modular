...
// CHECK-SAME: -> !pop.variant<@Error, !kgen.list<i0[0]>>
lit.func @return_raise_or(%cond: i1, %err: !kgen.declref<@Error>) -> !lit.raises_or<!lit.none> {
  hlcf.if %cond {
    // CHECK: %[[ERR:.*]] = pop.variant.create %arg1 : !kgen.declref<@Error> -> !pop.variant<@Error, !kgen.list<i0[0]>>
  ...
  // CHECK: %[[VAL:.*]] = pop.variant.create %{{.*}} : !kgen.list<i0[0]> -> !pop.variant<@Error, !kgen.list<i0[0]>>
...
lit.func @unwrap_or_propagate(%cond: i1, %err: !kgen.declref<@Error>) -> !lit.raises_or<index> {
  ...
  // CHECK-NEXT: %[[ISVAL:.*]] = pop.variant.is !kgen.list<i0[0]>, %[[WRAPPED]]
  // CHECK-NEXT: %[[VAL:.*]] = hlcf.if %[[ISVAL]]
  // CHECK-NEXT:   %[[UNWRAP:.*]] = pop.variant.get %[[WRAPPED]] : !pop.variant<@Error, !kgen.list<i0[0]>> as !kgen.list<i0[0]>
  // CHECK-NEXT:   hlcf.yield %[[UNWRAP]]
  // CHECK-NEXT: } else {
  // CHECK-NEXT:   %[[UNWRAP:.*]] = pop.variant.get %[[WRAPPED]] : !pop.variant<@Error, !kgen.list<i0[0]>> as !kgen.declref<@Error>
