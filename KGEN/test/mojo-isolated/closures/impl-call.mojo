...
# CHECK-NEXT: kgen.param.declare *"x`1x2": lifetime<1> = <(mutcast imm *"self`")>
# CHECK-NEXT: %[[V0REF:.*]] = kgen.rebind %[[V0]]
# CHECK-NEXT: %[[V1:.*]] = lit.ref.struct.ger %[[SELF]][field1]
# CHECK-NEXT: kgen.param.declare *"w`1x1": lifetime<1> = <(mutcast imm *"self`")>
...
# CHECK-NEXT: kgen.param.declare *"m`1x0": lifetime<0> = <*"self`">
# CHECK-NEXT: %[[W0CAST:.*]] = kgen.rebind %[[W0]] : !lit.ref<!MemType, imm *"self`"> to !lit.ref<!MemType, imm *"m`1x0">
...
# CHECK-NEXT: %[[W0:.*]] = lit.ref.struct.ger %[[SELF]][field0]
# CHECK-NEXT: kgen.param.declare *"m`1x0"
