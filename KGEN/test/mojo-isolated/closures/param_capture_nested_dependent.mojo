...
# CHECK-NEXT: lit.call {{.*}}@"`_CI_{{.*}}"::@"__init__{{.*}}<:!Int B, :!Int A, :origin<0> [[SELFO]]>(%self, %anonymous2A)
...
# CHECK-NEXT: lit.call @{{.*}}::@"fn{{.*}}"::@"__init__{{.*}}<:!Int B, :!Int A, :origin<0> [[SELFO]]>(%anonymous2A, %bar)
