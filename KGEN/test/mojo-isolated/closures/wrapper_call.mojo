...
# CHECK: lit.struct.field call : !kgen.signature<!lit.signature<[1](!kgen.pointer<none>, |, "n": !lit.ref<!MemType, imm {{.*}}> read_mem, "j": !Int) -> !Int>>
# CHECK: lit.func @"__call__{{.*}}(%[[SELF:.*]][{{.*}}]: !lit.ref<{{.*}}> read_mem, |, %n: !lit.ref<!MemType, imm {{.*}}> read_mem, %j: !Int) -> !Int
...
# CHECK: lit.struct.field call : !kgen.signature<!lit.signature<[2](!kgen.pointer<none>, |, "n": !lit.ref<!MemType, imm {{.*}}> read_mem, ?, "__result__": !lit.ref<!MemType, mut *[0,1]> byref_result) -> !kgen.none>>
# CHECK: lit.func @"__call__{{.*}}(%[[SELF:.*]][{{.*}}]: !lit.ref<{{.*}}> read_mem, |, %n: !lit.ref<!MemType, imm {{.*}}> read_mem, ?, %[[RES:.*]]: !lit.ref<!MemType, mut {{.*}}> byref_result)
