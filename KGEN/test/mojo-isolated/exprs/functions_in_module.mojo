...
# CHECK-DAG: [[TYPE3:#.*]] = #kgen.type<{{.*}}@__MLIRType<:type !lit.generator<("y": !Int, "z": !Int) -> !kgen.none>>{{.*}} : !AnyType
# CHECK-DAG: [[TYPE4:#.*]] = #kgen.type<{{.*}}@__MLIRType<:type !lit.generator<[2](?, "__error__": !lit.ref<!Error, mut *[0,0]> byref_error, "__result__": !lit.ref<none, mut *[0,1]> byref_result) throws -> i1>>{{.*}} : !AnyType
# CHECK-DAG: [[TYPE5:#.*]] = #kgen.type<{{.*}}@__MLIRType<:type !lit.generator<<"func_type": !AnyType, +, "func": !kgen.param<:!AnyType *(0,0)>>() -> !kgen.none>>{{.*}} : !AnyType
