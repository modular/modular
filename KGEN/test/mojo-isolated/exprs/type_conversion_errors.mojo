...
struct WrapsMadeFromPack[*Ts: AnyType]:
    ...
    @implicit
    fn __init__(out self, *args: *Ts):
        # expected-error @+1 {{cannot implicitly convert 'VariadicPack[False, __origin_of(args), AnyType, Ts]' value to 'MadeFromPack[Ts]'}}
        self.data = args
...
fn test_rp_trivial_inference(a: RP_NotTrivial, b: Foo):
    # expected-error @below {{invalid call to 'infer_rp_trivial': could not deduce parameter 'T', argument type 'RP_NotTrivial' is not a '@register_passable("trivial")' type, so does not satisfy AnyTrivialRegType}}
    _ = infer_rp_trivial(a)

    # expected-error @below {{invalid call to 'infer_rp_trivial': could not deduce parameter 'T', argument type 'Foo' is not a '@register_passable("trivial")' type, so does not satisfy AnyTrivialRegType}}
