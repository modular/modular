...
fn test_unbound_pack():
    # CHECK: lit.alias.decl *"all_unbound`1x0": metatype<#StructWithDefault <?, ?, ?, ?>, <"a": index, "b": index, "c": index = 1, "d": index = 2>>
    alias all_unbound = StructWithDefault[*_]

    # CHECK: lit.alias.decl *"first_bound`1x1": metatype<#StructWithDefault <5, ?, ?, ?>, <"b": index, "c": index = 1, "d": index = 2>>
    alias first_bound = StructWithDefault[`5`, *_]

    # CHECK: lit.alias.decl *"last_bound`1x2": metatype<#StructWithDefault <?, ?, ?, 6>, <"a": index, "b": index, "c": index = 1>>
    alias last_bound = StructWithDefault[*_, `6`]

    # CHECK: lit.alias.decl *"mid_unbound`1x3": metatype<#StructWithDefault <3, ?, ?, 4>, <"b": index, "c": index = 1>>
    alias mid_unbound = StructWithDefault[`3`, *_, `4`]

    # CHECK: lit.alias.decl *"last_bound_without_kw`1x4": metatype<#StructWithDefaultKwOnly <?, ?, 7, 2>, <"a": index, "b": index>>
    alias last_bound_without_kw = StructWithDefaultKwOnly[*_, `7`]

    # CHECK: lit.alias.decl *"last_bound_with_kw`1x5": metatype<#StructWithDefaultKwOnly <?, ?, 8, ?>, <"a": index, "b": index, *, "d": index = 2>>
