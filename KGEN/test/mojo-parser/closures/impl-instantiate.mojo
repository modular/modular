...
fn makes_escaping_closure(m: MemType, w: Int):
    # CHECK: %[[IMPL:.*]] = lit.varlet.decl "anonymous*" var synth anon : !lit.ref<mut !escaping2,
    ...
    # CHECK: %[[IMPL:.*]] = lit.varlet.decl "anonymous*" var synth anon : !lit.ref<mut !escaping,
    # CHECK-NEXT: [[IMPL_PTR:%.*]] = lit.ref.to_pointer %[[IMPL]]
    # CHECK-NEXT: [[A:%.*]] = lit.ref.load %a
    # CHECK-NEXT: kgen.call @{{.*}}::@"__init__{{.*}}"([[IMPL_PTR]], [[A]], %w)
    # CHECK-NEXT: %[[WRAPPER:.*]] = lit.varlet.decl "anonymous*" var synth anon :
