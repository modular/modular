...
# CHECK-NEXT: lit.alias.decl *"[[XREF:.*]]": @"${{.*}}"::@Foo<[[A:.*]]: !Int = p0> =
# CHECK-SAME: <apply(:!lit.signature<("b": !Int borrow, |) ownedresult -> !kgen.declref<@"${{.*}}"::@Foo<[[A]]: !Int = p0>>> @"${{.*}}"::@Foo::@"__init__{{.*}}"<:!Int p0>, #lit.struct<{value = 1}>)>
# CHECK-NEXT: kgen.param.constant: !Int = <#lit.struct.extract<:@"${{.*}}"::@Foo<[[A]]: !Int = p0> *"[[XREF]]", "b">>
fn parameter_capture[a: Int](c: Int) -> fn (x: Int) escaping -> Int:
    alias X = Foo[a](1)
