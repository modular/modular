...
# S1-DAG: lit.alias.decl *"dtype{{.*}}": !DType = <apply(:!lit.generator<("n": !Int) -> !DType> @{{.*}}::@"nonsense(::SIMD[::DType(int), ::SIMDSize(1)])`{{.*}}", {:scalar<index> 64})>
...
# S7-DAG: lit.struct.decl @"def[dtype: DType, //, simd_width: Int]() thin -> SIMD[dtype, SIMDSize(simd_width)]_PtrWrapper"
# S7-DAG: lit.alias.decl dtype: !DType = <__capture_dtype>
# S7-DAG: lit.fn @"__call__[::DType,::SIMD[::DType(int), ::SIMDSize(1)]](unified_closure_promotion::def[dtype: DType, //, simd_width: Int]() thin -> SIMD[dtype, SIMDSize(simd_width)]_PtrWrapper[$0, $1])"
...
# S7-LABEL: lit.fn @"s7_trigger[::SIMD[::DType(int), ::SIMDSize(1)],::DType]()"
# S7: %[[S7_WRAP:.*]] = lit.var.decl "__call_result_tmp__" synth : !lit.ref<!lit.struct
# S7-SAME: @{{.*}}::@"compute_init2[::SIMD[::DType(int), ::SIMDSize(1)]](){{.*}}"<:!DType *(0,0), :!Int *(0,1)>
# S7: %[[S7_INIT:.*]] = lit.call @{{.*}}::@"def[dtype: DType, //, simd_width: Int]() thin -> SIMD[dtype, SIMDSize(simd_width)]_PtrWrapper"::@"__init__()"{{.*}}(%[[S7_WRAP]])
