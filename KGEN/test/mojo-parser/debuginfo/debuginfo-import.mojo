...
# CHECK-DAG: #[[CALLED_STRUCT_BOUND:.*]] = #debuginfo.source_name<(struct)"CalledStruct{{.*}}_param"
...
# CHECK-DAG: #debuginfo.subprogram<compileUnit = #{{.*}}, scope = #file, name = #test_name, linkageName = "test(${{.*}}::CalledStruct[{{.*}}param])"
