...
fn take_closure_with_param_main():
    ...
    let value = Bound
    # CHECK: %1 = kgen.rebind %value : !kgen.signature<("y": index borrow) capturing -> index> to !kgen.signature<(index borrow) capturing -> index>
...
# CHECK-SAME: (%a: !kgen.pointer<!StructWithInit> owned_in_mem,
# CHECK-SAME:  %b: !kgen.pointer<!StructWithInit> borrow_in_mem)
...
# CHECK-SAME: (%arg0: !kgen.pointer<{{.*}}> borrow_in_mem)
...
fn referencesDefaultArgumentFunction():
    # CHECK: %f = lit.letreg.decl "f" = %0 : !kgen.signature<("a": !Int borrow, "b": !Int borrow = #lit.struct<{value = 3}>, "c": !Int borrow = #lit.struct<{value = 5}>)
...
struct StructWithInit:
    ...
    # CHECK-SAME: (%self: !kgen.pointer<!StructWithInit> init_self,
...
# CHECK-SAME: (%self: !kgen.pointer<!ValueMem> init_self,
# CHECK-SAME:  %existing: !kgen.pointer<!ValueMem> borrow_in_mem)
# CHECK-NEXT: %0 = lit.struct.gep %self[a]
...
# CHECK-SAME: (%self: !kgen.pointer<!ValueMem> init_self,
# CHECK-SAME:  %existing: !kgen.pointer<!ValueMem> owned_in_mem)
# CHECK-NEXT: %0 = lit.struct.gep %self[a]
# CHECK-NEXT: %1 = lit.struct.gep %existing[a]
...
# CHECK: lit.func @"__init__
# CHECK-SAME: (%a: !Int borrow,
# CHECK-SAME:  %b: !StructExample) ownedresult
# CHECK-SAME:    -> !ValueReg
...
struct SomeParamStruct[c_param: Int]:
    ...
    fn topLevelParamFn[a_param: Int](self):
        ...
        # CHECK: lit.alias.decl {{.*}}reff: <!Int>() -> !lit.none = <*"nestedFunction[{{.*}}$int::Int]()">
        alias reff = nestedFunction
        # CHECK: call_param[{{.*}}: bind_signature(:<!Int>() -> !lit.none *"nestedFunction[{{.*}}$int::Int]()", {{.*}}2{{.*}})]()
        nestedFunction[2]()
