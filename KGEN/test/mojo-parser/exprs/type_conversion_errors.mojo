...
fn test_rp_trivial_inference(a: RP_NotTrivial, b: Foo):
    # expected-error @below {{invalid call to 'infer_rp_trivial': value passed to 'val' cannot be converted from 'RP_NotTrivial' to 'T', argument type 'RP_NotTrivial' does not conform to trait 'TrivialRegisterType'}}
    _ = infer_rp_trivial(a)

    # expected-error @below {{invalid call to 'infer_rp_trivial': value passed to 'val' cannot be converted from 'Foo' to 'T', argument type 'Foo' does not conform to trait 'TrivialRegisterType'}}
    _ = infer_rp_trivial(b)


# expected-note @below {{function declared here}}
fn infer_rp_trivial[T: TrivialRegisterType](val: T):
    pass
