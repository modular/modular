...
def test_assert_with_param_expr[x: Int, y: Int]():
    # CHECK: kgen.param.assert <{{.*}}eq(:scalar<index> from_builtin(#lit.struct.extract<:!Int x, "_mlir_value">), from_builtin(#lit.struct.extract<:!Int y, "_mlir_value">)))>, ""
    comptime assert x == y
...
def test_assert_enables_where_constraint[x: Int](y: Int):
    ...
    # CHECK: kgen.param.assert <{{.*}}ge(:scalar<index> from_builtin(#lit.struct.extract<:!Int x, "_mlir_value">), 0))>, ""
    comptime assert x >= 0
