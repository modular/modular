...
struct MiniPack[element_trait: type_of(AnyType), //, *element_types: element_trait]:
    ...
    # CHECK: kgen.param.assert <to_builtin(:scalar<bool> sugar_preserved({{.*}}conforms_to(:!kgen.param<{{.*}}element_trait> #kgen.param_list.get<{{.*}}element_types.values{{.*}}, #lit.struct.extract<:!Int i, "_mlir_value">>{{.*}}>
