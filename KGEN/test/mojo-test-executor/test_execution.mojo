...
# RUN: not mojo-test-executor --pretty %t %t-entry  | FileCheck %s --check-prefix=CHECK-UNKNOWN

# RUN: echo '[{"id": "%s::test_fn_assert_failure()"}]' > %t
# RUN: mojo-test-executor --pretty %t %t-entry | FileCheck %s --check-prefix=CHECK-UNIT1

# RUN: echo '[{"id": "%s::test_def_assert_failure()"}]' > %t
# RUN: mojo-test-executor --pretty %t %t-entry | FileCheck %s --check-prefix=CHECK-UNIT2

# RUN: echo '[{"id": "%s@doc_test_failure_first_cell().__doc__::0",' > %t
# RUN: echo ' "location": {"endColumn": 1, "endLine": 86,' >> %t
# RUN: echo ' "startColumn": 1, "startLine": 83}},' >> %t
# RUN: echo '{"id": "%s@doc_test_failure_first_cell().__doc__::1",' >> %t
# RUN: echo ' "location": {"endColumn": 1, "endLine": 90,' >> %t
# RUN: echo ' "startColumn": 1, "startLine": 88}}]' >> %t
# RUN: mojo-test-executor --pretty %t %t-entry | FileCheck %s --check-prefix=CHECK-DOC-FIRST-CELL

# RUN: echo '[{"id": "%s@doc_test_failure_second_cell().__doc__::0",' > %t
# RUN: echo ' "location": {"endColumn": 1, "endLine": 110,' >> %t
# RUN: echo ' "startColumn": 1, "startLine": 108}},' >> %t
# RUN: echo '{"id": "%s@doc_test_failure_second_cell().__doc__::1",' >> %t
# RUN: echo ' "location": {"endColumn": 1, "endLine": 115,' >> %t
# RUN: echo ' "startColumn": 1, "startLine": 112}}]' >> %t
# RUN: mojo-test-executor --pretty %t %t-entry | FileCheck %s --check-prefix=CHECK-DOC-SECOND-CELL
