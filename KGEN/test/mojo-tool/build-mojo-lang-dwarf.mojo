...
# RUN: mojo debug %t -o 'image lookup -vs _build_mojo_lang_dwarf_foo_Odlp8ej4rAD' -b | FileCheck %s --check-prefix CHECK-MOJO
# CHECK-MOJO: language = "mojo"


# COM: Using the default language C
# RUN: mojo build --debug-level full -O0 %s -o %t
# RUN: mojo debug %t -o 'image lookup -vs _build_mojo_lang_dwarf_foo_Odlp8ej4rAD' -b | FileCheck %s --check-prefix CHECK-C
