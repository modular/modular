...
kgen.generator @pop_sizeof_alignof<N, T:type, DT:dtype>() {
  ...
  // CHECK-NEXT: <get_sizeof(!pop.array<N, i8>, #kgen.target<triple={{.*}}>)>
  ...
  // CHECK-NEXT: <get_alignof(!pop.array<N, i8>, #kgen.target<triple={{.*}}>)>
  ...
  // CHECK-NEXT: <get_sizeof(!pop.pointer<T>, #kgen.target<triple={{.*}}>)>
  %8 = kgen.param.constant = <get_sizeof(!pop.pointer<T>, #kgen.target<host>)>
  // CHECK-NEXT: <get_alignof(!pop.pointer<T>, #kgen.target<triple={{.*}}>)>
  ...
  // CHECK-NEXT: <get_sizeof(!pop.scalar<DT>, #kgen.target<triple={{.*}}>)>
  %12 = kgen.param.constant = <get_sizeof(!pop.scalar<DT>, #kgen.target<host>)>
  // CHECK-NEXT: <get_alignof(!pop.scalar<DT>, #kgen.target<triple={{.*}}>)>
  ...
  // CHECK-NEXT: <get_sizeof(!pop.simd<N, f32>, #kgen.target<triple={{.*}}>)>
  %16 = kgen.param.constant = <get_sizeof(!pop.simd<N, f32>, #kgen.target<host>)>
  // CHECK-NEXT: <get_alignof(!pop.simd<4, DT>, #kgen.target<triple={{.*}}>)>
