...
module attributes {M.target_info = #M.target<triple="", cpu="", features="", data_layout="", simd_bit_width=128>} {
  ...
  kgen.func @global_load() -> !pop.scalar<f32> {
    ...
    // CHECK-NEXT: %2 = builtin.unrealized_conversion_cast %1 : !llvm.ptr<f32, 3> to !pop.pointer<scalar<f32>, 3>
    ...
    %1 = pop.load %0 :!pop.pointer<scalar<f32>, 3>
...
module attributes {M.target_info = #M.target<triple="", cpu="", features="", data_layout="", simd_bit_width=128>} {
  ...
  kgen.func @global_store(%arg0: !pop.scalar<f32>) {
    ...
    // CHECK-NEXT: %3 = builtin.unrealized_conversion_cast %2 : !llvm.ptr<f32, 3> to !pop.pointer<scalar<f32>, 3>
    // CHECK-NEXT: %4 = llvm.addrspacecast %2 : !llvm.ptr<f32, 3> to !llvm.ptr<f32>
    // CHECK-NEXT: llvm.store %0, %4 {alignment = 4 : i64} : !llvm.ptr<f32>
    %0 = pop.global_alloc 2 x !pop.scalar<f32> align 4 address_space 3
    pop.store %arg0, %0 :!pop.pointer<scalar<f32>, 3>
    kgen.return
  }
}
