...
kgen.func @coroutine(%arg0: i32) -> !pop.coroutine<() -> i32> {
  ...
  // CHECK-NEXT: %[[PROMISE_MEM:.*]] = llvm.alloca %[[C1_i32]] x !llvm.struct<(struct<(ptr, ptr)>, i32)> {alignment = 8 : i64}
  // CHECK-NEXT: %[[PROMISE:.*]] = llvm.bitcast %[[PROMISE_MEM]] : !llvm.ptr<struct<(struct<(ptr, ptr)>, i32)>> to !llvm.ptr<i8>
...
kgen.func @other_coroutine_ops(%a: !pop.coroutine<() -> i32>) -> !pop.pointer<struct<i32>> {
  ...
  // CHECK: %[[PROMISE:.*]] = llvm.getelementptr inbounds %[[PROMISE_PTR]][16] : (!llvm.ptr<i8>) -> !llvm.ptr<i32>
  ...
  // CHECK: llvm.return %[[PROMISE]] : !llvm.ptr<i32>
