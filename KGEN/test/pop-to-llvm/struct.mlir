...
!struct1 = !pop.struct<!pop.struct<!meta.scalar<f32>>, !pop.array<4, !meta.scalar<f32>>>

// CHECK-LABEL: @struct_construct
kgen.func @struct_construct(
    %a: !pop.struct<!meta.scalar<f32>>,
    %b: !pop.array<4, !meta.scalar<f32>>
) -> !struct1 {
...
kgen.func @struct_insert(
    %a: !pop.struct<!meta.scalar<f32>>,
    %b: !meta.scalar<f32>
) -> !pop.struct<!meta.scalar<f32>> {
  // CHECK: llvm.insertvalue %{{.*}}, %{{.*}}[0] : !llvm.struct<(f32)>
  %0 = pop.replace_element %b, %a[0] : !pop.struct<!meta.scalar<f32>>
  kgen.return %0 : !pop.struct<!meta.scalar<f32>>
}

// CHECK-LABEL: @struct_extract
kgen.func @struct_extract(%a: !pop.struct<!meta.scalar<f32>>) -> !meta.scalar<f32> {
  // CHECK: llvm.extractvalue %{{.*}}[0]
  %0 = pop.get_element %a[0] : !pop.struct<!meta.scalar<f32>>
