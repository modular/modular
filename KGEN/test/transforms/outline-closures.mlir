...
// CHECK-LABEL: @call_region<fn: <A -> E>() always_inline -> index -> E>() always_inline -> index
kgen.generator @call_region<fn: <A -> E>() always_inline ->index -> E>() always_inline -> index {
  // CHECK-NEXT: kgen.param.declare BoundFn: <() -> E>() always_inline -> index = <bind_signature(:<A -> E>() always_inline -> index fn, 2)>
  kgen.param.declare BoundFn: <() -> E>() always_inline -> index = <bind_signature(:<A -> E>() always_inline -> index fn, 2)>
  // CHECK-NEXT: %0 = kgen.call_param[<() -> E>() always_inline -> index: BoundFn]<() -> Result>()
  %0 = kgen.call_param[<() -> E>() always_inline -> index: BoundFn]<() -> Result>()
...
kgen.generator @raiseClosure<Jefffffffffff -> index>(%arg0: !pop.scalar<index>) -> (index, index) {
  ...
  kgen.param.declare.region Fn = <A, B -> E>() always_inline -> index {
  ...
  %0 = kgen.call @call_region<fn: <A -> E>() always_inline ->index = BoundFn -> Result = E>() : () always_inline -> index
...
// CHECK-NEXT:    kgen.call @raise2Closures_Empty() : () always_inline -> ()
// CHECK-NEXT:    kgen.return

// CHECK-LABEL: kgen.generator @raise2Closures_Fn<C, A -> E>(%arg0: index) always_inline -> index
...
kgen.generator @raise2Closures() {
  ...
  // CHECK: kgen.param.declare Empty: <>() always_inline -> () = <@raise2Closures_Empty_wrapper>
  kgen.param.declare.region Empty = () always_inline -> () {
    kgen.return
  }

  // CHECK-NEXT: kgen.param.declare Fn: <A -> E>() always_inline -> index = <@raise2Closures_Fn_wrapper<C = C, A = #kgen.unbound>>
  kgen.param.declare.region Fn = <A -> E>() always_inline -> index {
  ...
  // CHECK: kgen.call @call_region<fn: <A -> E>() always_inline -> index = Fn -> Result = E>() : () always_inline -> index
  %0 = kgen.call @call_region<fn: <A -> E>() always_inline ->index = Fn -> Result = E>() : () always_inline -> index
  %1 = kgen.param.constant = <Result>
  // CHECK: kgen.return
  kgen.return
}

// CHECK-LABEL: kgen.generator @parametrizedClosure_Fn<T: type>(%arg0: !kgen.paramref<T>) always_inline -> !kgen.paramref<T>
...
// CHECK-NEXT:    kgen.param.declare Fn: <>() always_inline -> !kgen.paramref<T> = <@parametrizedClosure_Fn_wrapper<T: type = T>>
// CHECK-NEXT:    %1 = kgen.call_param[<>() always_inline -> !kgen.paramref<T>: Fn]()
...
kgen.generator @parametrizedClosure<T: type>(%arg0: !kgen.paramref<T>) -> !kgen.paramref<T> {
  kgen.param.declare.region Fn = <>() always_inline -> !kgen.paramref<T> {
...
kgen.generator @useAfterDef() -> index {
  ...
  // CHECK: kgen.call @call_region<fn: <A -> E>() always_inline -> index = Fn -> Result = E>() : () always_inline -> index
  %call = kgen.call @call_region<fn: <A -> E>() always_inline ->index = Fn -> Result = E>() : () always_inline -> index
  // CHECK-NEXT: kgen.param.constant
  %constant = kgen.param.constant = <Result>

  // CHECK-NEXT: kgen.param.declare Fn: <A -> E>() always_inline -> index = <@useAfterDef_Fn_wrapper<C = C, A = #kgen.unbound>>
  kgen.param.declare.region Fn = <A -> E>() always_inline -> index {
    %0 = kgen.param.constant = <add(A, C)>
    %1 = pop.cast_from_builtin %0 : index to !pop.scalar<index>
    %2 = pop.cast_from_builtin %cst : index to !pop.scalar<index>
...
kgen.generator @nested(%pred: i1) -> index {
  ...
  %if = scf.if %pred -> index {
    ...
    // CHECK-NEXT: kgen.call @call_region<fn: <A -> E>() always_inline -> index = Fn -> Result = E>() : () always_inline -> index
    %call = kgen.call @call_region<fn: <A -> E>() always_inline ->index = Fn -> Result = E>() : () always_inline -> index
  ...
  // CHECK: kgen.param.declare Empty: <>() always_inline -> () = <@nested_Empty_wrapper>
  kgen.param.declare.region Empty = () always_inline -> () {
    kgen.return
  }
...
kgen.generator @capture_crosses_parameter_domain<T: type>(%arg0: !kgen.paramref<T>) {
  ...
  kgen.param.declare.region Fn = <A, T: type>() always_inline -> !kgen.paramref<T> {
    kgen.return %arg0: !kgen.paramref<T>
  }
  kgen.return
}
...
kgen.generator @parametrizedSSACapture<T: type>(%arg0 : !kgen.paramref<T>) -> index {
  %0 = kgen.call_param[<>() always_inline -> index: fn]()
  // CHECK: kgen.param.declare fn: <>() always_inline -> index = <@parametrizedSSACapture_fn_wrapper<T: type = T>>
  kgen.param.declare.region fn = () always_inline -> index {
...
kgen.generator @dontBindInputParameters<T: type, I>(%arg0 : !kgen.paramref<T>) -> index {
  %0 = kgen.call_param[<>() always_inline -> index: bind_signature(: <I>() always_inline -> index fn, I)]()
  // CHECK: kgen.param.declare fn: <I>() always_inline -> index = <@dontBindInputParameters_fn_wrapper<T: type = T, I = #kgen.unbound>>
  kgen.param.declare.region fn = <I>() always_inline -> index {
