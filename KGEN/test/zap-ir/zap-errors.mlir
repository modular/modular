...
kgen.func @simd_load(%buff: !zap.buffer<4, si32>) {
  ...
  // expected-error @below {{'zap.buffer.load' op the type ('!zap.buffer<4, si32>') must have the same element type as the simd type ('!pop.simd<4, f32>')}}
  %0 = zap.buffer.load %buff[%idx]: !zap.buffer<4, si32>, !pop.simd<4, f32>
}
...
kgen.func @simd_store(%val : !pop.simd<4, f32>, %buff: !zap.buffer<4, si32>) {
  %idx = index.constant 0
  // expected-error @below {{'zap.buffer.store' op the type ('!zap.buffer<4, si32>') must have the same element type as the simd type ('!pop.simd<4, f32>')}}
  zap.buffer.store %val, %buff[%idx]: !pop.simd<4, f32>, !zap.buffer<4, si32>
}
