...
def test_parametric_alias_with_complex_arithmetic_line_breaking():
    ...
    source = (
        "alias ComplexArithmeticAlias[a: Int, b: Int, c: Int]: Int = "
        "(a * b) + (b * c) + (c * a)"
    )
    expected = (
        "alias ComplexArithmeticAlias[a: Int, b: Int, c: Int]: Int = (a * b) + (\n"
        "    b * c\n"
        ") + (c * a)\n"
    )
    assert_mojo_format(source, expected)
...
def test_parametric_alias_with_conditional_line_breaking():
    ...
    source = (
        "alias ConditionalParametricAlias[condition: Bool, value: Int]: Int = "
        "value if condition else 0"
    )
    expected = (
        "alias ConditionalParametricAlias[\n"
...
def test_parametric_alias_with_complex_nested_expressions():
    ...
    source = (
        "alias ComplexAlias[dt: DType, size: Int, offset: Int = 0] = "
        "SIMD[dt, size + offset]"
    )

    expected = (
        "alias ComplexAlias[dt: DType, size: Int, offset: Int = 0] = SIMD[\n"
        "    dt, size + offset\n"
        "]\n"
    )
    assert_mojo_format(source, expected)
...
def test_parametric_alias_spaces_in_multiline_expression():
    ...
    source = (
        "alias MultilineSpacing[x: Int, y: Int] : Int = ("
        "    x * y\n"
        "    ) + (\n"
        "    x + y\n"
        ")\n"
...
def test_parametric_alias_spaces_in_comments():
    ...
    source = (
        "alias CommentSpacing[x: Int] : Int = x + 1  # Add one\n"
