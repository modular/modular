...
# Do all imports with leading underscores. This ensures that they don't clash
# with user imports.
...
# traffic, we manually load the backend module and register it with matplotlib.
# `backend_str` below is provided by the module that imports this file, and
# contains the source code for the backend module.
...
def display(fig, metadata):
    if fig.canvas is None:
        _FigureCanvasBase(fig)

    # Encode the figure as a PNG so that we can send it to the kernel process.
    ...
    encoded_png = str(_b2a_base64(byte_stream.getvalue()).decode("ascii"))[:-1]
    ...
    # The kernel process needs to be able to extract our display data from the
    # output stream. We use a special delimiter to mark the start and end of
    # our display data. We also make sure to flush the output stream so that
    # the kernel process can read the data immediately. `display_start` and
    # `display_end` are provided by the script that imports this one so we are
    # never out of sync.
