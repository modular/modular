...
const nonWindowsMultipleCalls = async (options: any = {}) => {
  ...
  return Object.entries(returnValue)
      .filter(([, value ]: any) => value.comm && value.args && value.ppid &&
                                   value.uid && value['%cpu'] && value['%mem'])
      .map(([ key, value ]: any) => ({
...
const nonWindowsCall = async (options: any = {}) => {
  ...
  const processes = psLines
                        .map(line => {
                          ...
                          const {pid, ppid, uid, cpu, memory, comm}: any =
                              match.groups;
...
const nonWindows = async (options = {}) => {
  ...
  } catch { // If the error is not a parsing error, it should manifest itself in
...
export function psList(context: vscode.ExtensionContext,
                       options?: Options): Promise<ProcessDescriptor[]> {
