...
public:
  LLVMThreadPool(MLRT::Runtime &runtime)
...
private:
  ...
  struct TurnStile {
    TurnStile(MLRT::Runtime &runtime)
        : counter(1), chain(MLRT::AsyncValueRef<Chain>::allocate(runtime)) {}

    bool taskComplete();
    void waitAndReset(MLRT::Runtime &runtime);
