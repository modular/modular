...
// Adapted by Modular.
// SPDX-FileCopyrightText: Copyright (c) 2013-2020, Cameron Desrochers.
...
struct thread_id_size<4> {
  using numeric_t = std::uint32_t;
};
template <>
struct thread_id_size<8> {
  using numeric_t = std::uint64_t;
};
...
swap(typename ConcurrentQueue<T, Traits>::ImplicitProducerKVP &a,
     typename ConcurrentQueue<T, Traits>::ImplicitProducerKVP &b) noexcept;
...
public:
  ...
  inline void swap(ConcurrentQueue &other) noexcept { swap_internal(other); }
  ...
  struct ExplicitProducer : public ProducerBase {
    ...
    inline bool enqueue(U &&element) {
      ...
      if ((currentTailIndex & static_cast<index_t>(BLOCK_SIZE - 1)) == 0) {
        ...
        if constexpr (!noexcept(new (static_cast<T *>(nullptr))
        ...
        if constexpr (noexcept(new (static_cast<T *>(nullptr))
          ...
          while (currentTailIndex != stopIndex) {
            ...
            new ((*this->tailBlock)[currentTailIndex]) T(
                Details::nomove_if<!noexcept(new (static_cast<T *>(nullptr)) T(
                    Details::deref_noexcept(itemFirst)))>::eval(*itemFirst));
            ++currentTailIndex;
            ++itemFirst;
          }
        }
      ...
      if (Details::circular_less_than<size_t>(0, desiredCount)) {
        ...
        if (Details::circular_less_than<size_t>(0, actualCount)) {
          ...
          do {
            ...
            if (noexcept(Details::deref_noexcept(itemFirst) =
...
private:
  ...
  struct ImplicitProducerKVP {
    ...
    operator=(ImplicitProducerKVP &&other) noexcept {
      swap(other);
      return *this;
    }
  ...
  friend void MoodyCamel::swap(
      ...
      typename ConcurrentQueue<XT, XTraits>::ImplicitProducerKVP &) noexcept;
      ...
      do {
        ...
        ++index;
          ...
          Details::invalid_thread_id()); // Can happen if the hash has changed
                                         ...
                                         // or if we weren't added to this hash
                                         // in the first place
    }
...
inline void swap(ConcurrentQueue<T, Traits> &a,
                 ConcurrentQueue<T, Traits> &b) noexcept {
  a.swap(b);
}
