...
<!-- The `my_pow()` declaration below is repeated from "Optional arguments"
     on purpose, so this example reads on its own. The test code defines it
     once, in the `optional-args` region of tests.mojo, which is why this
     block is tagged `duplicate-of` instead of being matched line for line.
