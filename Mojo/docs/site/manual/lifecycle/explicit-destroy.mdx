...
```mojo
@fieldwise_init
struct Example(Deinitable where False else "call 'cleanup()'"):
...
@fieldwise_init
struct Transaction(
    Deinitable where False else "call 'commit()' or 'rollback()'"
):
...
struct CustomFileHandle(
    Deinitable where False else "call 'save_and_close()' or 'discard()'"
):
