...
Explicit destruction opts a type out of the `Deinitable` trait. Mark types
with a `not Deinitable` entry in the conformance list:

```mojo
@fieldwise_init
struct Example(not Deinitable else "call 'cleanup()'"):
...
@fieldwise_init
struct Transaction(not Deinitable else "call 'commit()' or 'rollback()'"):
...
# Order matters
struct MutexGuard(not Deinitable):
...
Add a custom message to the opt-out to improve compiler diagnostics:

```mojo
struct CustomFileHandle(
    not Deinitable else "call 'save_and_close()' or 'discard()'"
):
...
struct Tally(
    Movable, not Deinitable else "call 'destroy()' to free the counters"
):
