---
title: '@always_inline'
description: Copies the body of a function directly into the body of the calling function (in new code, use @inline instead).
codeTitle: true
---
...
`@always_inline` is equivalent to `@inline(.always)`, and
`@always_inline("nodebug")` to `@inline(.nodebug)`. The `"builtin"` version
described below has no `@inline` equivalent.
...
recommended. Use the standard [`@inline`](/docs/reference/decorators/inline/)
decorator instead.
