...
- Added a [Closures](/docs/manual/functions/closures/) page to the Mojo Manual
...
- The [`ImplicitlyCopyable`](/docs/std/traits/copyable/ImplicitlyCopyable/),
  ...
  [`ImplicitlyDestructible`](/docs/std/traits/deinitable/Deinitable/).
...
- Added the [`UntrackedOrigin`](/docs/std/origin/#untrackedorigin) and
  [`UnsafeAnyOrigin`](/docs/std/origin/#unsafeanyorigin) origin aliases (and
  their `Mut`/`Immut` variants) as the new names for `ExternalOrigin` and
  [`AnyOrigin`](/docs/std/origin/#anyorigin), respectively. `UntrackedOrigin` is
  the empty origin: it aliases nothing, so the lifetime checker has nothing to
  track, and it remains a supported tool for interfacing with memory from
  outside the Mojo program. `UnsafeAnyOrigin` is the universal origin: it might
  alias anything, defeating lifetime extension and exclusivity checking, so its
  `Unsafe` prefix marks it as an escape hatch slated for deprecation and
  removal.
