...
def test_read_only_tree_auto_redirects_without_config(
    ...
    # namespaced by the environment id and module name.
    env_id = mojo.importer._redirected_cache_env_id()
    expected_cache_dir = cache_root / "python_extensions" / env_id / name
    compiled = list(expected_cache_dir.glob(f"{name}.*.so"))
    assert compiled, f"expected a compiled .so under {expected_cache_dir}"
