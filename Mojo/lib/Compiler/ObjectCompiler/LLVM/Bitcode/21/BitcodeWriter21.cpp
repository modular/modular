...
static cl::opt<unsigned>
    IndexThreshold("bitcode21-mdindex-threshold", cl::Hidden, cl::init(25),
                   cl::desc("Number of metadatas above which we emit an index "
                            "to enable lazy-loading"));
static cl::opt<uint32_t> FlushThreshold(
    "bitcode21-flush-threshold", cl::Hidden, cl::init(512),
    cl::desc("The threshold (unit M) for flushing LLVM bitcode."));

static cl::opt<bool> WriteRelBFToSummary(
    "bitcode21-write-relbf-to-summary", cl::Hidden, cl::init(false),
...
public:
  ...
  ModuleBitcodeWriterBase(const Module &M, StringTableBuilder &StrtabBuilder,
    ...
    // (which would have been assigned an ID by the ValueEnumerator21).
    // The starting ValueId is just after the number of values in the
    // ValueEnumerator21, so that they can be emitted in the VST.
...
void ModuleBitcodeWriter::writeAttributeGroupTable() {
  const std::vector<M::KGEN::ValueEnumerator21::IndexAndAttrSet> &AttrGrps =
  ...
  for (M::KGEN::ValueEnumerator21::IndexAndAttrSet Pair : AttrGrps) {
...
void ModuleBitcodeWriter::writeTypeTable() {
  const M::KGEN::ValueEnumerator21::TypeList &TypeList = VE.getTypes();
...
void ModuleBitcodeWriter::writeConstants(unsigned FirstVal, unsigned LastVal,
  ...
  const M::KGEN::ValueEnumerator21::ValueList &Vals = VE.getValues();
  Type *LastTy = nullptr;
...
BitcodeWriter21::BitcodeWriter21(SmallVectorImpl<char> &Buffer)
    : Stream(new BitstreamWriter(Buffer)) {
  writeBitcodeHeader(*Stream);
}

BitcodeWriter21::BitcodeWriter21(raw_ostream &FS)
    : Stream(new BitstreamWriter(FS, FlushThreshold)) {
  writeBitcodeHeader(*Stream);
}

BitcodeWriter21::~BitcodeWriter21() { assert(WroteStrtab); }

void BitcodeWriter21::writeBlob(unsigned Block, unsigned Record,
                                StringRef Blob) {
...
void BitcodeWriter21::writeSymtab() {
...
void BitcodeWriter21::writeStrtab() {
  assert(!WroteStrtab);
...
void BitcodeWriter21::copyStrtab(StringRef Strtab) {
  writeBlob(bitc::STRTAB_BLOCK_ID, bitc::STRTAB_BLOB, Strtab);
  WroteStrtab = true;
}

void BitcodeWriter21::writeModule(const Module &M,
                                  bool ShouldPreserveUseListOrder,
                                  const ModuleSummaryIndex *Index,
...
void BitcodeWriter21::writeIndex(
    const ModuleSummaryIndex *Index,
    const ModuleToSummariesForIndexTy *ModuleToSummariesForIndex,
...
WriteBitcodeToFile(const llvm::Module &M, llvm::raw_ostream &Out,
                   bool ShouldPreserveUseListOrder = false,
                   const llvm::ModuleSummaryIndex *Index = nullptr,
                   bool GenerateHash = false,
                   llvm::ModuleHash *ModHash = nullptr) {
  auto Write = [&](BitcodeWriter21 &Writer) {
  ...
  if (TT.isOSDarwin() || TT.isOSBinFormatMachO()) {
    ...
    BitcodeWriter21 Writer(Buffer);
    Write(Writer);
    emitDarwinBCHeaderAndTrailer(Buffer, TT);
    Out.write(Buffer.data(), Buffer.size());
  } else {
    BitcodeWriter21 Writer(Out);
    Write(Writer);
  }
}
...
void BitcodeWriter21::writeThinLinkBitcode(const Module &M,
                                           const ModuleSummaryIndex &Index,
                                           const ModuleHash &ModHash) {
  assert(!WroteStrtab);
...
void embedBitcodeInModule21(llvm::Module &M, llvm::MemoryBufferRef Buf,
