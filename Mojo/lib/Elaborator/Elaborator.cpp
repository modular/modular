...
ElaborationState Elaborator::processComptimeIfOp(ImplNode *parent,
  ...
  item.onComplete = [resultBool, debug = config.elaborateDebugInfo](
    ...
    if (auto yieldOp = dyn_cast<ComptimeYieldOp>(terminator)) {
...
// of a kgen.comptime.for. If the loop is pointless, we can inline it into the
...
ElaborationState Elaborator::processComptimeForOp(ImplNode *parent,
  ...
  // kgen.comptime.for.break/continue across loop iterations and to the result
  // of the kgen.comptime.for.  These are created by mem2reg promoting stack
  ...
  // Lower the `kgen.comptime.for` into an outer loop and wrapper loops for each
  ...
  finalItem.onComplete = [op, parent,
    ...
    // Remove the original kgen.comptime.for now that it has been lowered.
    ...
    // Each iteration of the kgen.comptime.for will be turned into a nested
  ...
  // `kgen.comptime.for` terminators with the appropriate HLCF ones.
  auto makeCompletion =
    ...
    return [debug, &region, outerLabel](ImplNode *node) -> LogicalResult {
      ...
      // Replace the `kgen.comptime.for` terminators with the HLCF equivalent.
      region.walk([&](Operation *op) {
        if (isa<ComptimeForOp>(op))
          return WalkResult::skip();
        if (isa<ComptimeForBreakOp>(op)) {
          IRRewriter b{OpBuilder(op)};
          b.replaceOpWithNewOp<HLCF::BreakOp>(op, op->getOperands(),
                                              outerLabel);
          return WalkResult::advance();
        }
        if (isa<ComptimeForContinueOp>(op)) {
          IRRewriter b{OpBuilder(op)};
          b.replaceOpWithNewOp<HLCF::BreakOp>(op, op->getOperands(),
...
ElaborationState Elaborator::processOp(ImplNode *node, Operation *op) {
  ...
  if (auto forOp = dyn_cast<ComptimeForOp>(op))
