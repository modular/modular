...
kgen.func @loop_args() {
  ...
  "hlcf.loop"() ({
  ^bb0(%arg0: i32):
    hlcf.return // expected-note {{to beginning of region #0 here}}
  }) : () -> ()
  hlcf.return
}
...
kgen.func @return_mismatch_result_count(%arg0: i32) {
  hlcf.loop {
    // expected-error @below {{'hlcf.return' op expected 0 operands, but given 1}}
...
kgen.func @return_mismatch_result_count(%arg0: i32) -> i64 {
  hlcf.loop {
    // expected-error @below {{'hlcf.return' op operand #0 has type 'i32' but expected 'i64'}}
    hlcf.return %arg0 : i32
  }
}
