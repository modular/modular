...
kgen.func @elif(%arg0: index, %arg1: index, %arg2: index) {
  ...
  // CHECK-NEXT:      [[VAR0:%.*]] = hlcf.if [[VAR1B]] -> index {
...
kgen.func @comptime_match(%arg0: !kgen.scalar<bool>, %arg1: i32, %arg2: i64) {
  // CHECK-NEXT: hlcf.comptime.match <:scalar<bool> true> {
  hlcf.comptime.match <:!kgen.scalar<bool> true> {
    // CHECK-NEXT: hlcf.if %arg0 {
    hlcf.if %arg0 {
      // CHECK-NEXT: "test.use"() : () -> ()
      "test.use"() : () -> ()
  ...
  // CHECK: %[[V:.*]] = hlcf.comptime.match <0> -> i32 {
  %0 = hlcf.comptime.match <0> -> i32 {
    // CHECK-NEXT: hlcf.match.complete %arg1 : i32
    hlcf.match.complete %arg1 : i32
  // CHECK-NEXT: } else {
  }
  ...
  // CHECK: hlcf.comptime.match <1> -> i32, i64 {
  %1:2 = hlcf.comptime.match <1> -> i32, i64 {
    hlcf.match.next
  }
  case {
    hlcf.match.complete %arg1, %arg2 : i32, i64
...
// match.
// CHECK-LABEL: @nested_comptime_match
kgen.func @nested_comptime_match(%arg0: !kgen.scalar<bool>) {
  // CHECK: hlcf.comptime.match <0> {
  hlcf.comptime.match <0> {
    // CHECK: hlcf.comptime.match <1> {
    hlcf.comptime.match <1> {
      // CHECK: hlcf.match.next
      hlcf.match.next
    // CHECK-NEXT: } else {
    }
