...
// CHECK-NEXT:    hlcf.return %[[ARG0]] : si32
// CHECK-NEXT: }
...
kgen.generator @call_generator_test(%arg0: si32, %arg1: si32) -> (si32, si32, si32) {
  ...
  hlcf.return %0, %1, %2 : si32, si32, si32
}
...
// CHECK:    hlcf.return %[[V2]] : !kgen.scalar<f32>
...
// CHECK-NEXT: hlcf.return %[[V0]] : !kgen.scalar<ui64>
...
// CHECK-NEXT: hlcf.return %[[V0]] : !kgen.simd<2, f32>
...
kgen.generator @parametricAdd<sz, dt: dtype>
  ...
  hlcf.return %res : !kgen.simd<sz,dt>
}
...
kgen.generator @nopExample<dt:dtype>(%arg0: !kgen.scalar<dt>) -> !kgen.scalar<dt> {
  hlcf.return %arg0 : !kgen.scalar<dt>
}
...
// CHECK-NEXT: hlcf.return %[[V0]] : !kgen.scalar<f32>
...
kgen.generator @fwd(%a: !kgen.scalar<bool>) -> !kgen.scalar<bool> {
  hlcf.return %a : !kgen.scalar<bool>
}
...
kgen.generator @constexprIfBasic() {
  ...
  // CHECK-NEXT: "should.appear"
  %0 = hlcf.comptime.if lt(cond_var, 10) -> index {
...
kgen.generator @nestedConstexprIf() {
  ...
  %0 = hlcf.comptime.if lt(cond_var, 10) -> index {
    ...
    %3 = hlcf.comptime.if gt(cond_var, 30) -> index {
      %4 = "should.appear"() : () -> index
      hlcf.comptime.yield %4 : index
    } else {
...
kgen.generator @constexprIfInputParam<x>() {
  // CHECK-NEXT: "should.appear"
  %0 = hlcf.comptime.if gt(x, 10) -> index {
...
kgen.generator @constexprIfFunctionCallCondition() -> index {
  // CHECK-NEXT: [[RES:%[0-9]+]] = "should.appear"
  %1 = hlcf.comptime.if apply(:() -> !kgen.scalar<bool> @returnTrue) -> index {
    %2 = "should.appear"() : () -> index
    // CHECK-NEXT: hlcf.return [[RES]]
...
kgen.generator @returnTrueStruct() -> !kgen.struct<(scalar<bool>)> {
  ...
  hlcf.return %1 : !kgen.struct<(scalar<bool>)>
}

// CHECK-LABEL: @"ifFn
kgen.generator @ifFn<true: !kgen.struct<(scalar<bool>)>>() -> index {
  // CHECK-NEXT: [[RES:%[0-9]+]] = "should.appear"
  %1 = hlcf.comptime.if apply(:(!kgen.struct<(scalar<bool>)>) -> !kgen.scalar<bool> @returnInputParam, true) -> index {
    %2 = "should.appear"() : () -> index
    // CHECK-NEXT: hlcf.return [[RES]]
    hlcf.comptime.yield %2 : index
  } else {
...
// CHECK-NEXT:    hlcf.return %0 : index
...
kgen.generator @create<T: type>(%arg0: !kgen.param<T>) -> !kgen.variant<T, i1> {
  ...
  hlcf.return %0 : !kgen.variant<T, i1>
}
...
kgen.generator @rebind_type<p>(%arg0: !kgen.pointer<array<p, i8>>)
  ...
  hlcf.return %0 : !kgen.pointer<#paramtype>
}
...
kgen.generator @fwd_sig(%arg0: !kgen.generator<<type>(!kgen.param<apply(:(!kgen.type)->!kgen.type @fwd_type, *(0,0))>) -> ()>) -> !kgen.generator<<type>(!kgen.param<apply(:(!kgen.type)->!kgen.type @fwd_type, *(0,0))>) -> ()> {
  hlcf.return %arg0 :  !kgen.generator<<type>(!kgen.param<apply(:(!kgen.type)->!kgen.type @fwd_type, *(0,0))>) -> ()>
}
...
kgen.generator @forward_pointer(%arg0: !kgen.pointer<index>) -> !kgen.pointer<index> {
  hlcf.return %arg0 : !kgen.pointer<index>
}
...
kgen.generator @"Contrived::__foo__(::Int)"<n: index>(%arg0: !kgen.scalar<index>) -> !kgen.simd<n, index> {
  ...
  hlcf.return %result : !kgen.simd<n, index>
}
...
kgen.generator @use_fooable<T: type, n: index>(%arg: !kgen.param<T>) -> !kgen.simd<n, index> {
  ...
  hlcf.return %result3 : !kgen.simd<n, index>
}
...
kgen.generator @get_struct_field_types<T: type>() -> !kgen.param_list<type> {
  ...
  hlcf.return %param_list : !kgen.param_list<type>
}
...
kgen.generator @get_struct_field_names<T: type>() -> !kgen.param_list<string> {
  ...
  hlcf.return %param_list : !kgen.param_list<string>
}
...
kgen.generator @get_hello_address() -> !kgen.pointer<scalar<si8>> {
  ...
  hlcf.return %1 : !kgen.pointer<scalar<si8>>
}
...
kgen.generator @conformance_check<T: type>() always_inline {
  hlcf.comptime.if #kgen.type_conforms_to_trait<#kgen.param.decl.ref<"T"> : !kgen.type, #kgen.type<typevalue<#kgen.trait_ref<[@"A", @"B"]>>, type> : !kgen.type> {
    "sink-conforms-to"() : () -> ()
    hlcf.comptime.yield
  } else {
...
// Multi-condition hlcf.comptime.if: first false, second true — elaborator
...
// Multi-condition hlcf.comptime.if: all conditions false — elaborator takes else.
