...
lit.fn @throwing_func(%1: !lit.struct<@Error>) throws -> !kgen.variant<@Error, none> {
  ...
  // CHECK: hlcf.return %0 : !kgen.variant<struct<() memoryOnly>, none>
...
lit.fn @return_raise_or(%cond: !kgen.scalar<bool>, %err: !lit.struct<@Error>) -> !kgen.variant<@Error, none> {
  ...
  hlcf.if %cond {
    ...
    hlcf.return %0 : !kgen.variant<@Error, none>
  } else {
    hlcf.yield
  }
  ...
  hlcf.return %1 : !kgen.variant<@Error, none>
}
...
lit.fn @removeMetadata[imm a](%arg0: !lit.ref<index, imm a> mut) throws -> !kgen.variant<@Error, index> {
  ...
  hlcf.return %1 : !kgen.variant<@Error, index>
}
...
// CHECK-SAME{LITERAL}: fnArgAttrs = [[], ["llvm.someattr", 2 : index]]
// CHECK-SAME: fnAttrs = ["llvm.someattr",  3 : index]
lit.fn @metadata(%a: i32, %b: i32) attributes {
  fnArgAttrs = [[], ["llvm.someattr", 2 : index]],
  fnAttrs = ["llvm.someattr", 3 : index]
} {
  // CHECK: kgen.param.declare.region metadataNested
  lit.fn metadataNested(%c: i32, %d: i32) attributes {
    fnArgAttrs = [[], ["llvm.someattr", 4 : index]],
    fnAttrs = ["llvm.someattr",  5 : index]
  } {
    // CHECK-NEXT: hlcf.return
    hlcf.return
  // CHECK-NEXT{LITERAL}: fnArgAttrs = [[], ["llvm.someattr", 4 : index]]
  // CHECK-SAME: fnAttrs = ["llvm.someattr", 5 : index]
  }
  hlcf.return
}
...
lit.fn @testLifetimeOf2[imm *"a`"](%a: !lit.ref<!Mem, imm *"a`"> imm_mem) -> !lit.ref<!Mem, imm *"a`">{
  ...
  hlcf.return %a : !lit.ref<!Mem, imm *"a`">
}
...
lit.fn @callLifetimes[mut lt](%arg0[*""]: !lit.ref<index, mut lt>) -> !lit.ref<index, mut lt> {
  ...
  hlcf.return %0 : !lit.ref<index, mut lt>
}
