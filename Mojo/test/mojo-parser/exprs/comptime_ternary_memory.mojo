...
# kgen.comptime.if (not hlcf.if) for a memory-only type (String), and that the
...
# A comptime Bool condition on a memory-only ternary must produce kgen.comptime.if,
