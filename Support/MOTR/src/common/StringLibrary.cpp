...
void StringLibrary::ChunkDeleter::operator()(Chunk *chunk) const {
  delete chunk;
}
...
StringLibrary::StringLibrary(size_t maxChunks)
    : maxChunks(maxChunks),          // use setMaxChunks() to change
      placeholderChunk{new Chunk()}, //
      strings({})                    //
{
  chunks.emplace_back(ChunkPtr{new Chunk()});
}
...
std::string_view StringLibrary::getString(std::string_view str) const {
  Hash::Value hash{str};
  return getString(hash.v, false);
}
...
StringLibrary::addString2(std::string_view str) {
  ...
  std::string_view sv = getString(hash.v);
  if (sv.data() != nullptr)
    return {sv, hash.v};

  assert(!chunks.empty() && "No chunks in library");

  sv = chunks.back()->addString(hash.v, str);
  ...
  if (sv.data() == nullptr) {
    ...
    chunks.push_back(ChunkPtr{new Chunk()});
    sv = chunks.back()->addString(hash.v, str);
  }
