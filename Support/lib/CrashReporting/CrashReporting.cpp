...
ErrorOr<std::filesystem::path> M::getCrashpadHandlerPath(Config &config,
  ...
  if (program.empty()) {
    std::string mainExecutable = llvm::sys::fs::getMainExecutable(
        argv0, reinterpret_cast<void *>(generateNonFatalDump));
...
ErrorOr<std::filesystem::path> M::getCrashpadHandlerPath(Settings &settings) {
  ...
  auto program = llvm::dyn_cast_if_present<StringRef>(handlerPath);
...
static ErrorOrSuccess tryInitCrashpad(StringRef program, Settings &settings) {
  ...
  auto url = llvm::dyn_cast_if_present<StringRef>(urlSetting);
  std::string defaultURL;
