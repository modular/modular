...
static void fillWithDistribution(MutableArrayRef<EltType> buffer,
...
static ErrorOrSuccess fillFP(MutableArrayRef<EltType> buffer) {
  fillWithDistribution(buffer,
                       std::uniform_real_distribution<EltType>(-1.0, 1.0));
  return success();
}

template <typename EltType, typename DistributionT = EltType>
static ErrorOrSuccess fillInt(MutableArrayRef<EltType> buffer, bool isSigned) {
  EltType lowerLimit = isSigned ? -10 : 0;
  fillWithDistribution(
      buffer, std::uniform_int_distribution<DistributionT>(lowerLimit, 10));
  return success();
}
static ErrorOrSuccess fillInt(MutableArrayRef<uint8_t> buffer, bool isSigned) {
  return fillInt<uint8_t, uint16_t>(buffer, isSigned);
}
static ErrorOrSuccess fillInt(MutableArrayRef<int8_t> buffer, bool isSigned) {
  return fillInt<int8_t, int16_t>(buffer, isSigned);
}
...
ErrorOrSuccess M::fillRandom(void *destPtr, size_t numElements, DType eltType) {
  return eltType.dispatch<ErrorOrSuccess>(destPtr)
      .when([&](bool *destPtr) {
        fillWithDistribution(makeMutableArrayRef(destPtr, numElements),
                             std::bernoulli_distribution());
        return success();
      })
      .whenCXXInt([&](auto *destPtr) {
        return fillInt(makeMutableArrayRef(destPtr, numElements),
                       eltType.isSInt());
      })
      .whenCXXFP([&](auto *destPtr) {
        return fillFP(makeMutableArrayRef(destPtr, numElements));
      })
      .otherwise([&]() {
