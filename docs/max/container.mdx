...
- `maxserve_dp_active_token_occupancy`: Histogram of per-batch data-parallel
  balance, measured as the mean/max of per-rank active-token load as a
  percentage. 100% means perfectly balanced ranks. Recorded only when
  `data_parallel_degree > 1`.
- `maxserve_dp_context_token_occupancy`: Histogram of per-batch data-parallel
  balance, measured as the mean/max of per-rank context-token (KV/attention)
  load as a percentage. 100% means perfectly balanced ranks. Recorded only when
  `data_parallel_degree > 1`.
- `maxserve_vision_images_encoded_total`: Total images run through the vision
  encoder (cache misses).
- `maxserve_vision_images_cached_total`: Total images served from the vision
  encoder cache (cache hits).
- `maxserve_vision_patches_encoded_total`: Total image patches fed to the
  vision encoder.
- `maxserve_vision_tokens_encoded_total`: Total merged vision tokens produced
  by the vision encoder.
- `maxserve_vision_cache_hit_rate`: Histogram of per-batch vision encoder cache
  hit rate (0–100%).
