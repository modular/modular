...
- `maxserve_cache_hits_tokens_total`: Cumulative KV cache hit tokens (prompt
  tokens served from the prefix cache). Carries a `tier` label naming what
  ...
  The per-tier series sum to the untagged total, so a single-series query that
  doesn't group by `tier` is unaffected. A query that *pairs* this counter with
  a differently-labelled one does need updating: PromQL binary operations match
  on identical label sets, so a hit-rate expression against
  `maxserve_cache_misses_tokens_total` (which carries no `tier`) returns empty
  until you add `ignoring(tier)` or wrap the hits side in
  `sum without(tier) (...)`.
...
- `maxserve_dkv_reconnect_attempts`: Cumulative dKV reconnect attempts across
  all clients.
- `maxserve_dkv_read_blocks_total`: Cumulative KV blocks that landed in device
  memory from the dKV tier. Only confirmed-complete transfers count, so this is
  delivered reuse rather than an upper bound on it. Emitted only on dKV
  deployments: other connectors stamp
  `maxserve_cache_hits_tokens_total{tier="external"}` without reporting this
  counter, so don't read a missing series as blocks failing to land. On a dKV
  deployment the two are proportional for every load that lands, and comparing
  them needs the server's `--kv-cache-page-size`, since this counter is in
  blocks and that one is in tokens.
