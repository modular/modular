...
module provides functions for creating tensors with random values.
[`F.uniform()`](/max/api/python/experimental.functional/#max.experimental.functional.uniform)
generates values uniformly distributed over a range;
[`F.normal()`](/max/api/python/experimental.functional/#max.experimental.functional.normal)
