...
operation,
[`F.broadcast_to(x, shape)`](/max/api/python/experimental.functional#max.experimental.functional.broadcast_to)
...
[`Tensor.unsqueeze(axis)`](/max/api/python/generated/max.experimental.tensor.Tensor#max.experimental.tensor.Tensor.unsqueeze)
...
- **Slice, gather, mask, and scatter tensor values**:
  [Indexing and slicing](/max/develop/indexing)
- **Compile ops and see how shapes carry into graphs**:
  [Get started with MAX graphs](/max/develop/get-started-with-max-graph-in-python)
- **Review elementwise APIs**: [Basic operations](/max/develop/basic-ops)
