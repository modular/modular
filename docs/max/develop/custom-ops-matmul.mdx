...
github_url: https://github.com/modular/modular/tree/modular/v26.2/max/examples/custom_ops
show_languages: true
is_tutorial: true
---
...
API](/max/api/python/generated/max.graph.Graph). Rather, we're using a basic matmul operation
...
```mojo
var col = divmod(thread_idx.x, UInt(BN))
...
`comptime for` construct tells the compiler that this loop can be unrolled at
