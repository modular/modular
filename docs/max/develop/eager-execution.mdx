...
The [`max.experimental`](/max/api/python/experimental/) API abstracts the MAX
framework's [graph](/max/develop/graph/) foundation so you can write and run
...
The [`max.experimental`](/max/api/python/experimental/) API is still under
active development. Use the stable [`max.graph`](/max/api/python/graph) and
[`max.nn`](/max/api/python/nn) APIs to create production-ready models. Learn
more in [Graph overview](/max/develop/graph/).

:::

## Create a tensor

A [`Tensor`](/max/api/python/generated/max.experimental.tensor.Tensor/) is the
...
Here are some
[`max.experimental.functional`](/max/api/python/experimental.functional) APIs
in use:
...
- [Tensor fundamentals](/max/develop/tensors): Learn what tensors are,
  how to create them, and how to inspect their attributes.
- [Basic operations](/max/develop/basic-ops): Apply arithmetic operators,
  reductions, and the functional API to tensors.
- [Data types](/max/develop/dtypes): Specify how each element in a tensor is
