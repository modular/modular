...
[`max.graph`](/max/api/python/graph) and [`max.nn`](/max/api/python/nn)
...
([`max.experimental`](/max/api/python/experimental)) that you can learn about in
the [Eager fundamentals](/max/develop/eager-execution/) section. Be aware that
...
Learn more in
[Build a model graph with Module](/max/develop/modules/), and see [Write a
custom layer](/max/develop/layers/) for components like these.
...
Learn more in [Weight adapters](/max/develop/weight-adapters/).
...
[serve a model](/max/develop/serve-custom-model-architectures/) with your local
...
comparison](/max/develop/logit-comparison/).
