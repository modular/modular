...
[`F.gather`](/max/api/python/experimental.functional#max.experimental.functional.gather),
which takes a tensor of indices and returns the selected values. When a boolean
condition determines which values to keep,
[`F.where`](/max/api/python/experimental.functional#max.experimental.functional.where)
selects element-wise from two tensors based on a mask. To write values back to
specific positions,
[`F.scatter`](/max/api/python/experimental.functional#max.experimental.functional.scatter)
...
[`F.gather(input, indices, axis)`](/max/api/python/experimental.functional#max.experimental.functional.gather)
...
[`F.where(condition, x, y)`](/max/api/python/experimental.functional#max.experimental.functional.where)
...
[broadcasting rules](/max/develop/broadcasting) as arithmetic operations, the
...
[`F.tile`](/max/api/python/experimental.functional#max.experimental.functional.tile)
...
- **Review broadcasting for `F.where` and elementwise alignment**:
  [Broadcasting](/max/develop/broadcasting)
- **Compare eager execution and explicit graph construction**:
  [Eager execution](/max/develop/eager-execution)
