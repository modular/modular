...
- Familiarity with the MAX
  [`Graph`](/max/api/python/generated/max.graph.Graph/) and
  [`Module`](/max/api/python/generated/max.nn.Module/) APIs.
...
[Model debugging overview](/max/develop/debugging/).
...
1. Add print operations to the model with the
   [`PrintHook`](/max/api/python/generated/max.nn.hooks.PrintHook/) class.
   `PrintHook` adds a
   [`print()`](/max/api/python/graph.ops/#max.graph.ops.print) operation to
...
[`load_max_buffer()`](/max/api/python/generated/max.driver.load_max_buffer/),
...
graph compiler, see the [Graph overview](/max/develop/graph/).

## Next steps

Once you've localized the divergent layer and corrected your model:

- [Repeat logit comparison](/max/develop/logit-comparison/) to validate that
...
- Automate layer comparison with the
  ...
  skill.
- [Run benchmarks](/max/deploy/benchmark/) to measure model performance with
  `max benchmark`.
