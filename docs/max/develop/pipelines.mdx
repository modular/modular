...
This page explains how the [`pipelines`](/max/api/python/pipelines) API works,
...
and [`max.pipelines.kv_cache`](/max/api/python/pipelines.kv_cache).
...
- **Core modules**:
  [`Module`](/max/api/python/generated/max.experimental.nn.Module),
  [`Linear`](/max/api/python/generated/max.nn.Linear),
  [`Embedding`](/max/api/python/generated/max.experimental.nn.Embedding),
  [`Sequential`](/max/api/python/generated/max.experimental.nn.Sequential),
  [`ModuleList`](/max/api/python/generated/max.experimental.nn.ModuleList).
- **Normalization**:
  [`RMSNorm`](/max/api/python/generated/max.experimental.nn.norm.RMSNorm),
  [`GemmaRMSNorm`](/max/api/python/generated/max.experimental.nn.norm.GemmaRMSNorm).
- **Positional encodings**:
  [`RotaryEmbedding`](/max/api/python/generated/max.experimental.nn.rope.RotaryEmbedding),
  [`TransposedRotaryEmbedding`](/max/api/python/generated/max.experimental.nn.rope.TransposedRotaryEmbedding).
- **Utilities**:
  [`module_dataclass`](/max/api/python/generated/max.experimental.nn.module_dataclass)
  for creating module dataclasses.

These components are core to building model architectures.

:::note Graph API modules

For graph-based components like attention mechanisms and transformers,
use `max.nn`:
...
The [`Module`](/max/api/python/generated/max.experimental.nn.Module) base class
standardizes how layers manage weights and devices. Here's an example of
building a simple multi-layer perceptron:
...
The [`max.pipelines.kv_cache`](/max/api/python/pipelines.kv_cache) package
provides cache management for transformer inference. The main component is
[`PagedKVCacheManager`](/max/api/python/generated/max.pipelines.kv_cache.PagedKVCacheManager),
...
the
[`supported_encodings`](/max/api/python/generated/max.pipelines.lib.registry.SupportedArchitecture#max.pipelines.lib.registry.SupportedArchitecture.supported_encodings)
...
[`Module`](/max/api/python/generated/max.experimental.nn.Module) class. MAX then
compiles your module into an optimized executable for the target device, and the
pipeline orchestrates execution.

Here's the workflow:

1. **Define your model**: You create a
   [`Module`](/max/api/python/generated/max.experimental.nn.Module) that defines
   your model architecture. The module's
   [`forward`](/max/api/python/generated/max.experimental.nn.Module#max.experimental.nn.Module.forward)
   method defines what computations happen when processing inputs.
...
3. **Pipelines orchestrate execution**: The pipeline receives pre-tokenized
   ...
   your model's
   [`PipelineModel.execute()`](/max/api/python/generated/max.pipelines.PipelineModel#max.pipelines.PipelineModel.execute)
...
The [`PIPELINE_REGISTRY`](/max/api/python/pipelines.lib.registry) is the central
system that maps model architectures to their compiled pipelines and tokenizers.
When you call
[`retrieve()`](/max/api/python/generated/max.pipelines.lib.registry.PipelineRegistry#max.pipelines.lib.registry.PipelineRegistry.retrieve)
with a [`PipelineConfig`](/max/api/python/pipelines), the registry looks up the
...
kv_cache_manager.claim(context.request_id, replica_idx=0)

# 6. Run the generation loop
generated_text = ""
while True:
    # Allocate KV cache for the next token
    kv_cache_manager.alloc(context, replica_idx=0)
...
The [`max.pipelines.modeling.types`](/max/api/python/pipelines.modeling.types)
...
- [`Pipeline`](/max/api/python/generated/max.pipelines.modeling.types.Pipeline):
  Abstract base class for all pipelines. Defines `execute()` and `release()`
  methods that all pipeline implementations must provide.

- [`PipelineInputs`](/max/api/python/generated/max.pipelines.modeling.types.PipelineInputs):
  Base class for inputs to a pipeline, such as text generation requests or
  embeddings requests.

- [`PipelineOutput`](/max/api/python/generated/max.pipelines.modeling.types.PipelineOutput):
  Protocol for pipeline outputs. Must implement `is_done` to signal when
  generation is complete.

- [`PipelineTokenizer`](/max/api/python/generated/max.pipelines.modeling.types.PipelineTokenizer):
  Interface for tokenizers that convert between text and token IDs, and create
  context objects for requests.

- [`PipelineModel`](/max/api/python/generated/max.pipelines.PipelineModel):
...
[`TextGenerationInputs`](/max/api/python/generated/max.pipelines.modeling.types.TextGenerationInputs)
and
[`TextGenerationOutput`](/max/api/python/generated/max.pipelines.context.TextGenerationOutput)
extend them for specific use cases.

### Pipeline registry

The
[`PIPELINE_REGISTRY`](/max/api/python/generated/max.pipelines.lib.registry.PipelineRegistry)
...
[`BatchProcessor`](/max/api/python/generated/max.pipelines.lib.interfaces.BatchProcessor)
...
[`RaggedBatchProcessor`](/max/api/python/generated/max.pipelines.lib.interfaces.RaggedBatchProcessor),
which receives a
[`BatchProcessorRuntime`](/max/api/python/generated/max.pipelines.lib.interfaces.BatchProcessorRuntime)
...
- **KV cache parameters**: The `get_kv_params()` method constructs the KV cache
  ...
  [`MultiKVCacheParams`](/max/api/python/generated/max.nn.kv_cache.MultiKVCacheParams/)
  tree instead, which the cache manager consumes through the shared
  [`KVCacheParamInterface`](/max/api/python/generated/max.nn.kv_cache.KVCacheParamInterface/).
...
3. **Validate compatibility**: The registry checks that the architecture
   supports the requested encoding and KV cache strategy.
...
[`PipelineConfig` reference](/max/api/python/generated/max.pipelines.PipelineConfig)
...
- [Modules](/max/develop/modules): Learn how to build model graphs using
  MAX's `Module` class.
