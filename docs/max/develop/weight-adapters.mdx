...
[`Module`](/max/api/python/generated/max.nn.Module/). Then, integrate the weight
adapter into the [model pipeline](/max/develop/pipelines/).
...
1. The pipeline calls
   [`weights_format()`](/max/api/python/generated/max.graph.weights.weights_format/#max.graph.weights.weights_format))
...
3. The pipeline calls
   [`load_weights()`](/max/api/python/generated/max.graph.weights.load_weights/#max.graph.weights.load_weights)
   to load the checkpoint into a `Weights` object.
4. The pipeline constructs your
   [pipeline model](/max/api/python/generated/max.pipelines.PipelineModel/) with
