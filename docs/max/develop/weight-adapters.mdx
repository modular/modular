...
1. The pipeline calls
   [`weights_format()`](/max/api/python/generated/max.graph.weights.format/#max.graph.weights.format.weights_format)
...
3. The pipeline calls
   [`load_weights()`](/max/api/python/generated/max.graph.weights/#max.graph.weights.load_weights)
