...
MAX reads the standard OpenTelemetry SDK variables, so you can point it
at your own collector without MAX-specific configuration.
...
| `OTEL_EXPORTER_OTLP_ENDPOINT`         | Base endpoint, signal path appended  | URL             | Modular's               |
| `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT`  | Trace endpoint, used as given        | URL             | Modular's               |
...
A signal-specific endpoint takes precedence over the base endpoint. Setting
`OTEL_EXPORTER_OTLP_ENDPOINT` to `http://collector:4318` sends traces to
`http://collector:4318/v1/traces`, while `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT`
is used exactly as given.

MAX exports over OTLP HTTP only, so the endpoint must be an HTTP
receiver, conventionally port 4318. Pointing it at a gRPC receiver on port
4317 drops every span and every metric.

The published container images set `OTEL_SERVICE_NAME` to `max-serve`, so
override it explicitly rather than leaving it unset:

```bash
docker run --env "OTEL_SERVICE_NAME=my-service" modular/max-nvidia-full:latest ...
```

`OTEL_SDK_DISABLED` is enabled only by the value `true`, ignoring case and
