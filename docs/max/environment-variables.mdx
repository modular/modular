...
| `MAX_SERVE_API_TYPES`                   | Configures which API types MAX serve exposes. Accepts a JSON array of API type strings (e.g., `'["responses"]'`). Use this to enable the [Responses API](/serve/image-generation) for tasks like image generation. | JSON array string     | None                |
...
[MAX container telemetry](/container#telemetry).
