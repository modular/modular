...
[image and video to text](/max/inference/image-to-text) guides.
...
| `MAX_SERVE_API_TYPES`             | Configures which API types MAX serve exposes. Accepts a JSON array of API type strings (e.g., `'["responses"]'`). Use this to enable the [Responses API](/max/inference/image-generation) for tasks like image generation. | JSON array string     | None                |
...
[MAX container](/max/container#logs).
...
[MAX container telemetry](/max/container#telemetry).
