...
[`v1/responses`](/max/rest-api/#POST/v1/responses) endpoint to generate
images from text prompts or transform existing images, with examples for each
input type.

## Endpoint

The MAX [`v1/responses`](/max/rest-api/#POST/v1/responses) endpoint
...
[`provider_options`](/max/rest-api/#POST/v1/responses.body.provider_options).
...
[`max serve`](/max/cli/serve). FBCache reuses transformer state from the
...
**TaylorSeer**: enable with `--taylorseer` on
[`max serve`](/max/cli/serve). Available on the FLUX.2 Klein pipeline,
...
GPU](/max/packages#gpu-compatibility) with sufficient GPU RAM.
...
<InstallModular folder="image-generation-quickstart" />

### Serve your model

First, enable the `v1/responses` endpoint by setting the `MAX_SERVE_API_TYPES`
[environment variable](/max/environment-variables):
...
Then, use the [`max serve`](/max/cli/serve) command to start a local model
...
<ListingCards>
  <ListingCard title="Image and video to text"
    url="/max/inference/image-to-text"
  ...
  <ListingCard title="Structured output"
    url="/max/serve/structured-output"
    description="Enable structured output with your GenAI deployments for predictable responses" />
  <ListingCard title="Deploy MAX on GPU with self-hosted endpoints"
    url="/max/deploy/local-to-cloud"
