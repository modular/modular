...
For teams that need to scale, our ready-to-deploy Docker container removes
the complexity of deploying your own GenAI endpoint in the cloud infrastructure
of your choice.
...
Modular is a vertically integrated AI infrastructure stack that provides entry
points for users at every level.
...
- [**Start with pip**](/get-started): Install MAX with `pip` and run
inference with Python or a REST endpoint.

- [**Start with Docker**](/container): Run our Docker container to create a
REST endpoint.

In either case, you can select from hundreds of our
[supported models](/models/). You can also load weights from Hugging Face or
load your own fine-tuned weights.

For performance optimization, you can port models from PyTorch to MAX using
[graphs and modules](/develop/modules). For deeper customization, you
can extend MAX Models with [custom operations](/develop/build-custom-ops)
(ops) written in Mojo. Your custom ops are automatically analyzed and fused
into the model graph, delivering low-level acceleration without sacrificing
developer productivity.

## Stay in touch

Once you've explored MAX's features and experimented with self-hosted
endpoints, [reach out](https://www.modular.com/request-demo) to talk through
