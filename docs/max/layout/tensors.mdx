...
You can also create a `LayoutTensor` using a `Pointer`. This may be
...
- On the GPU: When a `DeviceBuffer` is passed in to
  ...
  `Pointer` in place of the `DeviceBuffer.` The kernel can then create a
...
or
[`enqueue_memset()`](https://mojolang.org/docs/std/gpu/host/device_context/DeviceContext/#enqueue_memset)
