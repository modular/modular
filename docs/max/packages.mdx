...
build. (Each release is described in the [release notes](/releases/).)
...
<Tabs groupId="install">
  <TabItem value="pixi" label="conda/pixi">
    ...
    - MAX tools and libraries:
      - [`max` CLI](/max/cli/)
      - [`max` Python library](/api/python/)
      - [`max` accelerator library](/api/kernels/)
      - [AI kernels library](/api/kernels/)
...
guide](/max/deploy/local-to-cloud/).
