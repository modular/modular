...
| `maxserve_cache_used_host_kv_pct_percent`                 | Histogram | Percentage of the KV connector's host tier bytes in use (0-100%), sampled once per scheduler batch when a connector with a host tier is configured.                                                                            |
| `maxserve_cache_used_disk_kv_pct_percent`                 | Histogram | Percentage of the KV connector's disk tier bytes in use (0-100%), sampled once per scheduler batch when the connector has a disk tier.                                                                                         |
...
| `maxserve_cache_h2d_bytes_copied_bytes_total`             | Counter   | Cumulative KV bytes copied from the connector's host tier to device. Rate this to see the PCIe bandwidth the host tier consumes.                                                                                               |
...
To send traces and metrics to your own collector, set the standard
[`OTEL_EXPORTER_OTLP_ENDPOINT`](/environment-variables) environment variable.
[`MAX_SERVE_OTLP_METRICS_ENDPOINT`](/environment-variables) is a separate
opt-in that additionally pushes an exponential-histogram shadow of every
histogram; leave it unset unless you want that comparison.
