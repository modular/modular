...
The root of a `json_schema` must resolve to an object. If the root omits `type`,
MAX treats it as `type: object`. MAX also accepts root-level type unions that
include `object`, such as `["object", "array", "string"]`. MAX rejects schemas
whose root is constrained to a single non-object type (for example, `{"type": "string"}`).
