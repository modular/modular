...
Some models define extra roles through their [chat
template](/serve/chat-templates). For example, MAX also accepts a `root` role,
but only when the model's tokenizer enables it. Requests that use a role the
model doesn't support are rejected.
