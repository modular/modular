...
- Mojo APIs to write custom op functions:
  ...
  - The [`max.extensibility`](/api/kernels/extensibility/) package adds
    essential Mojo APIs for writing custom ops, such as:
    - The
      [`foreach()`](/api/kernels/extensibility/managed_tensor_slice/foreach/)
      function, which efficiently executes an element-wise computation in
      parallel on either a GPU or CPU.

    - The
      [`ManagedTensorSlice`](/api/kernels/extensibility/managed_tensor_slice/ManagedTensorSlice/)
