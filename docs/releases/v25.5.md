...
- **OpenAI-compatible batch API**: The [`/v1/batches`
  API](/max/rest-api/) is now available with
  Mammoth.
...
- **Open-source graph APIs**: We've added the `max.graph` Python APIs to our
  ...
  [serve with MAX](/max/develop/serve-custom-model-architectures/).

## Documentation {#25-5-docs}

- New
  [Serve custom model architectures tutorial](/max/develop/serve-custom-model-architectures/),
  with
...
- Updated the [Deploy Llama 3 on GPU
  tutorial](/max/deploy/local-to-cloud/) with instructions using
  AMD MI300X (on Azure).
...
- Added
  [`max.experimental.torch.graph_op`](/max/api/python/generated/max.experimental.torch.graph_op/#max.experimental.torch.graph_op)
