...
- Added the `max benchmark` subcommand that runs a suite of benchmarks and
  ...
  [`benchmark_serving.py`](https://github.com/modular/modular/tree/max/v26.5/benchmark#benchmark-max)
...
- Added `generate()` to
  ...
  and
  [`StandaloneSpeculativeDecodingPipeline`](max.pipelines.lib.StandaloneSpeculativeDecodingPipeline),
