...
- The [`max` CLI docs](/max/cli/) are now generated from
...
- Added [`max.profiler.gpu`](/max/api/python/profiler.gpu/) APIs to expose
  common GPU statistics as might be reported by `nvidia-smi` or `rocm-smi`.

- Added the [`max.pipelines.kv_cache`](/max/api/python/pipelines.kv_cache/)
  package, which provides APIs to manage key-value caches used in transformer
  models. Not to be confused with the existing
  [`max.nn.kv_cache`](/max/api/python/nn.kv_cache/) package that includes
...
- Added
  [`NullKVCacheManager`](/max/api/python/generated/max.pipelines.kv_cache.DummyKVCache/)
  for compile-only mode, which avoids GPU memory allocation when compiling
  models without a physical GPU present.

- Added
  [`ResetPrefixCacheBackend`](/max/api/python/pipelines.kv_cache/)
  and
  [`ResetPrefixCacheFrontend`](/max/api/python/pipelines.kv_cache/)
...
- Changed
  [`LoRAConfig.max_num_loras`](/max/api/python/generated/max.pipelines.lora.LoRAConfig/#max.pipelines.lora.LoRAConfig.max_num_loras)
...
- Added
  [`ImageMetadata`](/max/api/python/interfaces/#max.interfaces.ImageMetadata)
  and
...
- Renamed the `tensor_internal` package to `tensor` and removed the
  ...
  docs](/max/api/kernels/extensibility/) moved.
