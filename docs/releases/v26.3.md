...
- New API for **multi-GPU model execution from Python**: the
  [`max.experimental.sharding`](/max/api/python/generated/max.experimental.sharding)
...
- Added new model developer guides covering
  ...
  [indexing](/max/develop/indexing/), and the
  [model bring-up workflow](/max/develop/model-bringup-workflow/).
- Added a [graph overview](/max/develop/graph/) and a new
  [graph and modules guide](/max/develop/modules/).
- Added [model debugging guides](/max/develop/debugging/) for accuracy, errors,
  GPU, and tracing.
- Updated the [speculative decoding](/max/serve/speculative-decoding/) guide.
- Updated the guide to
  [serve custom models](/max/develop/serve-custom-model-architectures/).
- Added API docs for
  [`max.pipelines.architectures`](/max/api/python/pipelines.architectures/).
- Redesigned [REST API reference](/max/rest-api/), now built with Scalar.
