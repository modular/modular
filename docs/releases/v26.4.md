...
- The new `import-model` skill converts a Hugging Face model into a
  ...
  the guide to
  [bringing up a model with AI agent skills](/max/develop/model-bringup-skills/).
...
- Added a [video generation](/max/inference/video-generation/) guide for serving
  text-to-video models (such as `Wan-AI/Wan2.2-T2V-A14B-Diffusers`) with the
  `v1/responses` endpoint.
- Added new model developer guides covering
  [reading a Hugging Face model config](/max/develop/read-model-config/)
  and [tensor realization](/max/develop/tensor-realization/).
- Added a guide to using the new `import-model` skill, called
  [Bringing up a model with AI agent skills](/max/develop/model-bringup-skills/).
- Updated guides for
  [speculative decoding](/max/serve/speculative-decoding/) (Kimi K2.6 EAGLE3 and
  Gemma 4 MTP), [prefix caching](/max/serve/prefix-caching/), and
  [fine-tuned model serving](/max/develop/max-pipeline-bring-your-own-model/).
...
- Added the `nemotron-opencode` dataset for
  [`max benchmark`](/max/cli/benchmark/), backed by
...
- Added [`max.graph.Module`](/max/api/python/generated/max.graph.Module/) for
...
- Added [`CompiledModel`](/max/api/python/generated/max.engine.CompiledModel/),
...
- Added
  [`CompletionFlag`](/max/api/python/generated/max.driver.CompletionFlag/), an
