...
struct Blur:
    ...
    fn execute[
        ...
        fn blur_kernel[
            ...
            var size = Int(blur_size)  # Kernel half-size

            var result_simd = SIMD[DType.uint8, simd_width]()

            # Process each lane in the SIMD vector
            for lane in range(simd_width):
                # FIXME(CRITICAL): figure out how to take "lane" into account
                var base_row: Int = idx[0]
                var base_col: Int = idx[1]

                var pix_val_accum: Float32 = 0
                var pixel_count: Float32 = 0
                ...
                for blur_row_offset in range(-size, size + 1):
                    for blur_col_offset in range(-size, size + 1):
                        ...
                        if 0 <= cur_row < height and 0 <= cur_col < width:
                            var neighbor_idx = IndexList[2](cur_row, cur_col)
                            # Load 1 element
                            var neighbor_val = img_in.load[1](neighbor_idx)
                            pix_val_accum += neighbor_val.cast[DType.float32]()
                            pixel_count += 1.0

                var avg_val: Float32 = 0.0
                if pixel_count > 0.0:
                    avg_val = pix_val_accum / pixel_count

                result_simd[lane] = avg_val.clamp(0, 255).cast[DType.uint8]()

            return result_simd

        # Apply the kernel to each element (or SIMD vector) of the output tensor
        foreach[blur_kernel, target=target](img_out, ctx)
