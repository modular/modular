...
# ===----------------------------------------------------------------------=== #
"""Execution config placeholder for AMD GCN 3rd generation (Volcanic Islands)
GPUs: Carrizo (gfx801), Tonga/Iceland (gfx802, e.g. Radeon R9 285/380), Fiji
(gfx803, e.g. Radeon R9 Fury), TongaPro (gfx805), Stoney (gfx810)."""
...
# Fiji (gfx803) shares its LLVM compiler target with GCN4 Polaris10/Polaris11
# (see gcn4.mojo); the two are split here because they are distinct product
# families, not distinct compiler targets.
struct GCN3ExecutionConfiguration(
    BlockExecutionConfig,
    ConstantMemoryMappingExecutionConfig,
    Copyable,
    ExecutionConfig,
