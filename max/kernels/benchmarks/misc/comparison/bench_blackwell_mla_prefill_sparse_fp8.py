...
# MAX-only benchmark for the FP8 sparse MLA prefill kernel
# (mo.mla.prefill.sparse.paged.fp8).
# Measures the same performance shapes as our existing BF16 sparse MLA prefill
# benchmark as well as FlashMLA's test_flash_mla_sparse_prefill.py.
#
# Run directly (no kbench):
#   python bench_blackwell_mla_prefill_sparse_fp8.py
#   python bench_blackwell_mla_prefill_sparse_fp8.py --num_iters 200
#   python bench_blackwell_mla_prefill_sparse_fp8.py --no-kineto   # for ncu/nsys
#
# Output format:
...
# Performance shapes matching FlashMLA's benchmark and our internal BF16 benchmark.
...
def bench_max(
    ...
    """Benchmark MAX FP8 sparse MLA prefill for one (s_q, s_kv, topk, h_q, qk_depth) shape.
    ...
    # Q stays BF16; KV cache is FP8 with one float32 scale per physical token.
    q_torch = torch.randn(
        s_q, h_q, qk_depth, dtype=torch.bfloat16, device="cuda"
    )

    # kv_blocks: [num_pages, 1, 1, PAGE_SIZE, 1, qk_depth] in fp8.
    # DLPack doesn't support float8, so transfer as uint8 and re-view.
    kv_blocks_bf16 = torch.randn(
        num_pages,
        1,
        1,
        _PAGE_SIZE,
    ...
    kv_blocks_fp8 = kv_blocks_bf16.to(torch.float8_e4m3fn)

    # kv_scales: [num_pages * PAGE_SIZE] float32 — one scale per physical KV token.
    # (scale_block_size == qk_depth, so tensorwise: one scale per token.)
    total_phys_tokens = num_pages * _PAGE_SIZE
    kv_scales_torch = torch.ones(
        total_phys_tokens, dtype=torch.float32, device="cuda"
    )
    ...
    # fp8 buffers must go through uint8 view for dlpack compatibility.
    kv_blocks_max = Buffer.from_dlpack(kv_blocks_fp8.view(torch.uint8)).view(
        DType.float8_e4m3fn
    )
    ...
    kv_scales_type = TensorType(
        DType.float32, shape=[total_phys_tokens], device=DeviceRef.GPU()
    )

    with Graph(
        "mla_prefill_sparse_fp8_bench",
        input_types=[
            q_type,
            blocks_type,
            cache_lengths_type,
    ...
    # 2 * total_topk * h_q * (d_qk + d_v).
    flops = 2 * s_q * topk * h_q * (qk_depth + v_depth)
    mem_bytes = (
        s_q * h_q * qk_depth * 2  # Q read (bf16 = 2 bytes)
        + s_q * topk * qk_depth * 1  # sparse KV read (fp8 = 1 byte)
        + s_q
        * topk
        * 4  # KV scales (float32 = 4 bytes, one per selected token)
        + s_q * h_q * v_depth * 2  # output write (bf16 = 2 bytes)
    )
    ...
    # mla_prefill_sparse_fp8_nqh{H}_nkvh1_{hash}
    # "mla_prefill_sparse_fp8" is a stable substring across all h_q variants.
    # Use --verbose-kineto to print the full table if the name ever changes.
    try:
        time_s = bench_kineto_with_cupti_warmup(
            run_kernel,
            kernel_names="mla_prefill_sparse_fp8",
            num_tests=num_iters,
        ...
        if "No kernel times found" in str(e):
            print(
                f"  Warning: kineto could not find kernel 'mla_prefill_sparse_fp8'. "
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="MAX FP8 sparse MLA prefill benchmark (mo.mla.prefill.sparse.paged.fp8)"
    )
    parser.add_argument(
