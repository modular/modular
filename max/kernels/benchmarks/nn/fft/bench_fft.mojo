...
def main():
    ...
    alias name = env_get_string["name", "cpu_multi_threaded"]()
    alias dtype = env_get_dtype["dtype", DType.float64]()
    alias bases_int = env_get_shape["shape", "[16, 8]"]()
    alias bases = UnsafePointer(to=bases_int.copy()).bitcast[List[UInt]]()[]
    ...
    alias cpu_fn = bench_cpu_radix_n_rfft
    alias N = 1_000_000

    @parameter
    if name == "gpu_sequential":
        alias func = bench_sequential_intra_block_radix_n_rfft
        alias func_name = "bench_sequential_intra_block_radix_n_rfft["
