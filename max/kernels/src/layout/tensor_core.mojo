...
struct TensorCore[
    ...
    fn _load_a_amd[
        ...
        # FP8 dtypes: use invalid on RDNA3 to avoid constraint failures
        alias fp8_dtype = DType.invalid if (
            _is_amd_rdna() and not _is_amd_rdna4()
        ) else get_amd_fp8_dtype()
        alias bf8_dtype = DType.invalid if (
            _is_amd_rdna() and not _is_amd_rdna4()
        ) else get_amd_bf8_dtype()
    ...
    fn _store_d_amd(
        ...
        if out_type is DType.float32:
            ...
            if reg_per_thread == 8:
                ...
                # NVIDIA and CDNA use 4 or 16 registers
    ...
    if has_nvidia_gpu_accelerator():
        ...
        if _is_amd_gcn():
            constrained[
                False,
                (
                    "GCN tensor core support requires fallback"
