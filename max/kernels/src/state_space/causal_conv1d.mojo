...
def causal_conv1d_fwd_cpu[
    ...
    seqlen: Int,
    width: Int,
    x: TileTensor[x_dtype, ...],  # (B, C, L) or (B, L, C) via strides
    weight: TileTensor[weight_dtype, ...],  # (C, W)
    output: TileTensor[mut=True, output_dtype, ...],  # same layout as x
    bias: TileTensor[bias_dtype, ...],  # (C,) — ignored unless has_bias
    seq_idx: TileTensor[
        seq_idx_dtype, ...
...
def causal_conv1d_channel_first_fwd_gpu[
    ...
    seqlen: Int,
    width: Int,
    x: TileTensor[x_dtype, x_LT, ImmutExternalOrigin],  # (B, C, L)
    weight: TileTensor[weight_dtype, weight_LT, ImmutExternalOrigin],  # (C, W)
    output: TileTensor[output_dtype, output_LT, MutExternalOrigin],  # (B, C, L)
    bias: TileTensor[bias_dtype, bias_LT, ImmutExternalOrigin],  # (C,)
    seq_idx: TileTensor[
        seq_idx_dtype, seq_idx_LT, ImmutExternalOrigin
    ],  # (B, L)
    x_batch_stride: UInt32,
    x_c_stride: UInt32,
    x_l_stride: UInt32,
...
def causal_conv1d_channel_last_fwd_gpu[
    ...
    seqlen: Int,
    width: Int,
    x: TileTensor[x_dtype, x_LT, ImmutExternalOrigin],  # (B, L, C)
    weight: TileTensor[weight_dtype, weight_LT, ImmutExternalOrigin],  # (C, W)
    output: TileTensor[output_dtype, output_LT, MutExternalOrigin],  # (B, L, C)
    bias: TileTensor[bias_dtype, bias_LT, ImmutExternalOrigin],  # (C,)
    seq_idx: TileTensor[
        seq_idx_dtype, seq_idx_LT, ImmutExternalOrigin
    ],  # (B, L)
...
def causal_conv1d_update_cpu[
    ...
    x: TileTensor[x_dtype, ...],  # (B, C, L) or (B, C) when L=1
