...
# ===----------------------------------------------------------------------=== #
"""General BF16×BF16+FP32→FP32 MMA test for all GPU architectures."""
...
fn test_mma_bf16_kernel(
    A: UnsafePointer[BFloat16],
    B: UnsafePointer[BFloat16],
    ...
    - AMD RDNA3+ (RX 7000, W7000): v_wmma_f32_16x16x16_bf16 instructions
    - AMD RDNA1/2: Falls back to scalar operations

    BFloat16 (BF16) is critical for modern LLM inference as it maintains FP32's
    exponent range while using half the bits. Major models using BF16:
    ...
    - A/B: 16 bf16 elements per lane (full K=16 dimension)
...
def main():
    ...
    print("Testing BF16×BF16+FP32→FP32 MMA...")
    ...
    with DeviceContext() as ctx:
        ...
        if errors == 0:
            print("✅ BF16 MMA test PASSED (", M * N, "elements correct)")
        else:
            print("❌ BF16 MMA test FAILED (", errors, "errors)")
