...
from std.utils.numerics import isinf, isnan, nan
...
def test_fill_uniform_in_range() raises:
    seed(3)
    var buf = Array[Float32, 512](fill=nan[.float32]())
    var span = Span(buf)
...
def test_fill_all_equal() raises:
    var buf = Array[Float32, 64](fill=nan[.float32]())
    var span = Span(buf)
...
def test_fill_sparse_mostly_zero() raises:
    seed(4)
    var buf = Array[Float32, 4096](fill=nan[.float32]())
    var span = Span(buf)
...
def test_fill_by_dist_all_compile_and_run() raises:
    ...
    for dist in range(NUM_VALUE_DISTS):
        var buf = Array[Float32, 128](fill=nan[.float32]())
        var span = Span(buf)
