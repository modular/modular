...
# ===----------------------------------------------------------------------=== #
"""Dispatch-level NVFP4 test: drives `block_scaled_matmul` (not the naive kernel
directly), so on consumer Blackwell (sm_121) it exercises the routing to the
naive CUDA-core path. Known-answer construction: all FP4 inputs decode to 1.0
and all scales are 1.0, so C[i, j] == K.
"""
...
def test_dispatch_nvfp4_known_answer(ctx: DeviceContext) raises:
    comptime M = 256
    comptime N = 256
    comptime K = 256
    ...
    block_scaled_matmul[SF_VECTOR_SIZE=SF_VECTOR_SIZE, transpose_b=True](
        c, a, b, a_sc, b_sc, Float32(1.0), ctx
    )
    ctx.synchronize()
...
def main() raises:
    with DeviceContext() as ctx:
        test_dispatch_nvfp4_known_answer(ctx)
