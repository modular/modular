...
# ===----------------------------------------------------------------------=== #
#
# Test: block-scaled MXFP8 matmul (sm100, small_bn) with weight prefetching.
#
# Mirrors test_matmul_sm100_block_scaled_mxfp8_small_bn.mojo with
# prefetch_tiles_n=2 added to BlockScaledMatmulConfig.
#
# Tile shapes: cta_group=1, bm=128, mma_n in [8, 16, 32].
# ===----------------------------------------------------------------------=== #
...
def test_blackwell_block_scaled_matmul_tma_umma_warp_specialized[
    ...
    print(
        t"[prefetch-small_bn-mxfp8] in/out dtypes=({a_type}, {b_type},"
        t" {c_type}, {scales_dtype})"
    ...
    vendor_blas.matmul(
        ...
        a_tensor,
        b_tensor,
        a_scales=a_scales_tensor.as_imm(),
        b_scales=b_scales_tensor.as_imm(),
        transpose_b=transpose_b,
        c_row_major=True,
    )
