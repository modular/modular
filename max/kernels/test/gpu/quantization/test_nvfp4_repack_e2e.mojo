# ===----------------------------------------------------------------------=== #
# Copyright (c) 2026, Modular Inc. All rights reserved.
#
# Licensed under the Apache License v2.0 with LLVM Exceptions:
# https://llvm.org/LICENSE.txt
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.
# ===----------------------------------------------------------------------=== #
"""End-to-end numerical-equivalence test for the NVFP4 weight/scale REPACK.

This proves the full canonical-checkpoint chain:

    canonical NVFP4 tensors --(repack_nvfp4_for_sm8x)--> kernel buffer
                            --(multistage_gemm_q[is_nvfp4=True])--> C_kernel

matches an INDEPENDENT CPU/GPU dequant reference:

    canonical NVFP4 tensors --(canonical E2M1 decode * fp8 * global)--> B_dense
                            --(multistage_gemm[transpose_b=True])--> C_ref

The reference dequantizes the canonical tensors directly (no kernel-layout
byte-shuffle), so a match exercises BOTH the repack byte layout AND the kernel
decode. Unlike `test_nvfp4_skeleton_gemm.mojo` (which feeds pre-shuffled bytes
straight into the kernel buffer and inverts them), this test starts from the
genuine canonical checkpoint format:

  - weights:      [N, K // 2] uint8, 2 E2M1 codes/byte (low nibble = even k).
  - block scales: [N, K // 16] float8_e4m3fn (group_size = 16).
  - global scale: scalar float32 (multiply).
"""

from std.math import ceildiv
from std.random import rand, randint, seed
from std.sys import size_of
from std.time import perf_counter_ns

from std.gpu import WARP_SIZE, block_dim, block_idx, thread_idx
from std.gpu.host import DeviceContext, FuncAttribute

from internal_utils import assert_almost_equal
from layout import (
    Coord,
    CoordLike,
    Idx,
    Layout,
    LayoutTensor,
    RuntimeLayout,
    TileTensor,
    row_major,
)
from layout.layout import *
from linalg.matmul.gpu import multistage_gemm
from linalg.utils_gpu import MatmulConfig, MatmulKernels
from quantization.qmatmul_gpu import multistage_gemm_q, repack_nvfp4_for_sm8x

from std.utils.index import Index, IndexList


# E2M1 canonical magnitude LUT: codes 0..7 -> {0,0.5,1,1.5,2,3,4,6}; bit 3 is
# the sign. Identical numerically to the Marlin bit-trick used by the kernel.
@always_inline
def _e2m1_decode(code: UInt8) -> Float32:
    var c = Int(code & 0x7)
    var mags = SIMD[DType.float32, 8](0.0, 0.5, 1.0, 1.5, 2.0, 3.0, 4.0, 6.0)
    var v = mags[c]
    return -v if (code & 0x8) != 0 else v


# Build the dense [N, K] bf16 reference B from the CANONICAL tensors.
def dequant_ref_kernel[
    weights_layout: Layout,
    scales_layout: Layout,
    out_layout: Layout,
    group_size: Int,
](
    weights: LayoutTensor[DType.uint8, weights_layout, ImmutAnyOrigin],
    block_scales: LayoutTensor[
        DType.float8_e4m3fn, scales_layout, ImmutAnyOrigin
    ],
    global_scale: Float32,
    b_out: LayoutTensor[DType.bfloat16, out_layout, MutAnyOrigin],
):
    comptime N = Int(out_layout.shape[0])
    comptime K = Int(out_layout.shape[1])
    var idx = block_idx.x * block_dim.x + thread_idx.x
    if Int(idx) >= N * K:
        return
    var n = Int(idx) // K
    var k = Int(idx) % K

    var byte = weights[n, k // 2]
    var nibble = (byte & 0x0F) if (k % 2 == 0) else ((byte >> 4) & 0x0F)
    var code_val = _e2m1_decode(rebind[UInt8](nibble))

    var fp8 = block_scales[n, k // group_size]
    var scale = fp8.cast[DType.float32]() * global_scale

    b_out[n, k] = (code_val * scale).cast[DType.bfloat16]()


def test_repack_e2e[
    NType: CoordLike,
    KType: CoordLike, //,
    group_size: Int = 16,
](ctx: DeviceContext, M: Int, n: NType, k: KType) raises:
    comptime a_type = DType.bfloat16
    # The kernel's internal repack packs 8 codes / uint32, and its weight
    # layout math (`row_major(N // 64, K * 64 // pack_factor)`) requires
    # pack_factor == 8 -- matching the working skeleton test. (The "2 codes /
    # byte" NVFP4 figure is the storage density, not this API knob.)
    comptime pack_factor = 8
    comptime group_bytes = group_size // 2 + 2

    var N = Int(n.value())
    var K = Int(k.value())
    comptime _N = NType.static_value
    comptime _K = KType.static_value

    print("NVFP4 repack e2e: M=", M, " N=", N, " K=", K)

    var global_scale = Float32(0.5)

    # ---- canonical host tensors ----------------------------------------- #
    var weights_size = N * (K // 2)
    var scales_size = N * (K // group_size)
    # Combined kernel buffer: [weights N*K/2][bf16 scales (K/group)*N * 2 bytes].
    var combined_size = N * K // 2 + (K // group_size) * N * size_of[a_type]()

    var a_host = ctx.enqueue_create_host_buffer[a_type](M * K)
    var weights_host = ctx.enqueue_create_host_buffer[DType.uint8](weights_size)
    var scales_host = ctx.enqueue_create_host_buffer[DType.uint8](scales_size)

    rand(a_host.unsafe_ptr(), M * K)
    # Random packed E2M1 codes (every nibble pattern is a valid code).
    randint(weights_host.unsafe_ptr(), weights_size, 0, 255)
    # Random small-positive fp8-e4m3 block scales. Draw f32 in (0, 1] and cast
    # to fp8 to land on representable e4m3 values.
    var scales_f32 = ctx.enqueue_create_host_buffer[DType.float32](scales_size)
    rand(scales_f32.unsafe_ptr(), scales_size, min=0.125, max=1.0)
    var scales_fp8 = scales_host.unsafe_ptr().bitcast[
        Scalar[DType.float8_e4m3fn]
    ]()
    for i in range(scales_size):
        scales_fp8[i] = scales_f32.unsafe_ptr()[i].cast[DType.float8_e4m3fn]()

    # ---- device buffers -------------------------------------------------- #
    var a_dev = ctx.enqueue_create_buffer[a_type](M * K)
    var weights_dev = ctx.enqueue_create_buffer[DType.uint8](weights_size)
    var scales_dev = ctx.enqueue_create_buffer[DType.uint8](scales_size)
    var combined_dev = ctx.enqueue_create_buffer[DType.uint8](combined_size)
    var b_ref_dev = ctx.enqueue_create_buffer[a_type](N * K)
    var c_kernel_dev = ctx.enqueue_create_buffer[a_type](M * N)
    var c_ref_dev = ctx.enqueue_create_buffer[a_type](M * N)

    ctx.enqueue_copy(a_dev, a_host)
    ctx.enqueue_copy(weights_dev, weights_host)
    ctx.enqueue_copy(scales_dev, scales_host)

    # ---- canonical typed views for repack & reference -------------------- #
    comptime weights_layout = Layout.row_major(_N, _K // 2)
    comptime scales_layout = Layout.row_major(_N, _K // group_size)
    comptime combined_layout = Layout.row_major(1, _N * _K // 2)

    var weights_lt = LayoutTensor[DType.uint8, weights_layout, ImmutAnyOrigin](
        weights_dev.unsafe_ptr()
    )
    var scales_lt = LayoutTensor[DType.uint8, scales_layout, ImmutAnyOrigin](
        scales_dev.unsafe_ptr()
    )
    var scales_fp8_lt = LayoutTensor[
        DType.float8_e4m3fn, scales_layout, ImmutAnyOrigin
    ](scales_dev.unsafe_ptr().bitcast[Scalar[DType.float8_e4m3fn]]())
    var combined_lt = LayoutTensor[
        DType.uint8, combined_layout, MutAnyOrigin
    ](combined_dev.unsafe_ptr())

    # ---- (1) build the dense bf16 reference B ---------------------------- #
    comptime b_ref_layout = Layout.row_major(_N, _K)
    var b_ref_lt = LayoutTensor[a_type, b_ref_layout, MutAnyOrigin](
        b_ref_dev.unsafe_ptr()
    )
    comptime dequant = dequant_ref_kernel[
        weights_layout, scales_layout, b_ref_layout, group_size
    ]
    comptime threads = 256
    ctx.enqueue_function[dequant](
        weights_lt,
        scales_fp8_lt,
        global_scale,
        b_ref_lt,
        grid_dim=(ceildiv(N * K, threads), 1, 1),
        block_dim=(threads, 1, 1),
    )

    # ---- (2) repack canonical -> kernel buffer --------------------------- #
    comptime repack = repack_nvfp4_for_sm8x[
        weights_layout, scales_layout, combined_layout, group_size
    ]
    # Grid mirrors repack_GPTQ_for_sm8x: BN=128 over N, BK=1024 over K.
    # smem holds BN * 2 * (WGROUP // 2) uint8 = 128 * 2 * 64 weight bytes.
    var repack_smem = 128 * 2 * 64
    ctx.enqueue_function[repack](
        weights_lt,
        scales_lt,
        global_scale,
        combined_lt,
        grid_dim=(ceildiv(N, 128), ceildiv(K, 1024), 1),
        block_dim=(128, 1, 1),
        shared_mem_bytes=repack_smem,
        func_attribute=FuncAttribute.MAX_DYNAMIC_SHARED_SIZE_BYTES(
            UInt32(repack_smem)
        ),
    )

    # ---- (3) run the skeleton kernel ------------------------------------- #
    comptime b_kernel_layout = Layout.row_major(_N, (_K // group_size) * group_bytes)
    var b_kernel_lt = LayoutTensor[
        DType.uint8, b_kernel_layout, ImmutAnyOrigin
    ](
        combined_dev.unsafe_ptr(),
        RuntimeLayout[
            b_kernel_layout,
            element_type = LayoutTensor[
                DType.uint8, b_kernel_layout, ImmutAnyOrigin
            ].layout_int_type,
            linear_idx_type = LayoutTensor[
                DType.uint8, b_kernel_layout, ImmutAnyOrigin
            ].linear_idx_type,
        ].row_major(
            IndexList[2](N, (K // group_size) * group_bytes).cast[
                LayoutTensor[
                    DType.uint8, b_kernel_layout, ImmutAnyOrigin
                ].layout_int_type
            ]()
        ),
    )

    comptime config = MatmulConfig[a_type, DType.uint8, a_type, True](
        block_tile_shape=Index(128, 128, 32),
        warp_tile_shape=Index(64, 64, 32),
        num_pipeline_stages=4,
    )

    var a_tt = TileTensor(a_dev, row_major(Coord(M, Idx[_K])))
    var c_kernel_tt = TileTensor(c_kernel_dev, row_major(Coord(M, Idx[_N])))
    var a_lt = a_tt.to_layout_tensor()
    var c_kernel_lt = c_kernel_tt.to_layout_tensor()

    multistage_gemm_q[
        group_size=group_size,
        pack_factor=pack_factor,
        config=config,
        is_nvfp4=True,
    ](c_kernel_lt, a_lt, b_kernel_lt, config, ctx)

    # ---- (4) reference matmul C_ref = A @ B_dense^T ---------------------- #
    comptime kernels_ref = MatmulKernels[a_type, a_type, a_type, True]()
    comptime config_ref = kernels_ref.ampere_128x128_4
    var c_ref_tt = TileTensor(c_ref_dev, row_major(Coord(M, Idx[_N])))
    var b_ref_tt = TileTensor(b_ref_dev, row_major(Coord(Idx[_N], Idx[_K])))
    multistage_gemm[transpose_b=True, config=config_ref](
        c_ref_tt, a_tt, b_ref_tt, ctx
    )

    # ---- (5) compare ----------------------------------------------------- #
    var c_kernel_host = ctx.enqueue_create_host_buffer[a_type](M * N)
    var c_ref_host = ctx.enqueue_create_host_buffer[a_type](M * N)
    ctx.enqueue_copy(c_kernel_host, c_kernel_dev)
    ctx.enqueue_copy(c_ref_host, c_ref_dev)
    ctx.synchronize()

    assert_almost_equal(
        c_kernel_host.unsafe_ptr(),
        c_ref_host.unsafe_ptr(),
        M * N,
        atol=0.001,
        rtol=2e-2,
    )
    print("  REPACK e2e correctness OK: M=", M, " N=", N, " K=", K)

    _ = a_dev^
    _ = weights_dev^
    _ = scales_dev^
    _ = combined_dev^
    _ = b_ref_dev^
    _ = c_kernel_dev^
    _ = c_ref_dev^
    _ = scales_f32^


def bench_repack[
    NType: CoordLike,
    KType: CoordLike, //,
    group_size: Int = 16,
](ctx: DeviceContext, M: Int, n: NType, k: KType) raises:
    """Microbenchmark the skeleton kernel at gemma4-representative shapes.

    MANUAL WALL-CLOCK: warmup + nrun back-to-back enqueues + ONE synchronize.
    Deliberately avoids ctx.execution_time (it adds ~900us/call -> false cliffs).
    """
    comptime a_type = DType.bfloat16
    comptime pack_factor = 8
    comptime group_bytes = group_size // 2 + 2
    comptime _N = NType.static_value
    comptime _K = KType.static_value

    var N = Int(n.value())
    var K = Int(k.value())

    var b_size = N * ((K // group_size) * group_bytes)
    var a_dev = ctx.enqueue_create_buffer[a_type](M * K)
    var b_dev = ctx.enqueue_create_buffer[DType.uint8](b_size)
    var c_dev = ctx.enqueue_create_buffer[a_type](M * N)
    ctx.synchronize()

    comptime b_layout = Layout.row_major(_N, (_K // group_size) * group_bytes)
    var b_lt = LayoutTensor[DType.uint8, b_layout, ImmutAnyOrigin](
        b_dev.unsafe_ptr(),
        RuntimeLayout[
            b_layout,
            element_type = LayoutTensor[
                DType.uint8, b_layout, ImmutAnyOrigin
            ].layout_int_type,
            linear_idx_type = LayoutTensor[
                DType.uint8, b_layout, ImmutAnyOrigin
            ].linear_idx_type,
        ].row_major(
            IndexList[2](N, (K // group_size) * group_bytes).cast[
                LayoutTensor[
                    DType.uint8, b_layout, ImmutAnyOrigin
                ].layout_int_type
            ]()
        ),
    )
    var a_lt = TileTensor(
        a_dev, row_major(Coord(M, Idx[_K]))
    ).to_layout_tensor()
    var c_lt = TileTensor(
        c_dev, row_major(Coord(M, Idx[_N]))
    ).to_layout_tensor()

    comptime config = MatmulConfig[a_type, DType.uint8, a_type, True](
        block_tile_shape=Index(128, 128, 32),
        warp_tile_shape=Index(64, 64, 32),
        num_pipeline_stages=4,
    )

    comptime nrun = 200
    for _ in range(10):
        multistage_gemm_q[
            group_size=group_size,
            pack_factor=pack_factor,
            config=config,
            is_nvfp4=True,
        ](c_lt, a_lt, b_lt, config, ctx)
    ctx.synchronize()

    var t0 = perf_counter_ns()
    for _ in range(nrun):
        multistage_gemm_q[
            group_size=group_size,
            pack_factor=pack_factor,
            config=config,
            is_nvfp4=True,
        ](c_lt, a_lt, b_lt, config, ctx)
    ctx.synchronize()
    var t1 = perf_counter_ns()

    var nstime = Float64(t1 - t0) / Float64(nrun)
    var tflops = (
        2.0 * Float64(M) * Float64(N) * Float64(K) * 1e-12 / (nstime * 1e-9)
    )
    print(
        "  bench M=",
        M,
        " N=",
        N,
        " K=",
        K,
        ": ",
        tflops,
        " TFLOP/s",
    )
    _ = a_dev^
    _ = b_dev^
    _ = c_dev^


def main() raises:
    seed(0)
    with DeviceContext() as ctx:
        # Correctness: gemma4-shaped, both prefill-ish and decode-ish M.
        test_repack_e2e(ctx, Int(482), Idx[4096], Idx[4096])
        test_repack_e2e(ctx, Int(64), Idx[4096], Idx[4096])
        test_repack_e2e(ctx, Int(482), Idx[15360], Idx[3840])
        test_repack_e2e(ctx, Int(482), Idx[3840], Idx[15360])

        # Throughput table (skeleton kernel, manual wall-clock).
        print("NVFP4 skeleton TFLOP/s (group=16, BK=32, stages=4):")
        bench_repack(ctx, Int(482), Idx[4096], Idx[4096])
        bench_repack(ctx, Int(64), Idx[4096], Idx[4096])
        bench_repack(ctx, Int(482), Idx[15360], Idx[3840])
        bench_repack(ctx, Int(482), Idx[3840], Idx[15360])
        bench_repack(ctx, Int(64), Idx[15360], Idx[3840])
        bench_repack(ctx, Int(64), Idx[3840], Idx[15360])
