...
class RotaryEmbedding(Module[..., Tensor]):
    ...
    def freqs_cis_base(self) -> Tensor:
        ...
        Tensor is scaled with theta parameter. Required to apply Rotary
        Position Embedding (RoPE) to tensor. See 'Roformer: Enhanced
        Transformer with Rotary Embedding' (arxiv.org/pdf/2104.09864).
    ...
    def freqs_cis(self) -> Tensor:
        """Returns the flattened frequency tensor for rotary embeddings."""
    ...
    def compute_scale(self, user_scale: float | None = None) -> float:
        """Computes the attention scale factor from head_dim or the given user scale."""
        n = self.head_dim
