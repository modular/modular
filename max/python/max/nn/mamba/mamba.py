...
class MambaSSM(Module):
    ...
    def __init__(
        ...
        self._conv_state: Weight | None = (
            None  # (batch, intermediate_size, d_conv)
        )
        self._ssm_state: Weight | None = (
            None  # (batch, intermediate_size, d_state)
        )
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if use_fast_path:
            ...
            x_conv_permuted = ops.permute(
                x_conv, [0, 2, 1]
            )  # (batch, seqlen, intermediate_size)
            x_flat = ops.reshape(
                x_conv_permuted,
            ...
            x_proj = self.x_proj(
                x_flat
            )  # (batch * seqlen, dt_rank + 2 * d_state)
            ...
            dt_reshaped = ops.reshape(
                dt_proj, shape=[batch_size, seqlen, self.intermediate_size]
            )
            delta = ops.permute(
                dt_reshaped, [0, 2, 1]
            )  # (batch, intermediate_size, seqlen)
            ...
            B_reshaped = ops.reshape(
                B_temp, shape=[batch_size, 1, self.d_state, seqlen]
            )
            ...
            C_reshaped = ops.reshape(
                C_temp, shape=[batch_size, 1, self.d_state, seqlen]
            )
            ...
            delta_bias = ops.broadcast_to(
                delta_bias, shape=[self.intermediate_size]
            )  # All zeros
            ...
            ss_output_permuted = ops.permute(
                ss_output, [0, 2, 1]
            )  # (batch, seqlen, intermediate_size)
    ...
    def step(
        ...
        if len(hidden_states.shape.static_dims) == 3:
            ...
            hidden_states = ops.reshape(
                hidden_states, shape=[batch_size, self.hidden_size]
            )
        else:
            # (batch, hidden_size)
            batch_size = hidden_states.shape[0]
        ...
        if hasattr(self.dt_proj, "bias") and self.dt_proj.bias is not None:
            ...
            delta_bias = ops.constant(
                0.0, dtype=self.dtype, device=hidden_states.device
            )
            delta_bias = ops.broadcast_to(
                delta_bias, shape=[self.intermediate_size]
            )
        ...
        if not self.fused_add_norm:
            ...
            if self.norm is not None:
                if residual is not None:
                    ...
                    residual = (
                        hidden_states.rebind(
                            residual.shape,
                            message="Asserting hidden_states and residual have equivalent shapes for addition",
                        )
                        + residual
                    )
                else:
            ...
            if not self.fused_add_norm:
                ...
                if self.norm2 is not None:
                    ...
                    residual = (
                        hidden_states.rebind(
                            residual.shape,
                            message="Asserting hidden_states and residual have equivalent shapes for MLP residual addition",
                        )
                        + residual
                    )
                    # Cast to norm dtype for normalization
                    norm2_dtype = (
                        self.norm2.weight.dtype
                    ...
                    residual = (
                        hidden_states.rebind(
                            residual.shape,
                            message="Asserting hidden_states and residual have equivalent shapes for MLP residual addition (no norm2)",
                        )
                        + residual
                    )
            else:
                ...
                if self.norm2 is not None:
                    ...
                    residual = (
                        hidden_states.rebind(
                            residual.shape,
                            message="Asserting hidden_states and residual have equivalent shapes for MLP residual addition (fused, no norm2)",
                        )
                        + residual
                    )
