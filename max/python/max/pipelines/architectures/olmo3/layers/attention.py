...
class Olmo3Attention(Module[..., Tensor]):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        qkv = qkv.reshape((-1, self.n_heads, 3, self.kv_params.head_dim))
        ...
        if self.use_qk_norm:
            # TODO: these should be rmsnorms
            q = q / F.sqrt(
                F.mean(q * q, axis=-1, keepdim=True) + self.qk_norm_eps
            )
            k = k / F.sqrt(
                F.mean(k * k, axis=-1, keepdim=True) + self.qk_norm_eps
            )

        qk = F.concat([q, k], dim=-1)
