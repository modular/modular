...
class PixelGenerationTokenizer(
    ...
    async def new_context(
        ...
        extra_params["attention_mask"] = attn_mask.astype(np.bool_, copy=False)
        if attn_mask_2 is not None:
            extra_params["attention_mask_2"] = attn_mask_2.astype(
                np.bool_, copy=False
            )
        if negative_attn_mask is not None:
            extra_params["negative_attention_mask"] = negative_attn_mask.astype(
                np.bool_, copy=False
            )
