...
def test_mla_prefill_sparse_glm_bf16_64head_e2e() -> None:
    """E2E fresh prefill for the BF16-attention (GLM) sparse path at 64 heads.

    GLM 5.2 (NVFP4) runs BF16 attention, so it uses SparseLatentAttentionWithRope
    (not the FP8 class) with num_heads=64. graph_mode="auto" routes the prefill
    branch through the combined BF16 sparse op
    (mo.mla.graph.prefill.decode.paged.sparse) into mla_sm100_prefill_sparse at 64
    heads. prefill_len > MLA_DECODE_MAX_SEQ_LEN forces the prefill branch. Asserts
    numerics (finite, no NaN, correct shape).
    """
    device = Accelerator(0)
    session = InferenceSession(devices=[Accelerator()])
