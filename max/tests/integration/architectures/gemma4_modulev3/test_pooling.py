...
# ===----------------------------------------------------------------------=== #
"""Tests for Gemma4 ModuleV3 vision pooling.

``avg_pool_by_positions`` and ``compute_pool_gather_index`` are shared
(NumPy) helpers that live in the graph arch package; only the pooler module
under test is the ModuleV3 port.
"""
...
from max.pipelines.architectures.gemma4_modulev3.vision_model.pooling import (
    Gemma4VisionPooler,
)
...
# ---------------------------------------------------------------------------
# Gemma4VisionPooler (ModuleV3) vs torch reference forward
# ---------------------------------------------------------------------------
...
def _pool_inputs(
    ...
    """Build the ``pool_gather_index`` input from grids.

    Mirrors what ``pack_vision_buffers`` ships to the vision model: a
    per-output gather table ``[num_pooled, k**2]`` of patch indices.
    """
    return compute_pool_gather_index(positions_list, out_lens, k)


class TestGemma4VisionPooler:
    """Tests for the ModuleV3 Gemma4VisionPooler module (gather pooling)."""
    ...
    def _run(
        ...
        """Compile + execute the pooler on GPU, return bf16 output."""
        ...
        input_types = (
            TensorType(MAX_DTYPE, tuple(hidden.shape), device=DeviceRef.GPU()),
            TensorType(
                DType.int32,
                tuple(pool_gather_index.shape),
                device=DeviceRef.GPU(),
            ),
        )
        compiled = pooler.compile(*input_types, weights={})
        result = compiled.execute_raw(
            Buffer.from_dlpack(hidden).to(device),
            Buffer.from_numpy(pool_gather_index).to(device),
        )[0]
        assert isinstance(result, Buffer)
        return torch.from_dlpack(result).cpu().float().to(TORCH_DTYPE)
    ...
    def test_output_shape(self) -> None:
        ...
        out = self._run(h, gather_index, k)
    ...
    def test_values_match_torch_reference(self) -> None:
        ...
        out = self._run(x[0], gather_index, k)
