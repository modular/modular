...
# ===----------------------------------------------------------------------=== #
"""Tests for the ModuleV3 Gemma4VisionPatchEmbedder layer."""
...
from max.pipelines.architectures.gemma4_modulev3.vision_model.embedding import (
...
def _build_and_run(
    ...
    """Compile the ModuleV3 Gemma4VisionPatchEmbedder, execute, return output."""
    ...
    compiled = embedder.compile(*input_types, weights=weights)
    result = compiled.execute_raw(
        Buffer.from_dlpack(patches_flat).to(device),
        Buffer.from_dlpack(pixel_position_ids).to(device),
    )[0]
...
def test_patch_embedder_output_shape() -> None:
    ...
    result = _build_and_run(config, weights, patches_flat, pixel_position_ids)
...
def test_patch_embedder_pixel_normalisation() -> None:
    ...
    Uses a zero position-embedding table so the output depends only on the
    pixel values fed through the linear layer (which must have been
    normalised).  An all-zeros patches_flat input normalises to -1, so the
    output must equal ``(-1) @ W.T``; this checks that the bfloat16
    normalisation step occurs by comparing against the reference.
    """
    torch.manual_seed(7)
