...
# ===----------------------------------------------------------------------=== #
"""End-to-end GPT-2 greedy-decode test on a HAL-backed accelerator.

This builds GPT-2 small from primitive ``max.experimental.nn`` layers
(following the "Build an LLM from scratch with MAX" guide), loads the real
HuggingFace ``openai-community/gpt2`` safetensors checkpoint directly, and
greedy-decodes a short prompt. Weights are adapted at compile time
...
The test targets the HAL ``DeviceContext`` (see
``oss/modular/mojo/stdlib/std/gpu/host/_device_context_hal.mojo``) rather than
the default device context. Selecting the HAL path requires no code change:
set ``MODULAR_DRIVER_PLUGINS`` to a HAL driver plugin and the standard
``max.driver.Accelerator()`` is backed by it. For example::

    MODULAR_DRIVER_PLUGINS=CUDA@/path/to/CUDAPlugin.so   # NVIDIA / CUDA
    MODULAR_DRIVER_PLUGINS=HIP@/path/to/HIPPlugin.so     # AMD / HIP
    MODULAR_DRIVER_PLUGINS=Metal@/path/to/MetalPlugin.so # Apple / Metal

Correctness is checked against a CPU run of the identical MAX model and
weights: the HAL device must produce the same greedy token sequence as the
...
# per-step argmax stable enough that the host and HAL device agree token for
...
def _load_gpt2_numpy_state() -> dict[str, np.ndarray]:
    ...
    and the HAL device never shares a host buffer between the two graphs.
    """
...
# on both CPU and the HAL accelerator from identical weights and the outputs
# are compared element-wise. Unlike the guide's eager ``Module.to(...)``
# comparison, every forward pass here is COMPILED to a graph and executed,
# exercising the same HAL ``DeviceContext`` graph-execution path as the
# end-to-end decode below.
# ---------------------------------------------------------------------------
...
def _probe_device(dev: Device) -> None:
    ...
    For the HAL path this proves the driver plugin actually loaded and runs,
    surfacing an unusable device as an immediate failure at import rather than
    a confusing mid-suite crash.
    """
...
def _resolve_device() -> Accelerator:
    """Resolve the HAL accelerator, failing hard if one is unavailable.

    This test is gated to GPU machines and runs with MODULAR_DRIVER_PLUGINS
    pointing at a HAL driver plugin, so a missing or unusable accelerator is a
    broken environment, not a reason to skip. Skipping would silently mask a
    regression in the HAL device-context path the test exists to exercise, so
    raise instead: the whole module then fails rather than reporting green.
    """
    if not os.environ.get("MODULAR_DRIVER_PLUGINS"):
        raise RuntimeError(
            "MODULAR_DRIVER_PLUGINS is not set; the HAL GPT-2 test needs a HAL "
            "driver plugin (e.g. CUDA@/path/to/CUDAPlugin.so)."
        )
    if accelerator_count() == 0:
        raise RuntimeError(
            "No accelerator detected; test_gpt2_hal requires a GPU."
        )
    device = Accelerator()
...
def _forward_cpu_vs_device(
    ...
    """Compile+run ``build()`` on CPU and the HAL device from identical weights.
...
class TestCausalMask:
    """causal_mask shape and values on the HAL device."""
...
def test_gpt2_hal_matches_cpu_reference() -> None:
    """GPT-2 greedy-decodes identically on a HAL-backed accelerator and CPU.

    Selecting the HAL device context requires only ``MODULAR_DRIVER_PLUGINS``
    to point at a HAL driver plugin (CUDA/HIP/Metal); the model code is the
    same as for the default device context. The HAL device's greedy token
    sequence must match a CPU run of the identical model and weights.
    """
    ...
    hal_tokens = _greedy_decode(
        _GPT2_SMALL, numpy_state, _device(), _PROMPT_TOKENS, _NUM_GENERATE
    )

    print(f"[gpt2_hal] cpu: {cpu_tokens}")
    print(f"[gpt2_hal] hal: {hal_tokens}")
    ...
    assert hal_tokens == cpu_tokens, (
        "HAL device greedy decode does not match the CPU reference.\n"
        f"  cpu: {cpu_tokens}\n"
        f"  hal: {hal_tokens}"
    )
