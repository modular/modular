...
Mojo syntax is covered in detail in the [Mojo manual](/docs/manual/). Here's a
...
- Slash (`/`): arguments declared before a slash are
  [positional-only arguments](/docs/manual/functions#positional-only-and-keyword-only-arguments).

- Star (`*`): a star by itself in place of an argument indicates that the
  arguments after the star are
  [keyword-only](/docs/manual/functions#positional-only-and-keyword-only-arguments).

- An equals sign (`=`) introduces a default value for an
  [optional argument](/docs/manual/functions#optional-arguments).
...
- An argument name prefixed by a single star character, like `*names` identifies
  a [variadic argument](/docs/manual/functions/#variadic-arguments).

- An argument name prefixed with a double star, like `**attributes` identifies a
  [variadic keyword-only argument](/docs/manual/functions/#variadic-keyword-arguments).
...
[infer-only parameters](/docs/manual/parameters/#infer-only-parameters). You
...
[Introduction to layouts](/docs/manual/layout/layouts) and [Using
...
kernels](/docs/manual/gpu/basics) that are portable across CPUs and GPUs.
