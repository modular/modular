...
struct SlidingWindow:
    ...
    fn __init__(out self, capacity: Int):
        self.slots = List[Int](length=capacity, fill=0)
...
This demonstrates the sliding behavior. The window holds 3 values because `SlidingWindow` is created with `capacity=3`, and the `__init__` method uses that `capacity` to create three slots via the `for _ in range(capacity)` loop. When you push the fourth value (4), the first value (1) is evicted.
...
Here, `dropped` is an `Optional`; calling `.take()` extracts its contained value while consuming the `Optional`, and that extracted value is then appended to the `evicted` list.
...
- Extracting with `.value()` gets the inner value; `.take()` transfers ownership
