...
All of the collection types in the [`collections`](/mojo/std/collections)
...
[`Dict`](/mojo/std/collections/dict/Dict). The first is to iterate directly
...
[`items()`](/mojo/std/collections/dict/Dict#items) method, which produces a
sequence of [`DictEntry`](/mojo/std/collections/dict/DictEntry) objects.
...
[`range()`](/mojo/std/builtin/range/range) function. It differs from the
