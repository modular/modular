...
Traits are what make generic types practical. Those constraints describe
...
operations, so it applies a trait composition constraint on `T`:

```mojo
comptime ComparableValue = Equatable & ImplicitlyCopyable
```

The `Pair` definition applies this constraint in two places: to the type
...
Sometimes a generic constraint is broader than what you need. You may want
operations from a specific trait that the constraint doesn't guarantee. Use
