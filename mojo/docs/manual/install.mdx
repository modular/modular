...
tutorial](/docs/manual/get-started), which includes installation steps.

:::note System requirements

You can use Mac, Linux, or Windows with WSL. For details, see the
[system requirements](/docs/requirements). For GPU programming, see
[GPU compatibility](/docs/requirements#gpu-compatibility).
...
2. Run it with the [`mojo`](/docs/cli) CLI:
...
Use Mojo agent skills with your AI tools to generate more accurate Mojo code
and learn current syntax and patterns.
...
These skills help AI tools produce code that reflects current syntax,
APIs, and best practices.

Two skills are especially useful when you're getting started:

- *Project setup* (`new-modular-project`): creates and structures new Mojo
  projects. Just describe what you want to build, and the skill handles the
  basic setup.
- *Mojo syntax* (`mojo-syntax`): keeps generated code aligned with the
  current language. The syntax skill keeps your code accurate and up-to-date
  as the language evolves, so your AI tool isn't limited by when the model
  was trained.
...
2. Navigate to one of the Mojo example subdirectories, such as the subdirectory
   ...
   [Get started with Mojo](/docs/manual/get-started) tutorial project, which is
...
3. Use `pixi` to download and install the dependencies for the project and to
   ...
   example with the [`mojo`](/docs/cli) CLI. For example, to run the reference
...
Relocated to the [system requirements](/docs/requirements) page.
