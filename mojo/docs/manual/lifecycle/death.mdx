...
For example, notice when the `__deinit__()` destructor is called for each
instance of `Balloon`:
...
when its lifetime ends. This makes it easier to write type-generic containers
and algorithms.
...
opts out of `Deinitable` destruction. The deletable trait provides
a default no-op destructor (`__deinit__()`), which you can override.

Create a custom `__deinit__()` implementation to perform any required cleanup.
This might include freeing dynamically allocated memory (for example,
through `dealloc()`) or releasing long-lived resources such as file
handles.
...
There's no need to define the `__deinit__()` destructor for this, because it's
a simple collection of other types (`String` and `Int`), and it doesn't
...
a pointer is destroyed, Mojo doesn't call the deinitializers on those values.
Likewise, an allocation is a handle to memory, but it doesn't own the values
stored in that memory.

So in the `HeapArray` example above, calling `dealloc()` releases the memory,
but doesn't call the deinitializers on the stored values. That's why the
destructor loops over the elements first, invoking the `unsafe_deinit_pointee()`
method provided by the `Pointer` type, and only then deallocates.
...
However, the `self` value inside the `__deinit__()` destructor is still whole
(so all fields are still usable) until the destructor returns, as we'll
...
<!-- TODO(DOCS-2253): Reflect changes to `@explicit_destroy` -->

Most types use implicit destruction. When a value's lifetime ends, Mojo
automatically calls `__deinit__()` to clean up. This works for most cases:
memory is freed and simple resources are released automatically.
...
Unlike the `__deinit__()` method, named explicit destructors can raise errors.
...
Code parameterized over `AnyType` can accept both `Deinitable`
and explicitly destructible values. However, it cannot automatically destroy
values that require explicit destruction.

Because explicit destructors are specific to their types, type-generic code
...
no way to invoke type-specific explicit destructors unless that behavior is
...
- Require implicit destruction with the Deinitable trait, which
  guarantees that a `__deinit__()` destructor is available and can be called
  automatically:
...
Both the consuming move initializer and the destructor take their operand
