...
Whereas, the following struct must define the `__deinit__()` method to free
the memory allocated by its
...
So in the `HeapArray` example above, calling `unsafe_free()` on the pointer
releases the memory, but doesn't call the destructors on the stored values. To
invoke the destructors, use the `unsafe_deinit_pointee()` method provided by
...
Code parameterized over `AnyType` can accept both implicitly deletable
...
def parameterized_function_2[T: ImplicitlyDeletable](var value: T):
...
- Require implicit destruction with the ImplicitlyDeletable trait, which
  ...
  ```mojo
  def parameterized_function_3[T: ImplicitlyDeletable](var value: T):
      pass  # value.__deinit__() called automatically
  ```
