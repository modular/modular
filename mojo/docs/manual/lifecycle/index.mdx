...
as the initializer, copy initializer, move initializer, and destructor, as
necessary. Mojo doesn't create any initializers by default, although it does
...
- The "lifecycle" of a value is defined by various [dunder
  ...
  such as the initializer (`__init__()`), the destructor (`__del__()`), the copy
  initializer (`__init__(copy=)`), and the move initializer (`__init__(take=)`).
