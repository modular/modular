...
## Mojo's default deinitializer

`__deinit__()` is Mojo's default _deinitializer_. While it
...
struct Contact:
    ...
    def __deinit__(deinit self):
        ...
        # `self` remains initialized until this method completes
...
## Deinitializers and moving fields
...
A deinitializer's secret sauce is the `deinit` convention. Because of this,
`deinit self` works in named deinitializers as well as `__deinit__()`:

```mojo
struct Parent:
    def consume(deinit self):
        # `consume` is a named deinitializer
        # Can move values out of fields here too
```

When you use the `deinit self` convention in a method, the compiler
recognizes the method as a deinitializer and lets you move a field without
reinitializing it:
...
Any struct method that uses `deinit self` as its first argument is technically
a deinitializer. It won't participate in explicit destruction unless the
type conforms to `Deinitable where False`.

## Explicit and implicit deinitializers

The `__deinit__()` method is the core of implicit deinitialization types.
It's Mojo's default deinitializer and works hand in hand with `Deinitable`,
a trait that all structs conform to by default. The compiler calls
`__deinit__()` automatically when it detects the final use of a
`Deinitable` struct value.

As the name suggests, explicit destruction requires you to call a
deinitializer. An explicitly destroyed type must define at least one
custom deinitializer, such as the `consume()` method shown in the
previous section. The compiler makes sure that the final use of an
...
Unless you opt into [explicit
destruction](/docs/manual/lifecycle/death/#explicitly-destroyed-types)
by conforming to `Deinitable where False`, Mojo uses implicit
deinitialization.
...
With ASAP destruction, the default implicit approach, Mojo destroys
instances immediately after their last use, not at the end of scope. This
can happen even in the middle of an expression.
...
better tail call optimization, but it also means deinitializers can run
earlier than you might expect.

Mojo also supports _explicitly-destroyed types_, which require you to make an
explicit call to a deinitializer method. For more information, see
...
deinitializer (`__deinit__(deinit self...)`) to destroy it.

If a type doesn't have an implicit deinitializer because it requires
explicit destruction, the compiler emits an error.

While the compiler checks that a deinitializer is called before an
instance goes out of scope, you're responsible for choosing _when_ that
happens. The key differences are:

- Deinitialization doesn't automatically happen at the point of last use.
- The explicit deinitializer isn't named `__deinit__()`.
...
For implicitly destroyed structs, the compiler inserts a deinitializer
call after the instance's last use. For explicitly destroyed values,
you must call a deinitializer before the instance goes out of scope.
Deinitialization is compiler-checked, but you choose when it happens.
The value's final use in scope must be that explicit deinitializer call.

Field operations are also constrained. Fieldwise initialization must
happen inside the `__init__()` initializer. You can move values out of a
...
initialization, use, and deinitialization remain safe and explicit.
