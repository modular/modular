...
[`@fieldwise_init`](/docs/manual/decorators/fieldwise-init) decorator to
...
[`UnsafePointer`](/docs/std/memory/unsafe_pointer/UnsafePointer)). We'll discuss
that more in [Death of a value](/docs/manual/lifecycle/death).
...
[argument convention](/docs/manual/values/ownership#argument-conventions), which
...
[`Atomic`](/docs/std/os/atomic/Atomic/) type is an example of a type that's
