...
[`Bool`](/docs/std/builtin/bool/Bool/), [`Int`](/docs/std/builtin/simd/#int),
...
For example, the `Optional[T]` type models a value that may or may not be
present. You can implicitly convert a value of type `T` into an `Optional[T]`,
and you can implicitly convert a value of `None` into an empty `Optional[T]`:
...
Implicit conversion is intended to reduce unnecessary boilerplate, not to
weaken type safety.

#### Enabling implicit conversion

Implicit conversion is supported only when the target type defines
...
struct HeapArray(Copyable):
    ...
    def append(mut self, val: Int):
        ...
        if self.size < self.cap:
            (self.data + self.size).unsafe_write(val)
            self.size += 1
        else:
            print("Out of bounds")
