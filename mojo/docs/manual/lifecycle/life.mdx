...
sidebar_position: 2
description: Learn how and when Mojo creates values.
---
...
For many types, lifecycle operations are *trivial*: they have no custom
behavior, so the compiler can optimize or eliminate them.
...
- `IsTriviallyCopyable[T]` is true when `T` is `Copyable` and copying a
  value's bits has no side effects.
- `IsTriviallyMovable[T]` is true when `T` is `Movable` and moving a
  value's bits has no side effects.
