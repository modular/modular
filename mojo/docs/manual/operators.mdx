...
github_url: https://github.com/modular/modular/tree/modular/v26.2/mojo/examples/operators
---
...
| `a if condition else b`| Conditional expression | Right to left |
...
The [`Int`](/mojo/std/builtin/int/Int) and
[`UInt`](/mojo/std/builtin/uint/UInt) types represent signed and unsigned
...
and the other `UInt` the result is an `Int`.
...
The Mojo standard library defines the [`SIMD`](/mojo/std/builtin/simd/SIMD)
...
`SIMD` values of a numeric [`DType`](/mojo/std/builtin/dtype/DType) support
...
[`Scalar`](/mojo/std/builtin/simd/) values are simply aliases for
...
[`FloatLiteral`](/mojo/std/builtin/float_literal/FloatLiteral) are
...
#### Operating on non-boolean values with boolean-returning operators

Many of Mojo's word-based operators (`in`, `not in`, `is`, `is not`,
`not`, `and`, `or`) return booleans without their arguments being
boolean themselves.

For example, `in` and `not in` are used with values and collections:

```mojo
var list = [1, 2, 3]
print(1 in list)  # True, `in` operator checks for membership in the list
print(4 in list)  # False, `in` operator
print(4 not in list)  # True, `not in` operator
```

Some types have _identities_ that can be tested with `is` and `not is`.
Pointers can use their unique addresses to distinguish one allocation
from another:

```mojo
# Identity, Wrapped pointers
...
var p1 = MemoryHolder(); var p2 = MemoryHolder()
print(p1 is p2)  # False, two pointer allocations
print(p1 is not p2)  # True, two pointer allocations
print(p1 is p1)  # True, same pointer allocation
```

`Boolable` types adopt a kind of _truthiness_. You
can use them directly in `if`-statements or convert
them to `Bool`.

```mojo
var s1 = "Hello"; var s2 = "" # Empty strings are Falsy
print(Bool(not s1))  # False, non-empty string is Truthy
print(Bool(not s2))  # True, empty string is Falsy
print(Bool(s1 and s2)) # False
print(Bool(s1 and s1)) # True
print(Bool(s1 or s2)) # True
print(Bool(s2 or s2)) # False
```

Integers are `Boolable`, so you can use them with
`and` and `or`. In this example, `0 or 1` provides
a boolean check on whether a function can compile.
0 is falsy and 1 is truthy:

```
# Change the constraint to 0 to activate the error
def test() where 0 or 1
    print("This should be compiled, the condition is true")
test()  # This should be compiled, the condition is true
```
...
individually and work with the resulting boolean mask for further processing.

### String operators

As discussed in [Strings](/mojo/manual/types#strings), the
[`String`](/mojo/std/collections/string/string/String) type represents a
mutable string value. In contrast, the
[`StringLiteral`](/mojo/std/builtin/string_literal/StringLiteral) type
...
a single byte. Byte positions are identified with a zero-based index
starting from the first byte. You can also specify a negative index to
...
[register-passable types](/mojo/manual/traits#register-passable), the
compiled results might be equivalent at run-time. But for a memory-only
type, the first example allocates storage for the result of `a + b` and
then assigns the value to the variable, whereas the second example can
do an in-place modification of the existing value.
...
In Mojo, as in Python, a conditional expression enables you to select
a value from one of two options:

```mojo title="Mojo"
result = "pass" if score > 65 else "fail"
```

This pattern can be used in expressions, assignments, and as
function arguments:

```mojo
exp = 5 # or -5
result = 2.0 ** Float64(exp)
    if exp >= 0 \
    else 1.0 / (2.0 ** abs(Float64(exp))) \
    # For 5, 32, for -5, 0.03125

greet("Sami" if is_sami else "Cass")
```
...
proposal,
[Customizable Type Merging in Mojo](https://github.com/modular/modular/blob/modular/v26.2/mojo/proposals/custom-type-merging.md).
...
implements the [`Comparable`](/mojo/std/builtin/comparable/Comparable) trait
...
[`Equatable`](/mojo/std/builtin/equality_comparable/Equatable)
...
When a type conforms to [`Identifiable`](/mojo/std/builtin/identifiable/Identifiable),
these operators call the [`__is__()`](/mojo/std/builtin/identifiable/Identifiable/#__is__)
method to determine whether the left-hand operand has the same identity as the right-hand operand. For example,
you might compare raw values or use some other way to establish what "identity" means in your work.

The `is not` operator uses the automatically provided
[`__isnot__()`](/mojo/std/builtin/identifiable/Identifiable/#__isnot__) method, which
returns the logical negation of `__is__()`.
...
struct MySeq[type: Copyable]:
    def __getitem__(self, idx: Int) -> type:
        ...
        ...
    def __setitem__(mut self, idx: Int, value: type):
...
struct MySeq[type: Copyable]:
    # 2-dimension support
    def __getitem__(self, x_idx: Int, y_idx: Int) -> type:
        ...
    # Arbitrary-dimension support
    def __getitem__(self, *indices: Int) -> type:
        ...
```
...
[`Slice`](/mojo/std/builtin/builtin_slice/Slice) (or multiple `Slice`s in
...
repo](https://github.com/modular/modular/tree/modular/v26.2/mojo/examples/operators).

:::note

Note that the Mojo standard library implements a parameterized
[`ComplexSIMD`](/mojo/std/complex/complex/ComplexSIMD) struct that provides
...
The `Complex` struct is a simple value type with register passable
numeric fields. It requires no special constructor or destructor
behaviors. Conforming it to
[RegisterPassable](/mojo/manual/traits#register-passable) and
adopting `Copyable` lets you assign the value:

```mojo
@fieldwise_init
struct Complex(RegisterPassable):
    var re: Float64
    var im: Float64
```

This definition allows you to construct and use an instance without
building an initializer:

```mojo
var c1 = Complex(-1.2, 6.5)
print(t"c1: Real: {c1.re}; Imaginary: {c1.im}") # c1: Real: -1.2; Imaginary: 6.5
```

To allow you to construct a `Complex` instance with just the real
component, add a convenience initializer:
...
Now you can create a `Complex` instance with just a real component:

```mojo
var c2 = Complex(-1.2)
print(t"c2: Real: {c2.re}; Imaginary: {c2.im}") # c2: Real: -1.2; Imaginary: 0.0
```

### Implement the `Writable` trait

To make it simpler to print `Complex` values, let's implement the
[Writable](/mojo/std/format/Writable) trait so that we can use the
`String()` constructor to generate a `String` representation of a
`Complex` value. You can find out more about these traits and their associated
methods in [The `Writable` trait](/mojo/manual/traits#the-stringable-representable-and-writable-traits).
...
```mojo
    ...
    def __setitem__(mut self, idx: Int, value: Float64) raises -> None:
        if idx == 0:
            self.re = value
        elif idx == 1:
            self.im = value
...
# ...
    ...
    def squared_norm(self) raises -> Float64:
        return self.re * self.re + self.im * self.im

    def norm(self) raises -> Float64:
...
```mojo
    # ...
    def __add__(self, rhs: Float64) raises -> Self:
        return Self(self.re + rhs, self.im)

    def __radd__(self, lhs: Float64) raises -> Self:
        return Self(self.re + lhs, self.im)

    def __sub__(self, rhs: Float64) raises -> Self:
        return Self(self.re - rhs, self.im)

    def __rsub__(self, lhs: Float64) raises -> Self:
        return Self(lhs - self.re, -self.im)

    def __mul__(self, rhs: Float64) raises -> Self:
        return Self(self.re * rhs, self.im * rhs)

    def __rmul__(self, lhs: Float64) raises -> Self:
        return Self(lhs * self.re, lhs * self.im)

    def __truediv__(self, rhs: Float64) raises -> Self:
        return Self(self.re / rhs, self.im / rhs)

    def __rtruediv__(self, lhs: Float64) raises -> Self:
...
```mojo
    # ...
    def __iadd__(mut self, rhs: Self) raises -> None:
        self.re += rhs.re
        self.im += rhs.im

    def __iadd__(mut self, rhs: Float64) raises -> None:
        self.re += rhs

    def __isub__(mut self, rhs: Self) raises -> None:
        self.re -= rhs.re
        self.im -= rhs.im

    def __isub__(mut self, rhs: Float64) raises -> None:
        self.re -= rhs

    def __imul__(mut self, rhs: Self) raises -> None:
        new_re = self.re * rhs.re - self.im * rhs.im
        new_im = self.re * rhs.im + self.im * rhs.re
        self.re = new_re
        self.im = new_im

    def __imul__(mut self, rhs: Float64) raises -> None:
        self.re *= rhs
        self.im *= rhs

    def __itruediv__(mut self, rhs: Self) raises -> None:
        denom = rhs.squared_norm()
    ...
    def __itruediv__(mut self, rhs: Float64) raises -> None:
        self.re /= rhs
        self.im /= rhs
```
