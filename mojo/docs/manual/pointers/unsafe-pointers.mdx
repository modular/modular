...
An `UnsafePointer` is a type that holds an address to memory. You can store and
retrieve values in that memory. The `UnsafePointer` type is *parameterized*—it
...
access the values, as if they were an array, like `ptr[unsafe_offset=3]`. The
empty subscript `[]` has the same meaning as `[unsafe_offset=0]`.
...
The
[`unsafe_take_pointee()`](/docs/std/memory/unsafe_pointer/UnsafePointer/#unsafe_take_pointee)
...
Calling
[`unsafe_write_move_from(self, src)`](/docs/std/memory/unsafe_pointer/UnsafePointer/#unsafe_write_move_from)
...
Calling
[`unsafe_free()`](/docs/std/memory/unsafe_pointer/UnsafePointer/#unsafe_free) on
a pointer frees the memory allocated by the pointer. It doesn't call the
...
memory, you need to make sure you only call `unsafe_free()` on one of them.
Freeing the same memory twice is also an error.
...
using the `unsafe_bitcast()` method. Similarly, you can create an opaque
pointer from an existing pointer by bitcasting to `NoneType`. For example:
...
[`unsafe_load()`](/docs/std/memory/unsafe_pointer/UnsafePointer/#unsafe_load)
and
[`unsafe_store()`](/docs/std/memory/unsafe_pointer/UnsafePointer/#unsafe_store)
...
[`unsafe_strided_load()`](/docs/std/memory/unsafe_pointer/UnsafePointer/#unsafe_strided_load)
and
[`unsafe_strided_store()`](/docs/std/memory/unsafe_pointer/UnsafePointer/#unsafe_strided_store)
...
The
[`unsafe_gather()`](/docs/std/memory/unsafe_pointer/UnsafePointer/#unsafe_gather)
and
[`unsafe_scatter()`](/docs/std/memory/unsafe_pointer/UnsafePointer/#unsafe_scatter)
