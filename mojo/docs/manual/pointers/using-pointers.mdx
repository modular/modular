...
- Pointing to initialized memory. You can initialize an allocated, uninitialized
  ...
  construct a pointer to an existing value by calling the constructor with the
  `to` keyword argument.
...
[implicitly copyable](/docs/std/builtin/value/ImplicitlyCopyable/) type (like
...
and first_ptr + 2 points to the
third.](../images/pointers/pointer-offset.png#light)
![Four consecutive memory locations at addresses 0x..101 through 0x..104,
holding the characters M, o, j, and o. first_ptr points to the first location,
and first_ptr + 2 points to the
third.](../images/pointers/pointer-offset-dark.png#dark)
...
have to worry about the origin, as long as the pointer isn't exposed outside
