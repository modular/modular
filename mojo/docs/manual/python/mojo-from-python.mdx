...
[`PythonModuleBuilder`](/mojo/std/python/bindings/PythonModuleBuilder).
...
behind the scenes looks for a `.mojo` file that matches the imported
module name, and if found, compiles it using
[`mojo build --emit shared-lib`](/mojo/cli/build#--emit-file_type) to generate a
dynamic library. The resulting file is stored in `__mojocache__`, and is rebuilt
only when it becomes stale (typically, when the Mojo source file changes).
...
When you call
[`add_type()`](/mojo/std/python/bindings/PythonModuleBuilder/#add_type), it
returns a
[`PythonTypeBuilder`](/mojo/std/python/bindings/PythonTypeBuilder), which
...
[`Representable`](/mojo/std/builtin/repr/Representable/) to be bound for use
...
[`def_py_init()`](/mojo/std/python/bindings/PythonTypeBuilder#def_py_init) when
...
[`def_init_defaultable()`](/mojo/std/python/bindings/PythonTypeBuilder#def_init_defaultable)
method:
...
[`PythonObject`](/mojo/std/python/python_object/PythonObject) values as
...
[`PythonObject`](/mojo/std/python/python_object/PythonObject), but
...
Python as well, using [`PythonTypeBuilder.def_method()`](/mojo/std/python/bindings/PythonTypeBuilder/#def_method).
...
Python Mojo bindings supports exposing Python `@staticmethods`, bound using [`PythonTypeBuilder.def_staticmethod()`](/mojo/std/python/bindings/PythonTypeBuilder/#def_staticmethod). A function declared using `def_staticmethod()` is callable as a static method on the type within Python, without needing an object instance.
...
2. [Variadic keyword arguments](/manual/functions/#variadic-keyword-arguments): `def foo(**kwargs: Int)`
    This is supported in Python Mojo bindings when used in the unsugared form: `def foo(kwargs: OwnedKwargsDict)`. (The `**kwargs` syntax limitation will be removed in the future.)

You can define Mojo functions that accept variadic keyword arguments using [`OwnedKwargsDict[PythonObject]`](/mojo/std/collections/dict/OwnedKwargsDict) as the last argument. A simple example looks like:
...
[`def_py_function()`](/mojo/std/python/bindings/PythonModuleBuilder#def_py_function)
...
- **Functions taking more than 6 arguments.**
  ...
  arguments: `def(PythonObject, PythonObject, PythonObject, PythonObject, PythonObject, PythonObject)`.
...
[downcast_value_ptr]: /mojo/std/python/python_object/PythonObject#downcast_value_ptr
