...
A [`TileTensor`](/mojo/layout/tile_tensor/TileTensor/)
...
- A [layout](/mojo/manual/tile-tensor/layouts), defining how the elements are
  laid out in memory.
- A [`DType`](/mojo/std/builtin/dtype/DType/), defining the data type stored
  in the tensor.
...
[`LayoutTensor`](/mojo/layout/layout_tensor/LayoutTensor/). `TileTensor`
...
[`to_layout_tensor()`](/mojo/layout/tile_tensor/TileTensor/#to_layout_tensor).
...
coordinates as a single [`Coord`](/mojo/layout/coord/Coord/) value. For an
...
section on [comptime constraints](/mojo/manual/metaprogramming/constraints/).
...
[`load()`](/mojo/layout/tile_tensor/TileTensor/#load) and
[`store()`](/mojo/layout/tile_tensor/TileTensor/#store) methods, which
...
[TileTensor layouts](/mojo/manual/tile-tensor/layouts#modes). These layouts have
...
[`Layout`](/mojo/layout/tile_layout/Layout/) and a block
of memory to store the tensor data. A common way to allocate memory for a
`TileTensor` is to use an
[`InlineArray`](/mojo/std/collections/inline_array/InlineArray/) or a `List`:
...
[`Span`](/mojo/std/memory/span/Span/), a type that represents a contiguous block
...
or
[`enqueue_memset()`](/mojo/std/gpu/host/device_context/DeviceContext#enqueue_memset)
...
[`stack_allocation()`](/mojo/layout/tile_tensor/stack_allocation/)
...
The [`TileTensor` layouts](/mojo/manual/tile-tensor/layouts/) page describes
...
([`zipped_divide()`](/mojo/layout/tile_layout/zipped_divide/)).
...
The
[`TileTensor.tile()`](/mojo/layout/tile_tensor/TileTensor/#tile)
...
The [`vectorize()`](/mojo/layout/tile_tensor/TileTensor/#vectorize)
...
[`distribute()`](/mojo/layout/tile_tensor/TileTensor/#distribute)
...
[`lane_id()`](/mojo/std/gpu/primitives/id/lane_id/) as the thread ID. Lane ID
...
The [`tile_io` package](/mojo/layout/tile_io/) includes a `TileCopier` trait and
...
- [`GenericToSharedTileCopier`](/mojo/layout/tile_io/GenericToSharedTileCopier/)
- [`SharedToGenericTileCopier`](/mojo/layout/tile_io/SharedToGenericTileCopier/)
- [`GenericToLocalTileCopier`](/mojo/layout/tile_io/GenericToLocalTileCopier/)
- [`LocalToGenericTileCopier`](/mojo/layout/tile_io/LocalToGenericTileCopier/)
- [`SharedToLocalTileCopier`](/mojo/layout/tile_io/SharedToLocalTileCopier/)
- [`LocalToSharedTileCopier`](/mojo/layout/tile_io/LocalToSharedTileCopier/)

In addition to the synchronous tile copiers, the `tile_io` module currently
includes one *asynchronous* tile copier:

- [`GenericToSharedAsyncTileCopier`](/mojo/layout/tile_io/GenericToSharedAsyncTileCopier/)
