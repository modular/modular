...
[`Array`](/docs/std/collections/array/Array/) or a `List`:
...
[`row_major_nested()`](/docs/layout/tile_layout/row_major_nested/) and
[`col_major_nested()`](/docs/layout/tile_layout/col_major_nested/).
...
The [`split()`](/docs/layout/tile_tensor/TileTensor/#split) method divides a
tensor into non-overlapping partitions along an axis. Unlike `tile()`, which
returns a single sub-tile, `split()` returns every partition at once (or a
single requested partition). Split views are immutable, so call
[`as_immut()`](/docs/layout/tile_tensor/TileTensor/#as_immut) on a mutable
tensor first.
...
[`copy_from()`](/docs/layout/tile_tensor/TileTensor/#copy_from) method for
copying tensor data. In addition, the
[`tile_io`](/docs/layout/tile_io/) module provides a set of
utilities specialized for copying between various GPU memory spaces. All of the
tensor copy methods respect the layouts—so you can transform a tensor by copying
it to a tensor with a different layout (provided both layouts have the same
