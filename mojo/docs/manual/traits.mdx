...
most often in collections and parameterized data structures.
...
Use required and provided `comptime` members to describe parameterized
families of types.

For more information, see [`comptime` members for
parameterized types](#comptime-members-for-generics).
...
- They use a special trivial (no-op) destructor. They are *trivially
  movable* (via `Copyable`, which refines `Movable`), *trivially
...
## Parameterized structs with traits {#generic-structs-with-traits}

You can also use traits when defining a parameterized container. A parameterized
container is a container (for example, an array or hashmap) that can hold
different data types. In a dynamic language like Python it's easy to add
different types of items to a container. But in a statically-typed environment,
the compiler needs to be able to identify the types at compile time. For
example, if the container needs to copy a value, the compiler needs to verify
that the type can be copied.

The [`List`](/docs/std/collections/list/) type is an example of a
parameterized container. A single `List` can only hold a single type of data.
...
Building parameterized containers is an advanced topic. For an introduction, see
the section on
[parameterized structs](/docs/manual/parameters/#parameterized-structs).

### `comptime` members for parameterized types {#comptime-members-for-generics}
...
`comptime` members are most useful for writing traits for parameterized types.
For example, imagine that you want to write a trait that describes a
parameterized stack data structure that stores elements that conform to the
`Copyable` trait.

By adding the element type as a `comptime` member on the trait, you can specify
parameterized methods on the trait:
...
```mojo
struct MyStack[type: Copyable & Deinitable](Stacklike):
    ...
    def dump[
        WritableEltType: Writable & Copyable & Deinitable
...
<!--
  FIXME: Move discussion to parameterized types. Trait docs only need
  to mention `comptime` members can be defined. If retaining the example,
    update it to use real conditional conformance. Update text below to point
    to real conditional conformance doc, if appropriate.  
-->
...
The following code exercises this new trait by defining a parameterized method,
