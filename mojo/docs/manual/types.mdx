...
[`comptime` values](/mojo/manual/metaprogramming/comptime-evaluation/#comptime-values)
...
[`Stringable`](/mojo/std/builtin/str/Stringable) trait, which indicates
that a value can be converted into a `String` using the `String(...)`
constructor.

The built-in [`print()`](/mojo/std/io/io/print) function accepts values
that conform to the `Writable` trait. For values that are only
`Stringable`, use `String(value)` first and then pass the resulting
`String` to `print()`.
...
Or, use the string constructor with variadic `Stringable` types,
...
[`Writable`](/mojo/std/format/Writable) representations of expressions
