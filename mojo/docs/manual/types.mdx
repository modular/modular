...
value to the [`UInt()`](/docs/std/builtin/uint/UInt#__init__) constructor.
...
Mojo strings provide Unicode-aware emoji support. You can count the number
of bytes, code points, and graphemes in a string using the `byte_length()`,
`codepoints()`, and `count_graphemes()` methods, respectively:
...
def show(label: StaticString, s: StringSlice):
    print(
        label,
        "bytes=", s.byte_length(),
        "codepoints=", len(s.codepoints()),
        "graphemes=", s.count_graphemes(),
    )
```

Mojo uses colon-delimited
[emoji shortcodes](https://www.unicode.org/emoji/charts/full-emoji-list.html),
also known as the "CLDR Short Name", which are human-readable names for
emoji:

```mojo
def main():
    show("family    ", ":man-woman-girl-boy:")  # 🧑‍🧑‍🧒‍🧒
    show("flag      ", ":us:")  # 🇺🇸
    show("wave      ", ":wave::skin-tone-4:")  # 👋🏽
    show("café      ", "café")   # NFD: e + U+0301
    show("namaste   ", "नमस्ते")
```
