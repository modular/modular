...
- `Array` now supports repetition with the `*` operator when its type `T` is
  `Copyable` and the multiplier is an integer literal. The array is consumed:
  its elements are copied into all but the last repetition and moved into the
  last one.
