...
for `self` in `__deinit__` and the argument in move constructors:
...
Used only in function types, `thin` indicates a function pointer type (not
a closure) and ensures the function value doesn't capture values from its
...
closure mechanism, `abi("C")` always appears together with `thin` in
function types:
...
keyword-only argument named `move`:
...
The `move` argument must use the `deinit` convention. Move
