...
:::note
Most types on this page require no imports. They're built into the
language or included in the Standard Library prelude, which is
automatically available. The exception is `Float4_e2m1fn`, which
requires `from std.builtin.simd import Float4_e2m1fn`.
:::
...
of supported operations. `DType.float8_e3m4` also exists as a dtype
value but has no `Scalar` alias; use `Scalar[DType.float8_e3m4]`
directly.
...
- `DType.float8_e3m4` has no arithmetic support at any stage,
...
constructor or cast. Mojo doesn't perform implicit numeric
