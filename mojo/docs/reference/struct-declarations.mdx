...
`ImplicitlyDeletable` when all members are also implicitly deletable. With
parameterized types, the parameter's traits must include `ImplicitlyDeletable`
for this to apply:
...
is safe to destroy using the built-in `__deinit__()` destructor:
...
Opt out with `ImplicitlyDeletable where False`.
:::
