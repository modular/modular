...
- A new [`@fieldwise_init`](/docs/manual/decorators/fieldwise-init) decorator
  ...
  marks the initializer as [`@implicit`](/docs/manual/decorators/implicit).
...
- `VariadicPack.each()` and `VariadicPack.each_idx()` methods have been removed.
  Use the [`@parameter
  for`](/docs/manual/decorators/parameter#parametric-for-statement)
