...
- You can now **`pip install mojo`**!
  ...
  [Mojo install guide](/docs/manual/install).
...
- We've released a new version of the Mojo extension for Visual Studio Code that
  ...
  [Add the VS Code extension](/docs/manual/install#add-the-vs-code-extension)
...
- Added [`os.atomic.fence()`](/docs/std/os/atomic/fence) for creating atomic
  memory fences.
...
- Add a memory ordering parameter to
  [`Atomic.load()`](/docs/std/os/atomic/Atomic).
...
- Nightly `mojo` Python wheels are now available. To install everything needed
  ...
  For more information, see the [Mojo install guide](/docs/manual/install).
