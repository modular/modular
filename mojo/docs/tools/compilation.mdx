...
startup code initializes the Mojo runtime — the thread pool that
parallel and asynchronous APIs such as
[`parallelize()`](/docs/std/algorithm/backend/cpu/parallelize/parallelize/) and
[`TaskGroup`](/docs/std/runtime/asyncrt/TaskGroup/) depend on. When the
process `main()` belongs to a C or C++ host instead, that startup code
never runs, so the runtime is never initialized. An exported function
that then uses a runtime-dependent API crashes with a segmentation
fault.

To fix this, call
[`initialize_runtime()`](/docs/std/runtime/asyncrt/initialize_runtime/)
