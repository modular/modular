...
Once connected, ask your assistant a Mojo question and it searches the docs
before answering, so the response reflects the current documentation. For
example, if you ask how to write a GPU kernel, it grounds its answer in the
current Mojo APIs instead of guessing from its training data.
...
```text
In Mojo, you write a GPU kernel as a function and launch it through a
DeviceContext. The kernel runs once per thread, and you use the thread and
block indices to select the element each thread handles:

1. Define the kernel function that operates on device buffers.
2. Get a DeviceContext for the target GPU.
3. Enqueue the kernel with the grid and block dimensions.
4. Synchronize, then copy the results back to the host.
