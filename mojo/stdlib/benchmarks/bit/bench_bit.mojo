...
fn next_power_of_two_int_v2(val: Int) -> Int:
    ...
    return 1 << (Int.BIT_WIDTH - count_leading_zeros(val - 1))
...
fn next_power_of_two_int_v3(val: Int) -> Int:
    ...
    return Int(
        mlir_value=v.le(1)
        .select(1, 1 << (Int.BIT_WIDTH - count_leading_zeros(v - 1)))
        .__index__()
    )
...
fn next_power_of_two_int_v4(val: Int) -> Int:
    return 1 << (
        (Int.BIT_WIDTH - count_leading_zeros(val - 1)) & splat(likely(val > 1))
    )
...
fn next_power_of_two_uint_v1(val: UInt) -> UInt:
    ...
    return UInt(1 << (UInt.BIT_WIDTH - count_leading_zeros(Int(val - 1))))
...
fn next_power_of_two_uint_v2(val: UInt) -> UInt:
    ...
    return UInt(
        mlir_value=v.eq(0)
        .select(1, 1 << (UInt.BIT_WIDTH - count_leading_zeros(v - 1)))
        .__index__()
    )
