...
struct _FormatCurlyEntry(CollectionElement, CollectionElementNew):
    ...
    fn format(fmt_src: StringSlice, args: Self._args_t) raises -> String:
        ...
        var offset = 0
        var ptr = fmt_src.unsafe_ptr().origin_cast[mut=False]()
        alias S = StringSlice[ptr.origin]

        @always_inline("nodebug")
        fn _build_slice(
            p: UnsafePointer[UInt8, mut=False, origin = ptr.origin],
            start: Int,
            end: Int,
        ) -> S:
    ...
    fn _handle_field_and_break(
        ...
        @always_inline("nodebug")
        fn _build_slice(
            p: UnsafePointer[UInt8, **_], start: Int, end: Int
        ) -> S:
            return S(
                ptr=p.origin_cast[mut=False, origin=ImmutableAnyOrigin]()
                + start,
                length=end - start,
            )
