...
struct Bench(Stringable, Writable):
    ...
    fn pad[
        ...
        """
        __comptime_assert (
            pad_str.byte_length() == 1
        ), "pad_str must be length 1."
    ...
    fn _get_max_name_width(self, label: StaticString) -> Int:
        ...
        for i in range(len(self.info_vec)):
            var namelen = String(self.info_vec[i].name).byte_length()
    ...
    fn _get_max_timing_widths(self, met_label: StaticString) -> List[Int]:
        ...
        var max_min = materialize[
            ConfigType.VERBOSE_TIMING_LABELS[0].byte_length()
        ]()
        var max_mean = materialize[
            ConfigType.VERBOSE_TIMING_LABELS[1].byte_length()
        ]()
        var max_max = materialize[
            ConfigType.VERBOSE_TIMING_LABELS[2].byte_length()
        ]()
        var max_dur = materialize[
            ConfigType.VERBOSE_TIMING_LABELS[3].byte_length()
        ]()
        for i in range(len(self.info_vec)):
            # TODO: Move met (ms) to the end of the table to align with verbose
            # timing, don't repeat `Mean (ms)`, and make sure it works with
