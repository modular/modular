...
struct Array[T: AnyType, length: Int](
    ...
    def __mul__(
        deinit self,
        rhs: IntLiteral,
        out result: Array[Self.T, Self.length * Int(IntLiteral[rhs.value]())],
    ) where (
        conforms_to(Self.T, Copyable) and (Int(IntLiteral[rhs.value]()) > 0),
        ...
        Args:
            rhs: The number of repetitions. Must be greater than 0.
        ...
        ```mojo
        var a = [1, 2] * 3  # Array[Int, 6](1, 2, 1, 2, 1, 2)
        ```
        """
        comptime n = Int(IntLiteral[rhs.value]())
        result = {uninitialized = True}
