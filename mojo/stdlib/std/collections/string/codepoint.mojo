...
struct Codepoint(Comparable, ImplicitlyCopyable, Intable, Movable, Writable):
    ...
    **Codepoints versus scalar values:**
    ...
    """The big-endian UTF-8 sequence represented by this type, padded by zeros
    if it is less than 4 bytes e.g. `Codepoint("a")` -> `vec = {97, 0, 0, 0}`.
    """
    ...
    def __init__(out self, *, unsafe_parse_first: StringSlice):
        ...
        var num_bytes = Self.utf8_byte_length(first_utf8_byte=b0)
    ...
    def __init__[
        ...
        self._values = {0}
        var num_bytes = UInt8(Self.utf8_byte_length(unsafe_unchecked_codepoint))
        ...
        comptime if optimize_ascii:
            ...
            var shift_c = splat[DType.uint8](Scalar[DType.bool](num_bytes > 2))
            var shift_d = splat[DType.uint8](Scalar[DType.bool](num_bytes > 3))
            ...
            var shift_b = splat[DType.uint8](Scalar[DType.bool](num_bytes > 1))
            var shift_c = splat[DType.uint8](Scalar[DType.bool](num_bytes > 2))
            var shift_d = splat[DType.uint8](Scalar[DType.bool](num_bytes > 3))
            var shift_mask = SIMD[DType.uint32, 4](
                UInt32(factor),
    ...
    def __lt__(self, other: Self) -> Bool:
        ...
        Notes:
            This does a UTF-8 byte-order comparison.
        """
        var s_vec: SIMD[DType.uint8, 4]
        var o_vec: SIMD[DType.uint8, 4]
        comptime if is_big_endian():
            s_vec = self._values
            o_vec = other._values
        else:
            s_vec = byte_swap(self._values)
            o_vec = byte_swap(other._values)
        return bitcast[DType.uint32, 1](s_vec) < bitcast[DType.uint32, 1](o_vec)
    ...
    def write_repr_to(self, mut writer: Some[Writer]):
        ...
        """
        fmt.FormatStruct(writer, "Codepoint").fields(self.to_u32())
    ...
    @staticmethod
    def unsafe_decode_utf8_to_utf32(
        string: StringSlice[mut=False, _]
    ) -> Tuple[UInt32, Int]:
        ...
        Notes:
            Using this function is faster than doing
            `Codepoint(unsafe_parse_first=str).to_u32()`
            especially if the call-site also requires the utf8 byte length.
        """
    ...
    @staticmethod
    @always_inline
    def utf8_byte_length(*, first_utf8_byte: UInt8) -> Int:
    ...
    def utf8_byte_length(self) -> Int:
        ...
        Notes:
            The returned value is always between 1 and 4 bytes.
        """
        return Self.utf8_byte_length(first_utf8_byte=self.first_byte())

    @staticmethod
    @always_inline
    def utf8_byte_length(codepoint: UInt32) -> Int:
        """Returns the number of UTF-8 bytes required to encode this character.
