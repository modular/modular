...
def _memchr2[
    ...
    if (
        ...
        for i in range(len(source)):
            if ptr[unsafe_offset=i] == char1 or ptr[unsafe_offset=i] == char2:
                return ptr.unsafe_offset(i)
        return {}
    else:
...
def _memchr2_impl[
    ...
    for i in range(0, vectorized_end, bool_mask_width):
        var block = haystack.unsafe_load[width=bool_mask_width](i)
    ...
    for i in range(vectorized_end, length):
        if (
            haystack[unsafe_offset=i] == char1
            or haystack[unsafe_offset=i] == char2
        ):
            return haystack.unsafe_offset(i)
