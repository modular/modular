...
struct Span[
    ...
    def map_reduce[
        ...
        *,
        maps_to_scalar: Bool,
        map_fn: def[w: Int](idx: Int) capturing -> SIMD[
        ...
        def run_fns[width: Int](idx: Int) unified {mut res_s, mut res_v}:
            var vec = map_fn[width](idx).cast[accum_dtype]()

            comptime if vec.size == 1:
    ...
    def count[
        ...
        @parameter
        def map_fn[w: Int](idx: Int) -> D[1]:
            var v = func(
                self.unsafe_ptr().load[width=w](idx)
            ).reduce_bit_count()
            return UInt(v)

        var res = self.map_reduce[
