...
struct Span[
    ...
    def find[
        ...
        comptime if unsafe_dont_normalize:
            ...
            var v = start + (_len & splat(start < 0))
            n_s = (v & splat(0 < v < _len)) + (_len & splat(v >= _len))
        var s_ptr = self.unsafe_ptr()
        var haystack = type_of(self)(
            ptr=s_ptr + n_s, length=_len - n_s
        ).get_immutable()
        var loc: Optional[type_of(haystack.unsafe_ptr())]

        comptime if from_left and not single_value:
        ...
        return (Int(loc.value()) - Int(s_ptr)) if loc else -1
