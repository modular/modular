...
fn to_integer(
    ...
    for i in range(CONTAINER_SIZE):
        if not (Byte(ord("0")) <= std_x_ptr[i] <= Byte(ord("9"))):
            ...
            var number_as_string = String(capacity=CONTAINER_SIZE)
            for j in range(CONTAINER_SIZE):
                number_as_string.append_byte(std_x_ptr[j])
            raise Error(
    ...
    if too_large:
        raise Error(
            ...
            StringSlice(ptr=std_x_ptr, length=len(standardized_x)).lstrip("0"),
            "'.",
        )
