...
fn get_errno() -> ErrNo:
    ...
    if CompilationTarget.is_linux():
        ...
        var errno = InlineArray[c_int, 1](0)
...
fn set_errno(errno: ErrNo):
    ...
    if CompilationTarget.is_linux():
        ...
        _ = external_call["_set_errno", c_int](errno.value)
    else:
...
struct ErrNo(Copyable, EqualityComparable, Movable, Stringable, Writable):
    ...
    """No child processes"""
    alias EAGAIN         = Self(pm["EAGAIN",           linux=11, macos=35, windows=11]())
    """Try again"""
    ...
    """Not a typewriter"""
    alias ETXTBSY        = Self(pm["ETXTBSY",          linux=26, macos=26, windows=139]())
    """Text file busy"""
    alias EFBIG          = Self(27)
    """File too large"""
    ...
    alias EDEADLK        = Self(pm["EDEADLK",          linux=35, macos=11, windows=36]())
    """Resource deadlock would occur"""
    alias ENAMETOOLONG   = Self(pm["ENAMETOOLONG",     linux=36, macos=63, windows=38]())
    """File name too long"""
    alias ENOLCK         = Self(pm["ENOLCK",           linux=37, macos=77, windows=39]())
    """No record locks available"""
    alias ENOSYS         = Self(pm["ENOSYS",           linux=38, macos=78, windows=40]())
    """Function not implemented"""
    alias ENOTEMPTY      = Self(pm["ENOTEMPTY",        linux=39, macos=66, windows=41]())
    """Directory not empty"""
    alias ELOOP          = Self(pm["ELOOP",            linux=40, macos=62, windows=114]())
    """Too many symbolic links encountered"""
    alias EWOULDBLOCK    = Self.EAGAIN
    """Operation would block"""
    alias ENOMSG         = Self(pm["ENOMSG",           linux=42, macos=91, windows=122]())
    """No message of desired type"""
    alias EIDRM          = Self(pm["EIDRM",            linux=43, macos=90, windows=111]())
    """Identifier removed"""
    ...
    alias ENOSTR         = Self(pm["ENOSTR",           linux=60, macos=99, windows=125]())
    """Device not a stream"""
    alias ENODATA        = Self(pm["ENODATA",          linux=61, macos=96, windows=120]())
    """No data available"""
    alias ETIME          = Self(pm["ETIME",            linux=62, macos=101, windows=137]())
    """Timer expired"""
    alias ENOSR          = Self(pm["ENOSR",            linux=63, macos=98, windows=124]())
    ...
    """Object is remote"""
    alias ENOLINK        = Self(pm["ENOLINK",          linux=67, macos=97, windows=121]())
    ...
    alias EPROTO         = Self(pm["EPROTO",           linux=71, macos=100, windows=134]())
    """Protocol error"""
    alias EMULTIHOP      = Self(pm["EMULTIHOP",        linux=72, macos=95]())
    """Multihop attempted"""
    alias EDOTDOT        = Self(pm["EDOTDOT",          linux=73]())
    """RFS specific error"""
    alias EBADMSG        = Self(pm["EBADMSG",          linux=74, macos=94, windows=104]())
    """Not a data message"""
    alias EOVERFLOW      = Self(pm["EOVERFLOW",        linux=75, macos=84, windows=132]())
    ...
    alias EILSEQ         = Self(pm["EILSEQ",           linux=84, macos=92, windows=42]())
    ...
    """Too many users"""
    alias ENOTSOCK       = Self(pm["ENOTSOCK",         linux=88, macos=38, windows=128]())
    """Socket operation on non-socket"""
    alias EDESTADDRREQ   = Self(pm["EDESTADDRREQ",     linux=89, macos=39, windows=109]())
    """Destination address required"""
    alias EMSGSIZE       = Self(pm["EMSGSIZE",         linux=90, macos=40, windows=115]())
    """Message too long"""
    alias EPROTOTYPE     = Self(pm["EPROTOTYPE",       linux=91, macos=41, windows=136]())
    """Protocol wrong type for socket"""
    alias ENOPROTOOPT    = Self(pm["ENOPROTOOPT",      linux=92, macos=42, windows=123]())
    """Protocol not available"""
    alias EPROTONOSUPPORT= Self(pm["EPROTONOSUPPORT",  linux=93, macos=43, windows=135]())
    """Protocol not supported"""
    alias ESOCKTNOSUPPORT= Self(pm["ESOCKTNOSUPPORT",  linux=94, macos=44]())
    """Socket type not supported"""
    alias EOPNOTSUPP     = Self(pm["EOPNOTSUPP",       linux=95, macos=102, windows=130]())
    """Operation not supported on transport endpoint"""
    alias EPFNOSUPPORT   = Self(pm["EPFNOSUPPORT",     linux=96, macos=46]())
    """Protocol family not supported"""
    alias EAFNOSUPPORT   = Self(pm["EAFNOSUPPORT",     linux=97, macos=47, windows=102]())
    """Address family not supported by protocol"""
    alias EADDRINUSE     = Self(pm["EADDRINUSE",       linux=98, macos=48, windows=100]())
    """Address already in use"""
    alias EADDRNOTAVAIL  = Self(pm["EADDRNOTAVAIL",    linux=99, macos=49, windows=101]())
    """Cannot assign requested address"""
    alias ENETDOWN       = Self(pm["ENETDOWN",         linux=100, macos=50, windows=116]())
    """Network is down"""
    alias ENETUNREACH    = Self(pm["ENETUNREACH",      linux=101, macos=51, windows=118]())
    """Network is unreachable"""
    alias ENETRESET      = Self(pm["ENETRESET",        linux=102, macos=52, windows=117]())
    """Network dropped connection because of reset"""
    alias ECONNABORTED   = Self(pm["ECONNABORTED",     linux=103, macos=53, windows=106]())
    """Software caused connection abort"""
    alias ECONNRESET     = Self(pm["ECONNRESET",       linux=104, macos=54, windows=108]())
    """Connection reset by peer"""
    alias ENOBUFS        = Self(pm["ENOBUFS",          linux=105, macos=55, windows=119]())
    """No buffer space available"""
    alias EISCONN        = Self(pm["EISCONN",          linux=106, macos=56, windows=113]())
    """Transport endpoint is already connected"""
    alias ENOTCONN       = Self(pm["ENOTCONN",         linux=107, macos=57, windows=126]())
    ...
    alias ETIMEDOUT      = Self(pm["ETIMEDOUT",        linux=110, macos=60, windows=138]())
    """Connection timed out"""
    alias ECONNREFUSED   = Self(pm["ECONNREFUSED",     linux=111, macos=61, windows=107]())
    """Connection refused"""
    alias EHOSTDOWN      = Self(pm["EHOSTDOWN",        linux=112, macos=64]())
    """Host is down"""
    alias EHOSTUNREACH   = Self(pm["EHOSTUNREACH",     linux=113, macos=65, windows=110]())
    """No route to host"""
    alias EALREADY       = Self(pm["EALREADY",         linux=114, macos=37, windows=103]())
    """Operation already in progress"""
    alias EINPROGRESS    = Self(pm["EINPROGRESS",      linux=115, macos=36, windows=112]())
    ...
    """Wrong medium type"""
    alias ECANCELED      = Self(pm["ECANCELED",        linux=125, macos=89, windows=105]())
    """Operation Canceled"""
    ...
    alias EOWNERDEAD     = Self(pm["EOWNERDEAD",       linux=130, macos=105, windows=133]())
    """Owner died"""
    alias ENOTRECOVERABLE= Self(pm["ENOTRECOVERABLE",  linux=131, macos=104, windows=127]())
    ...
    # macOS and windows
    alias ENOTSUP        = Self(pm["ENOTSUP",          macos=45, windows=129]())
