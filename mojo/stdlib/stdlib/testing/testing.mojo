...
# FIXME(#5250): This should take in a writable argument
@always_inline
fn _assert_error[
...
fn assert_equal(
    ...
    *,
    ...
    # compared
    if lhs != rebind[__type_of(lhs)](rhs):
        raise _assert_cmp_error["`left == right` comparison"](
...
struct assert_raises:
    ...
    fn __exit__(self, error: Error) raises -> Bool:
        ...
        if self.message_contains:
            # FIXME: some weird origin bug that only happens for PyObjectPtr
            # for macos 15 and ubuntu 24.04
            @no_inline
            @parameter
            fn evaluate() -> Bool:
