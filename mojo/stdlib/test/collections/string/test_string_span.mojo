...
def test_unsafe_strlen() raises:
    comptime simd_width = simd_width_of[DType.bool]()

    # Empty string: just a null terminator.
    var empty = List[Byte](capacity=1)
    empty.append(0)
    assert_equal(_unsafe_strlen(empty.unsafe_ptr()), 0)
    ...
    short_buf.append(0)
    assert_equal(_unsafe_strlen(short_buf.unsafe_ptr()), 2)
    ...
    sub_block.append(0)
    assert_equal(_unsafe_strlen(sub_block.unsafe_ptr()), simd_width - 1)
    ...
    assert_equal(
        _unsafe_strlen(multi_block.unsafe_ptr()), multi_block_len
    )
    ...
    long_buf.append(0)
    assert_equal(_unsafe_strlen(long_buf.unsafe_ptr(), 3), 3)

    # Bounded scan: max exactly equal to the string length.
    assert_equal(_unsafe_strlen(long_buf.unsafe_ptr(), 5), 5)

    # Bounded scan: max larger than the string length --- returns actual length.
    assert_equal(_unsafe_strlen(long_buf.unsafe_ptr(), 100), 5)
    ...
    assert_equal(_unsafe_strlen(unicode_buf.unsafe_ptr()), 2)


def test_unsafe_strlen_unaligned() raises:
    ...
    for i in range(capacity):
        buf[unsafe_offset=i] = Byte(0xFF)  # non-null sentinel
    ...
    for misalign in range(simd_width):
        var p = buf.unsafe_offset(aligned_offset + misalign)
        p[unsafe_offset=0] = Byte(ord("h"))
        p[unsafe_offset=1] = Byte(ord("i"))
        p[unsafe_offset=2] = 0
        assert_equal(_unsafe_strlen(p), 2)
        p[unsafe_offset=2] = Byte(0xFF)  # restore non-null for next iteration
