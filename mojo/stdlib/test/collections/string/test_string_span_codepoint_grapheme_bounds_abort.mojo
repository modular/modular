...
def test_string_codepoint_end_oob() raises:
    # Through `String`'s `codepoint=` accessor, which delegates to `StringSlice`.
    var s = String("🔄🔥🔄")
...
def test_grapheme_start_oob_no_end() raises:
    ...
    _assert_aborts(
        trigger,
        contains=(
            "slice start index 100 is out of bounds, valid range is 0 to 4"
        ),
    )


def test_grapheme_negative_end() raises:
    var s = StringSlice("café")
