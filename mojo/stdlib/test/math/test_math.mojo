...
fn test_isclose_numerics[*, symm: Bool]() raises:
    ...
    fn edge_val[symm: Bool](a: T, atol: T, rtol: T) -> T:
        ...
        @parameter
        if symm:
            return a + max(atol, abs(a) * rtol)
        else:
            return a + atol + a * rtol

    alias all_close: List[Tuple[T, T]] = [
        (T(1, 0), T(1, 0)),
        (T(atol), T(0)),
        (T(inf_, -inf_), T(inf_, -inf_)),
        (T(1), edge_val[symm](1, atol, 0)),
        (v, edge_val[symm](v, atol, 0)),
        (v, edge_val[symm](v, 0, rtol)),
        (v, edge_val[symm](v, atol, rtol)),
    ]
...
def test_isclose():
    # boolean
    assert_true(isclose(Scalar[DType.bool](True), Scalar[DType.bool](True)))
    assert_false(isclose(Scalar[DType.bool](True), Scalar[DType.bool](False)))

    # integer
    assert_true(isclose(UInt32(2), UInt32(2)))
    assert_false(isclose(UInt32(2), UInt32(3)))
