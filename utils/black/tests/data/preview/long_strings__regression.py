...
#   Path:   tests/data/preview/long_strings__regression.py
#
# ===----------------------------------------------------------------------=== #
...
stupid_format_method_bug = (
    ...
    " 'format' method to hang over the line length boundary. This is pretty"
    " annoying.".format("perfect", "length")
)
...
class A:
    def foo():
        os.system(
            "This is a regression test. xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx"
    ...
    for (xxx_xxxx, _xxx_xxx, _xxx_xxxxx, xxx_xxxx) in xxxx:
        for xxx in xxx_xxxx:
            assert ("x" in xxx) or (xxx in xxx_xxx_xxxxx), (
                "{0} xxxxxxx xx {1}, xxx {1} xx xxx xx xxxx xx xxx xxxx: xxx"
                " xxxx {2}".format(xxx_xxxx, xxx, xxxxxx.xxxxxxx(xxx_xxx_xxxxx))
            )
...
func_call_where_string_arg_has_method_call_and_bad_parens(
    "A long string with {}. This string is so long that it is ridiculous. It"
    " can't fit on one line at alllll.".format("formatting"),
)

func_call_where_string_arg_has_old_fmt_and_bad_parens(
    "A long string with {}. This string is so long that it is ridiculous. It"
    " can't fit on one line at alllll." % "formatting",
)

func_call_where_string_arg_has_old_fmt_and_bad_parens(
    "A long string with {}. This {} is so long that it is ridiculous. It can't"
    " fit on one line at alllll." % ("formatting", "string"),
)
...
xxxxxxx = {
    "xx": (
        "xxxx xxxxxxx xxxxxxxxx -x xxx -x /xxx/{0} -x xxx,xxx -xx {1} -xx {1}"
        " -xx xxx=xxx_xxxx,xxx_xx,xxx_xxx,xxx_xxxx,xxx_xx,xxx_xxx | xxxxxx -x"
        " xxxxxxxx -x xxxxxxxx -x"
    ),
    "xx": (
        "xxxx xxxxxxx xxxxxxxxx -x xxx -x /xxx/{0} -x xxx,xxx -xx {1} -xx {1}"
        " -xx xxx=xxx_xxxx_xxx_xxxx,xxx_xx_xxx_xxxx,xxx_xxxx_xxx_xxxx,xxx_xx_xxxx_xxxx,xxx_xxx_xxxx,xxx_xxx_xxxx"
    ...
    def foo(self):
        if True:
            xxxxx_xxxxxxxxxxxx(
                ...
                " xxxxxxx "
                + "xx xxxxxx xxxxxx xxxxxx xx xxxxxxx xxx xxx ${0} xx x"
                " xxxxxxxx xxxxx".xxxxxx(xxxxxx_xxxxxx_xxx)
            )
...
some_commented_string = (  # This comment gets thrown to the top.
    ...
    " it at allllllllllll".format("ha")  # comments here are fine
)
...
lpar_and_rpar_have_comments = func_call(  # LPAR Comment
    "Long really ridiculous type of string that shouldn't really even exist at"
    " all. I mean commmme onnn!!!",  # Comma Comment
)  # RPAR Comment
...
cmd_fstring = (
    "sudo -E deluge-console info --detailed --sort-reverse=time_added {'' if"
    f" ID is None else ID}} | perl -nE 'print if /^{field}:/'"
)

fstring = (
    "This string really doesn't need to be an {{fstring}}, but this one"
    f" most certainly, absolutely {does}."
)
...
(
    "my very long string that should get formatted if I'm careful to make sure"
    " it goes over 88 characters which it has now"
)
...
lpar_and_rpar_have_comments = func_call(  # LPAR Comment
    (  # Comma Comment
        "Long really ridiculous type of string that shouldn't really even exist"
        " at all. I mean commmme onnn!!!"
    ),
)  # RPAR Comment
