...
#   Path:   tests/data/simple_cases/composition_no_trailing_comma.py
#
# ===----------------------------------------------------------------------=== #
...
class C:
    def test(self) -> None:
        with patch("black.out", print):
            ...
            self.assertEqual(
                unstyle(str(report)),
                "1 file reformatted, 1 file left unchanged, 1 file failed to"
                " reformat.",
            )
            self.assertEqual(
                unstyle(str(report)),
    ...
    def tricky_asserts(self) -> None:
        ...
        assert expected == {
            ...
            "Not what we expected and the message is too long to fit in one"
            " line because it's too long"
        )
